...
用于验证通过回测生成的选股策略是否能在买点日期选出相同的股票
"""
...
def verify_strategy():
    ...
    try:
        ...
        # 解析策略配置
        logger.info("开始解析策略配置")
        strategy_parser = StrategyParser()
        strategy_file = os.path.join(root_dir, "config", "strategies", "backtest_strategy.yaml")
        ...
        result_file = os.path.join(get_result_dir(), "strategy_verification_report.csv")
