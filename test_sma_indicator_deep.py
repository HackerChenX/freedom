...
class SMAIndicatorDeepTester:
    ...
    def test_sma_instance_creation(self) -> Dict[str, Any]:
        """测试SMA指标实例创建"""
        print("🔍 测试SMA指标实例创建...")
        ...
        try:
            # 测试通过指标管理器创建
            sma_indicator = self.indicator_manager.create_indicator('SMA')
            
            if sma_indicator is not None:
                result['details']['manager_creation'] = True
                result['details']['indicator_name'] = getattr(sma_indicator, 'name', 'Unknown')
                result['details']['has_calculate_method'] = hasattr(sma_indicator, 'calculate')
                result['details']['has_get_signal_method'] = hasattr(sma_indicator, 'get_signal')
                
                # 检查基类继承
                from indicators.base_indicator import BaseIndicator
                result['details']['inherits_base_indicator'] = isinstance(sma_indicator, BaseIndicator)
                ...
                result['errors'].append("无法通过指标管理器创建SMA指标")
    ...
    def test_sma_calculate_method(self) -> Dict[str, Any]:
        """测试SMA指标calculate()方法"""
        print("🔍 测试SMA指标calculate()方法...")
        
        result = {
            'test_name': 'SMA指标calculate()测试',
            'passed': False,
            'details': {},
            'errors': []
        }
        ...
        try:
            ...
            if sma_indicator is None:
                result['errors'].append("无法创建SMA指标实例")
                return result
            
            # 测试计算功能
            calc_result = sma_indicator.calculate(self.test_data)
            ...
            if calc_result is not None and not calc_result.empty:
                ...
                result['details']['result_shape'] = calc_result.shape if hasattr(calc_result, 'shape') else 'N/A'
                
                # 对于Series类型，转换为DataFrame进行统一处理
                if isinstance(calc_result, pd.Series):
                    calc_result = calc_result.to_frame('sma_value')
                
                result['details']['columns'] = list(calc_result.columns) if hasattr(calc_result, 'columns') else []
                
                # 检查数值合理性
                if not calc_result.empty:
                    sma_values = calc_result.iloc[:, 0].dropna()  # 取第一列
                    if len(sma_values) > 0:
                        result['details']['sma_values_range'] = {
                            'min': float(sma_values.min()),
                            'max': float(sma_values.max()),
                            'mean': float(sma_values.mean())
                        }
                        result['details']['values_reasonable'] = all([
                            sma_values.min() > 0,  # SMA值应该为正数
                            sma_values.max() < 1000,  # 合理范围
                            not sma_values.isna().all()  # 不全是NaN
                        ])
    ...
    def test_sma_get_signal_method(self) -> Dict[str, Any]:
        """测试SMA指标get_signal()方法"""
        print("🔍 测试SMA指标get_signal()方法...")
        
        result = {
            'test_name': 'SMA指标get_signal()测试',
            'passed': False,
            'details': {},
            'errors': []
        }
        ...
        try:
            ...
            # 测试信号生成
            signal = sma_indicator.get_signal(self.test_data)
    ...
    def test_sma_data_validation(self) -> Dict[str, Any]:
        """测试SMA指标数据验证"""
        print("🔍 测试SMA指标数据验证...")
        
        result = {
            'test_name': 'SMA指标数据验证测试',
            'passed': False,
            'details': {},
            'errors': []
        }
        ...
        try:
            ...
            try:
                ...
                signal_empty = sma_indicator.get_signal(empty_data)
            ...
            try:
                ...
                signal_invalid = sma_indicator.get_signal(invalid_data)
            ...
            try:
                ...
                signal_insufficient = sma_indicator.get_signal(insufficient_data)
    ...
    def test_sma_performance(self) -> Dict[str, Any]:
        """测试SMA指标性能"""
        print("🔍 测试SMA指标性能...")
    ...
    def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        tests = [
            self.test_sma_instance_creation,
            self.test_sma_calculate_method,
            self.test_sma_get_signal_method,
            self.test_sma_data_validation,
            self.test_sma_performance
        ]
        ...
        if summary['overall_passed']:
            print("🎉 SMA指标100%通过所有测试，符合L4文档设计预期！")
        else:
            print("⚠️ SMA指标未能100%通过测试，需要进一步修复")
        
        return summary

def main():
    """主函数"""
    tester = SMAIndicatorDeepTester()
