...
class PeriodManager:
    ...
    def convert_period(self, data: pd.DataFrame, from_period: Period, to_period: Period) -> pd.DataFrame:
        ...
        # 根据目标周期设置重采样规则
        resample_rule = None
        if to_period == Period.MIN_15 and from_period == Period.MIN_5:
            resample_rule = '15min'
        elif to_period == Period.MIN_30 and from_period in [Period.MIN_5, Period.MIN_15]:
            resample_rule = '30min'
        elif to_period == Period.MIN_60 and from_period in [Period.MIN_5, Period.MIN_15, Period.MIN_30]:
            resample_rule = '60min'
        elif to_period == Period.DAILY:
            resample_rule = 'D'
        ...
        if resample_rule is None:
            logger.error(f"不支持的周期转换: {from_period.value} -> {to_period.value}")
            return pd.DataFrame()
