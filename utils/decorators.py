...
def cache_result_decorators(max_size: int = 128, ttl: Optional[float] = None):
    ...
    def decorator_decorators(func):
        ...
        wrapper_decorators.clear_cache = clear_cache_decorators
        wrapper_decorators.cache_info = get_cache_info
...
def validate_dataframe(check_empty=True, required_columns=None):
    ...
    return decorator
# 添加向后兼容别名
exception_handler = exception_handler_decorators


# 添加更多向后兼容别名
cache_result = cache_result_decorators
# safe_run already exists
# performance_monitor already exists
