...
class MAIndicatorDeepTester:
    ...
    def test_ma_instance_creation(self) -> Dict[str, Any]:
        """测试MA指标实例创建"""
        print("🔍 测试MA指标实例创建...")
        ...
        try:
            # 测试通过指标管理器创建
            ma_indicator = self.indicator_manager.create_indicator('MA')
            
            if ma_indicator is not None:
                result['details']['manager_creation'] = True
                result['details']['indicator_name'] = ma_indicator.name
                result['details']['has_calculate_method'] = hasattr(ma_indicator, 'calculate')
                result['details']['has_get_signal_method'] = hasattr(ma_indicator, 'get_signal')
                
                # 检查基类继承
                from indicators.base_indicator import BaseIndicator
                result['details']['inherits_base_indicator'] = isinstance(ma_indicator, BaseIndicator)
                ...
                result['errors'].append("无法通过指标管理器创建MA指标")
    ...
    def test_ma_calculate_method(self) -> Dict[str, Any]:
        """测试MA指标calculate()方法"""
        print("🔍 测试MA指标calculate()方法...")
        
        result = {
            'test_name': 'MA指标calculate()测试',
            'passed': False,
            'details': {},
            'errors': []
        }
        ...
        try:
            ...
            if ma_indicator is None:
                result['errors'].append("无法创建MA指标实例")
                return result
            
            # 测试计算功能
            calc_result = ma_indicator.calculate(self.test_data)
            ...
            if calc_result is not None and not calc_result.empty:
                ...
                # 检查是否包含MA相关列
                ma_columns = [col for col in calc_result.columns if 'MA' in col.upper() or col.lower().startswith('ma')]
                result['details']['ma_columns_count'] = len(ma_columns)
                result['details']['ma_columns'] = ma_columns
                
                # 检查数值合理性
                if ma_columns:
                    first_ma_col = ma_columns[0]
                    ma_values = calc_result[first_ma_col].dropna()
                    if len(ma_values) > 0:
                        result['details']['ma_values_range'] = {
                            'min': float(ma_values.min()),
                            'max': float(ma_values.max()),
                            'mean': float(ma_values.mean())
                        }
                        result['details']['values_reasonable'] = all([
                            ma_values.min() > 0,  # MA值应该为正数
                            ma_values.max() < 1000,  # 合理范围
                            not ma_values.isna().all()  # 不全是NaN
                        ])
                
                result['passed'] = all([
                    result['details']['calculation_success'],
                    result['details']['ma_columns_count'] > 0,
    ...
    def test_ma_get_signal_method(self) -> Dict[str, Any]:
        """测试MA指标get_signal()方法"""
        print("🔍 测试MA指标get_signal()方法...")
        
        result = {
            'test_name': 'MA指标get_signal()测试',
            'passed': False,
            'details': {},
            'errors': []
        }
        ...
        try:
            ...
            # 测试信号生成
            signal = ma_indicator.get_signal(self.test_data)
    ...
    def test_ma_data_validation(self) -> Dict[str, Any]:
        """测试MA指标数据验证"""
        print("🔍 测试MA指标数据验证...")
        
        result = {
            'test_name': 'MA指标数据验证测试',
            'passed': False,
            'details': {},
            'errors': []
        }
        ...
        try:
            ...
            try:
                ...
                signal_empty = ma_indicator.get_signal(empty_data)
            ...
            try:
                ...
                signal_invalid = ma_indicator.get_signal(invalid_data)
            ...
            try:
                ...
                signal_insufficient = ma_indicator.get_signal(insufficient_data)
    ...
    def test_ma_performance(self) -> Dict[str, Any]:
        """测试MA指标性能"""
        print("🔍 测试MA指标性能...")
    ...
    def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        tests = [
            self.test_ma_instance_creation,
            self.test_ma_calculate_method,
            self.test_ma_get_signal_method,
            self.test_ma_data_validation,
            self.test_ma_performance
        ]
        ...
        if summary['overall_passed']:
            print("🎉 MA指标100%通过所有测试，符合L4文档设计预期！")
        else:
            print("⚠️ MA指标未能100%通过测试，需要进一步修复")
        
        return summary

def main():
    """主函数"""
    tester = MAIndicatorDeepTester()
