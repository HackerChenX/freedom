...
def create_realistic_stock_data(n_periods=1000):
    ...
    # 生成OHLCV数据，特别设计一些CCI超买超卖形态
    data = []
    for i in range(n_periods):
        close = prices[i]
        
        # 每60个数据点插入一个明显的CCI超买超卖形态
        if i % 60 == 0 and i > 0:
            # CCI超买超卖形态：先超买后超卖
            cci_pattern = [1.08, 1.15, 1.22, 1.30, 1.25, 1.20, 1.15, 1.10, 1.05, 0.95, 0.90, 0.85, 0.80, 0.85, 0.90, 0.95]
            for j, multiplier in enumerate(cci_pattern):
                idx = i + j
                if idx < n_periods:
...
def quick_cci_test_adjusted():
    ...
    try:
        ...
        result = cci.calculate_Cci(test_data.head(100))
        
        if result is not None:
            # 查找CCI相关的列
            cci_columns = [col for col in result.columns if any(keyword in col for keyword in ['CCI', 'cci'])]
            print(f"  - 找到CCI相关列: {cci_columns[:10]}...")  # 只显示前10个
            
            if len(cci_columns) >= 1:  # 至少应该有1个CCI分析列
                # 过滤出数值列
                ...
                if len(numeric_columns) >= 1:
                    ...
                    # 验证CCI计算逻辑（基本验证）
                    if 'CCI_Cci' in result.columns:
                        cci_values = result['CCI_Cci'].dropna()
                        if len(cci_values) > 0:
                            print(f"    - ✅ CCI值验证通过: 均值{cci_values.mean():.2f}")
                        else:
                            print(f"    - ⚠️ CCI值验证异常: 无有效值")
                    
                    if algorithm_correct:
                        print(f"    - ✅ CCI算法验证通过: 严格符合商品通道指数理论")
                        stage1_score = 100.0
                    else:
                        print(f"    - ❌ CCI算法验证失败: 不符合标准公式")
                        stage1_score = 0
        ...
        # 参数管理测试
        param_tests = {
            'has_calculate_method': hasattr(cci, 'calculate_Cci'),
            'has_set_parameters': hasattr(cci, 'set_parameters_Cci'),
            'has_minimum_periods': hasattr(cci.__class__, 'minimum_periods'),
            'has_period_parameter': hasattr(cci, 'period')
        }
        ...
        error_handled = 0
        try:
            empty_result = cci.calculate_Cci(pd.DataFrame())
            error_handled += 1
        except:
            error_handled += 1
        
        try:
            invalid_result = cci.calculate_Cci(pd.DataFrame({'invalid': [1, 2, 3]}))
            error_handled += 1
        except:
            error_handled += 1
        ...
        result = cci.calculate_Cci(large_data)
        ...
        if result is not None:
            ...
            # 统计所有CCI信号
            cci_columns = [col for col in result.columns if col not in ['date', 'code']]
            for col in cci_columns:
                if result[col].dtype == 'bool':
                    # 对于布尔列，统计True值
                    true_signals = result[col].sum()
                    cci_signals += true_signals
                elif result[col].dtype in ['float64', 'int64']:
                    # 对于数值列，统计非零值
                    non_zero_signals = (result[col] != 0).sum()
                    cci_signals += non_zero_signals
            
            signal_ratio = cci_signals / len(large_data)
            
            print(f"  - CCI信号识别: {cci_signals}个")
        ...
        architecture_checks = {
            'has_calculate_method': hasattr(cci, 'calculate_Cci'),
            'has_minimum_periods_property': hasattr(cci.__class__, 'minimum_periods'),
            'inherits_from_base': isinstance(cci, BaseIndicator),  # 修复后的检查
            'proper_naming': 'Cci' in cci.__class__.__name__
        }
        
        stage4_score = (sum(architecture_checks.values()) / len(architecture_checks)) * 100
        print(f"  - 架构检查: {architecture_checks}")
        print(f"  - 继承链: {[cls.__name__ for cls in cci.__class__.__mro__[:5]]}")
        ...
        # 预热运行，避免首次运行的初始化开销
        _ = cci.calculate_Cci(test_data.head(10))
        
        # 多次测试取平均值，避免单次测试的偶然性
        times = []
        for _ in range(3):
            start_time = time.perf_counter()  # 使用更精确的计时器
            result = cci.calculate_Cci(test_data)
...
if __name__ == "__main__":
    success, details = quick_cci_test_adjusted()
