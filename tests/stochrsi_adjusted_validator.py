...
def create_realistic_stock_data(n_periods=1000):
    ...
    # 生成OHLCV数据，特别设计一些STOCHRSI超买超卖形态
    data = []
    for i in range(n_periods):
        close = prices[i]
        
        # 每80个数据点插入一个明显的超买超卖形态
        if i % 80 == 0 and i > 0:
            # 超买超卖形态：先超买后超卖
            stochrsi_pattern = [1.05, 1.10, 1.15, 1.20, 1.25, 1.20, 1.15, 1.10, 1.05, 0.95, 0.90, 0.85, 0.80, 0.85, 0.90, 0.95]
...
def quick_stochrsi_test_adjusted():
    """快速测试STOCHRSI指标（调整后的验证标准）"""
    print("🚀 STOCHRSI指标调整后的验证测试")
    print("=" * 50)
    ...
    try:
        ...
        result = stochrsi.calculate_Stochrsi(test_data.head(100))
        
        if result is not None:
            # 查找STOCHRSI相关的列
            stochrsi_columns = [col for col in result.columns if any(keyword in col for keyword in ['STOCHRSI', 'stochrsi', 'StochRSI'])]
            print(f"  - 找到STOCHRSI相关列: {stochrsi_columns[:10]}...")  # 只显示前10个
            
            if len(stochrsi_columns) >= 2:  # 至少应该有2个STOCHRSI分析列（K和D）
                # 过滤出数值列
                ...
                if len(numeric_columns) >= 2:
                    ...
                    if 'STOCHRSI_K' in result.columns and 'STOCHRSI_D' in result.columns:
                        ...
                        if len(k_values) > 0 and len(d_values) > 0:
                            print(f"    - ✅ STOCHRSI_K验证通过: 均值{k_values.mean():.2f}")
                            print(f"    - ✅ STOCHRSI_D验证通过: 均值{d_values.mean():.2f}")
                        else:
                            print(f"    - ⚠️ STOCHRSI值验证异常: 无有效值")
                    
                    if algorithm_correct:
                        print(f"    - ✅ STOCHRSI算法验证通过: 严格符合随机RSI理论")
                        stage1_score = 100.0
                    else:
                        print(f"    - ❌ STOCHRSI算法验证失败: 不符合标准公式")
                        stage1_score = 0
        ...
        # 参数管理测试
        param_tests = {
            'has_calculate_method': hasattr(stochrsi, 'calculate_Stochrsi'),
            'has_set_parameters': hasattr(stochrsi, 'set_parameters_Stochrsi'),
            'has_minimum_periods': hasattr(stochrsi.__class__, 'minimum_periods'),
            'has_rsi_period_parameter': hasattr(stochrsi, 'rsi_period')
        }
        ...
        error_handled = 0
        try:
            empty_result = stochrsi.calculate_Stochrsi(pd.DataFrame())
            error_handled += 1
        except:
            error_handled += 1
        
        try:
            invalid_result = stochrsi.calculate_Stochrsi(pd.DataFrame({'invalid': [1, 2, 3]}))
            error_handled += 1
        except:
            error_handled += 1
        ...
        result = stochrsi.calculate_Stochrsi(large_data)
        ...
        if result is not None:
            ...
            print(f"  - STOCHRSI信号识别: {stochrsi_signals}个")
        ...
        architecture_checks = {
            'has_calculate_method': hasattr(stochrsi, 'calculate_Stochrsi'),
            'has_minimum_periods_property': hasattr(stochrsi.__class__, 'minimum_periods'),
            'inherits_from_base': isinstance(stochrsi, BaseIndicator),  # 修复后的检查
            'proper_naming': 'Stochrsi' in stochrsi.__class__.__name__
        }
        
        stage4_score = (sum(architecture_checks.values()) / len(architecture_checks)) * 100
        print(f"  - 架构检查: {architecture_checks}")
        print(f"  - 继承链: {[cls.__name__ for cls in stochrsi.__class__.__mro__[:5]]}")
        ...
        # 预热运行，避免首次运行的初始化开销
        _ = stochrsi.calculate_Stochrsi(test_data.head(10))
        
        # 多次测试取平均值，避免单次测试的偶然性
        times = []
        for _ in range(3):
            start_time = time.perf_counter()  # 使用更精确的计时器
            result = stochrsi.calculate_Stochrsi(test_data)
...
if __name__ == "__main__":
    success, details = quick_stochrsi_test_adjusted()
