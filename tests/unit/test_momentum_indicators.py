...
class TestMomentum(unittest.TestCase, IndicatorTestMixin):
    def setUp(self):
        super().setUp()
        self.indicator = complete_registry.create_indicator('MOMENTUM', period=12, signal_period=6)
...
class TestMTM(unittest.TestCase, IndicatorTestMixin):
    def setUp(self):
        super().setUp()
        self.indicator = complete_registry.create_indicator('MTM', period=12, ma_period=6)
...
class TestRSIMA(unittest.TestCase, IndicatorTestMixin):
    def setUp(self):
        super().setUp()
        self.indicator = complete_registry.create_indicator('RSIMA', rsi_period=14, ma_periods=[6])
