...
class TestUnifiedIndicatorEngine(unittest.TestCase):
    """统一指标计算引擎测试类"""
    
    def setUp(self):
        """测试前准备"""
        self.engine = UnifiedIndicatorEngine(enable_cache=False)
    ...
    def test_validate_data(self):
        """测试数据验证功能"""
        # 测试有效数据
        self.assertTrue(self.engine._validate_data(self.test_data))
        
        # 测试空数据
        self.assertFalse(self.engine._validate_data(pd.DataFrame()))
        
        # 测试None数据
        self.assertFalse(self.engine._validate_data(None))
        
        # 测试缺少必要列的数据
        invalid_data = pd.DataFrame({'open': [1, 2, 3]})
        self.assertFalse(self.engine._validate_data(invalid_data))
    ...
    def test_calculate_ma(self):
        ...
        # 检查返回类型
        self.assertIsInstance(ma5, pd.Series)
        
        # 检查长度
        self.assertEqual(len(ma5), len(self.test_data))
        
        # 检查第5个值（应该是前5个收盘价的平均）
        expected_ma5_5th = self.test_data['close'][:5].mean()
        self.assertAlmostEqual(ma5.iloc[4], expected_ma5_5th, places=6)
        
        # 检查最后一个值
        expected_ma5_last = self.test_data['close'][-5:].mean()
        self.assertAlmostEqual(ma5.iloc[-1], expected_ma5_last, places=6)
    ...
    def test_calculate_ema(self):
        ...
        # 检查返回类型
        self.assertIsInstance(ema12, pd.Series)
        ...
        self.assertIsNotNone(ema12.iloc[-1])
    ...
    def test_calculate_sma(self):
        ...
        # 第一个值应该等于输入的第一个值
        self.assertEqual(sma_result.iloc[0], test_series.iloc[0])
    ...
    def test_calculate_macd(self):
        ...
        for key in expected_keys:
            self.assertIn(key, macd_result)
            self.assertIsInstance(macd_result[key], pd.Series)
            self.assertEqual(len(macd_result[key]), len(self.test_data))
    ...
    def test_calculate_kdj(self):
        ...
        # 检查返回类型
        self.assertIsInstance(kdj_result, dict)
        
        # 检查包含的键
        expected_keys = ['K', 'D', 'J']
        for key in expected_keys:
            self.assertIn(key, kdj_result)
            self.assertIsInstance(kdj_result[key], pd.Series)
            self.assertEqual(len(kdj_result[key]), len(self.test_data))
    ...
    def test_calculate_all_indicators(self):
        ...
        for indicator in expected_indicators:
            ...
            self.assertIsInstance(all_indicators[indicator], pd.Series)
    ...
    def test_get_indicator_value_at_date(self):
        ...
        self.assertIsInstance(ma5_value, float)
        self.assertFalse(np.isnan(ma5_value))
        ...
        self.assertIsInstance(kdj_values, dict)
