...
def quick_atr_revalidation_fixed():
    """快速重新验证ATR指标（使用修复后的架构检查）"""
    print("🚀 ATR指标修复后的重新验证测试")
    print("=" * 50)
    ...
    try:
        ...
        # 测试阶段1: 算法准确性
        print(f"\n📊 阶段1: 算法准确性测试")
        atr.set_parameters_Atr(period=14)
        result = atr.calculate(real_data.head(100))
        
        if result is not None:
            # 查找ATR相关的列
            atr_columns = [col for col in result.columns if any(keyword in col.upper() for keyword in ['ATR', 'TRUE', 'RANGE'])]
            print(f"  - 找到ATR相关列: {atr_columns}")
            
            if len(atr_columns) >= 1:  # 至少应该有ATR主线
                # 过滤出数值列
                numeric_columns = [col for col in atr_columns if col not in ['date', 'code'] and result[col].dtype in ['float64', 'int64']]
        ...
        # 参数管理测试
        param_tests = {
            'has_set_parameters': hasattr(atr, 'set_parameters_Atr'),
            'has_get_default_parameters': hasattr(atr, '_get_default_parameters_atr'),
            'has_calculate_method': hasattr(atr, 'calculate')
        }
        ...
        error_handled = 0
        try:
            empty_result = atr.calculate(pd.DataFrame())
            error_handled += 1
        except:
            error_handled += 1
        
        try:
            invalid_result = atr.calculate(pd.DataFrame({'invalid': [1, 2, 3]}))
            error_handled += 1
        except:
            error_handled += 1
        ...
        result = atr.calculate(large_data)
        
        if result is not None and len(atr_columns) > 0:
            # 过滤出数值列
            numeric_columns = [col for col in atr_columns if col not in ['date', 'code'] and result[col].dtype in ['float64', 'int64']]
            ...
            if len(numeric_columns) > 0:
                ...
                if len(values) > 20:
                    # 波动性识别测试（ATR用于衡量波动性）
                    high_volatility = (values > values.quantile(0.8)).sum()  # 高波动性
                    low_volatility = (values < values.quantile(0.2)).sum()   # 低波动性
                    total_signals = high_volatility + low_volatility
                    
                    signal_ratio = total_signals / len(values)
                    print(f"  - 高波动性: {high_volatility}个")
                    print(f"  - 低波动性: {low_volatility}个")
                    print(f"  - 信号比例: {signal_ratio:.4f}")
                    
                    # ATR应该能够识别波动性变化
                    if signal_ratio >= 0.3:  # 至少30%的波动性信号
                        signal_score = 100
                    ...
                    print(f"  - 波动性识别: {signal_score}/100")
        ...
        architecture_checks = {
            'has_calculate_method': hasattr(atr, 'calculate'),
            'has_set_parameters_method': hasattr(atr, 'set_parameters_Atr'),
            'inherits_from_base': isinstance(atr, BaseIndicator),  # 修复后的检查
            'proper_naming': 'ATR' in atr.__class__.__name__ or 'Atr' in atr.__class__.__name__
        }
        
        stage4_score = (sum(architecture_checks.values()) / len(architecture_checks)) * 100
        print(f"  - 架构检查: {architecture_checks}")
        print(f"  - 继承链: {[cls.__name__ for cls in atr.__class__.__mro__[:5]]}")
        ...
        result = atr.calculate(real_data)
        end_time = time.time()
