...
def quick_gann_tools_test_adjusted():
    """快速测试GANN_TOOLS指标（调整后的验证标准）"""
    print("🚀 GANN_TOOLS指标调整后的验证测试")
    print("=" * 50)
    ...
    try:
        ...
        result = gann_tools.calculate_Tools_Gann_Tools(test_data.head(100))
            ...
            if len(gann_columns) >= 5:  # 至少应该有5个江恩分析列
                ...
                if len(numeric_columns) > 0:
                    ...
                    if algorithm_correct:
                        ...
                        print(f"    - ❌ GANN_TOOLS算法验证失败: 不符合标准公式")
                        stage1_score = 0
        ...
        error_handled = 0
        try:
            empty_result = gann_tools.calculate_Tools_Gann_Tools(pd.DataFrame())
            error_handled += 1
        except:
            error_handled += 1
        ...
        print(f"  - 继承链: {[cls.__name__ for cls in gann_tools.__class__.__mro__[:5]]}")
        ...
        for _ in range(3):
            ...
            result = gann_tools.calculate_Tools_Gann_Tools(test_data)
...
if __name__ == "__main__":
    success, details = quick_gann_tools_test_adjusted()
