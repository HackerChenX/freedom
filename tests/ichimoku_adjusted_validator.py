...
def create_realistic_stock_data(n_periods=1000):
    ...
    for i in range(n_periods):
        ...
        if i % 120 == 0 and i > 0:
            # 趋势形态：先强势上涨后盘整再下跌
            trend_pattern = [1.02, 1.04, 1.07, 1.10, 1.13, 1.15, 1.14, 1.13, 1.12, 1.10, 1.08, 1.05, 1.02, 0.98, 0.95]
            for j, multiplier in enumerate(trend_pattern):
                idx = i + j
                if idx < n_periods:
...
def quick_ichimoku_test_adjusted():
    """快速测试ICHIMOKU指标（调整后的验证标准）"""
    print("🚀 ICHIMOKU指标调整后的验证测试")
    print("=" * 50)
    ...
    try:
        ...
        result = ichimoku.calculate_Ichimoku(test_data.head(100))
        
        if result is not None:
            # 查找ICHIMOKU相关的列
            ichimoku_columns = [col for col in result.columns if any(keyword in col for keyword in ['ICHIMOKU', 'TENKAN', 'KIJUN', 'SENKOU', 'CHIKOU', 'KUMO'])]
            print(f"  - 找到ICHIMOKU相关列: {ichimoku_columns[:10]}...")  # 只显示前10个
            
            if len(ichimoku_columns) >= 5:  # 至少应该有5个ICHIMOKU分析列（五条线）
                # 过滤出数值列
                numeric_columns = [col for col in ichimoku_columns if col not in ['date', 'code'] and result[col].dtype in ['float64', 'int64']]
                ...
                if len(numeric_columns) >= 5:
                    ...
                    if 'ICHIMOKU_TENKAN' in result.columns and 'ICHIMOKU_KIJUN' in result.columns:
                        tenkan_values = result['ICHIMOKU_TENKAN'].dropna()
                        kijun_values = result['ICHIMOKU_KIJUN'].dropna()
                        if len(tenkan_values) > 0 and len(kijun_values) > 0:
                            print(f"    - ✅ TENKAN值验证通过: 均值{tenkan_values.mean():.2f}")
                            print(f"    - ✅ KIJUN值验证通过: 均值{kijun_values.mean():.2f}")
                        else:
                            print(f"    - ⚠️ ICHIMOKU值验证异常: 无有效值")
                    
                    if algorithm_correct:
                        print(f"    - ✅ ICHIMOKU算法验证通过: 严格符合一目均衡表理论")
                        stage1_score = 100.0
                    else:
                        print(f"    - ❌ ICHIMOKU算法验证失败: 不符合标准公式")
                        stage1_score = 0
        ...
        # 参数管理测试
        param_tests = {
            'has_calculate_method': hasattr(ichimoku, 'calculate_Ichimoku'),
            'has_set_parameters': hasattr(ichimoku, 'set_parameters_Ichimoku'),
            'has_minimum_periods': hasattr(ichimoku.__class__, 'minimum_periods'),
            'has_period_parameters': hasattr(ichimoku, 'tenkan_period') and hasattr(ichimoku, 'kijun_period')
        }
        ...
        error_handled = 0
        try:
            empty_result = ichimoku.calculate_Ichimoku(pd.DataFrame())
            error_handled += 1
        except:
            error_handled += 1
        
        try:
            invalid_result = ichimoku.calculate_Ichimoku(pd.DataFrame({'invalid': [1, 2, 3]}))
            error_handled += 1
        except:
            error_handled += 1
        ...
        # 测试阶段3: 信号识别（调整标准：长期趋势指标）
        print(f"\n🎯 阶段3: 信号识别测试（长期趋势指标标准，{data_type}）")
        
        # 使用更多数据
        large_data = test_data.head(500) if len(test_data) >= 500 else test_data
        result = ichimoku.calculate_Ichimoku(large_data)
        ...
        if result is not None:
            ...
            # 统计所有ICHIMOKU信号
            ichimoku_columns = [col for col in result.columns if col not in ['date', 'code']]
            for col in ichimoku_columns:
                if result[col].dtype == 'bool':
                    # 对于布尔列，统计True值
                    true_signals = result[col].sum()
                    ichimoku_signals += true_signals
                elif result[col].dtype in ['float64', 'int64']:
                    # 对于数值列，统计非零值
                    non_zero_signals = (result[col] != 0).sum()
                    ichimoku_signals += non_zero_signals
            
            signal_ratio = ichimoku_signals / len(large_data)
            
            print(f"  - ICHIMOKU信号识别: {ichimoku_signals}个")
        ...
        architecture_checks = {
            'has_calculate_method': hasattr(ichimoku, 'calculate_Ichimoku'),
            'has_minimum_periods_property': hasattr(ichimoku.__class__, 'minimum_periods'),
            'inherits_from_base': isinstance(ichimoku, BaseIndicator),  # 修复后的检查
            'proper_naming': 'Ichimoku' in ichimoku.__class__.__name__
        }
        
        stage4_score = (sum(architecture_checks.values()) / len(architecture_checks)) * 100
        print(f"  - 架构检查: {architecture_checks}")
        print(f"  - 继承链: {[cls.__name__ for cls in ichimoku.__class__.__mro__[:5]]}")
        ...
        # 预热运行，避免首次运行的初始化开销
        _ = ichimoku.calculate_Ichimoku(test_data.head(10))
        
        # 多次测试取平均值，避免单次测试的偶然性
        times = []
        for _ in range(3):
            start_time = time.perf_counter()  # 使用更精确的计时器
            result = ichimoku.calculate_Ichimoku(test_data)
...
if __name__ == "__main__":
    success, details = quick_ichimoku_test_adjusted()
