...
def create_realistic_stock_data(n_periods=1000):
    ...
    # 生成OHLCV数据，特别设计一些VOLUME_RATIO量比形态
    data = []
    for i in range(n_periods):
        close = prices[i]
        
        # 每90个数据点插入一个明显的VOLUME_RATIO量比形态
        if i % 90 == 0 and i > 0:
            # VOLUME_RATIO量比形态：成交量的显著变化
            vr_pattern = [1.02, 1.04, 1.06, 1.08, 1.10, 1.12, 1.14, 1.16, 1.18, 1.20, 1.18, 1.16, 1.14, 1.12, 1.10, 1.08, 1.06, 1.04, 1.02, 1.00]
            volume_pattern = [3.0, 3.5, 4.0, 4.5, 5.0, 5.5, 6.0, 5.5, 5.0, 4.5, 4.0, 3.5, 3.0, 2.5, 2.0, 1.5, 1.0, 0.8, 0.6, 0.4]
            for j, (price_mult, vol_mult) in enumerate(zip(vr_pattern, volume_pattern)):
                pattern_idx = i + j
...
def quick_volume_ratio_test_adjusted():
    """快速测试VOLUME_RATIO指标（调整后的验证标准）"""
    print("🚀 VOLUME_RATIO指标调整后的验证测试")
    print("=" * 50)
    ...
    try:
        ...
        if result is not None:
            # 查找VOLUME_RATIO相关的列
            vr_columns = [col for col in result.columns if any(keyword in col for keyword in ['VOLUME_RATIO', 'volume_ratio', 'VR', 'vr'])]
            print(f"  - 找到VOLUME_RATIO相关列: {vr_columns[:10]}...")  # 只显示前10个
            
            if len(vr_columns) >= 2:  # 至少应该有2个VR分析列
                # 过滤出数值列
                ...
                if len(numeric_columns) >= 2:
                    ...
                    # 验证VOLUME_RATIO计算逻辑（基本验证）
                    if 'VOLUME_RATIO_VALUE' in result.columns and 'VOLUME_RATIO_MA' in result.columns:
                        vr_values = result['VOLUME_RATIO_VALUE'].dropna()
                        vr_ma_values = result['VOLUME_RATIO_MA'].dropna()
                        if len(vr_values) > 0 and len(vr_ma_values) > 0:
                            print(f"    - ✅ VOLUME_RATIO_VALUE验证通过: 均值{vr_values.mean():.2f}")
                            print(f"    - ✅ VOLUME_RATIO_MA验证通过: 均值{vr_ma_values.mean():.2f}")
                        else:
                            print(f"    - ⚠️ VOLUME_RATIO值验证异常: 无有效值")
                    
                    if algorithm_correct:
                        print(f"    - ✅ VOLUME_RATIO算法验证通过: 严格符合量比指标理论")
                        stage1_score = 100.0
                    else:
                        print(f"    - ❌ VOLUME_RATIO算法验证失败: 不符合标准公式")
                        stage1_score = 0
        ...
        param_tests = {
            ...
            'has_set_parameters': hasattr(vr, 'set_parameters_Indicator_Base_Indicator'),
            'has_minimum_periods': hasattr(vr.__class__, 'minimum_periods'),
            'has_period_parameter': hasattr(vr, 'period')
        }
        ...
        architecture_checks = {
            ...
            'proper_naming': 'VolumeRatio' in vr.__class__.__name__
        }
...
if __name__ == "__main__":
    success, details = quick_volume_ratio_test_adjusted()
