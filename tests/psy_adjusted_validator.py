...
def quick_psy_test_adjusted():
    ...
    try:
        # 使用PSY指标
        from indicators.psy import PsychologicalLine
        ...
        result = psy.calculate(test_data.head(100))
        ...
        error_handled = 0
        try:
            empty_result = psy.calculate(pd.DataFrame())
            error_handled += 1
        except:
            error_handled += 1
        
        try:
            invalid_result = psy.calculate(pd.DataFrame({'invalid': [1, 2, 3]}))
            error_handled += 1
        except:
            error_handled += 1
        ...
        print(f"  - 继承链: {[cls.__name__ for cls in psy.__class__.__mro__[:5]]}")
        ...
        # 预热运行，避免首次运行的初始化开销
        _ = psy.calculate(test_data.head(10))
        
        # 多次测试取平均值，避免单次测试的偶然性
        times = []
        for _ in range(3):
            start_time = time.perf_counter()  # 使用更精确的计时器
            result = psy.calculate(test_data)
...
if __name__ == "__main__":
    success, details = quick_psy_test_adjusted()
