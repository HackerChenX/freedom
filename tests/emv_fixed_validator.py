...
def quick_emv_test_fixed():
    """快速测试EMV指标（使用修复后的架构检查）"""
    print("🚀 EMV指标修复后的快速验证测试")
    print("=" * 50)
    
    try:
        from indicators.emv import EmvEmv
        ...
        # 测试阶段1: 算法准确性
        print(f"\n📊 阶段1: 算法准确性测试")
        emv.set_parameters_Emv(volume_divisor=10000, period=14)
        result = emv.calculate(real_data.head(100))
        
        if result is not None:
            # 查找EMV相关的列
            emv_columns = [col for col in result.columns if any(keyword in col.upper() for keyword in ['EMV', 'EASE', 'MOVEMENT'])]
            print(f"  - 找到EMV相关列: {emv_columns}")
            
            if len(emv_columns) >= 1:  # 至少应该有EMV主线
                # 过滤出数值列
                numeric_columns = [col for col in emv_columns if col not in ['date', 'code'] and result[col].dtype in ['float64', 'int64']]
        ...
        # 参数管理测试
        param_tests = {
            'has_set_parameters': hasattr(emv, 'set_parameters_Emv'),
            'has_get_default_parameters': hasattr(emv, '_get_default_parameters_emv'),
            'has_calculate_method': hasattr(emv, 'calculate')
        }
        ...
        error_handled = 0
        try:
            empty_result = emv.calculate(pd.DataFrame())
            error_handled += 1
        except:
            error_handled += 1
        
        try:
            invalid_result = emv.calculate(pd.DataFrame({'invalid': [1, 2, 3]}))
            error_handled += 1
        except:
            error_handled += 1
        ...
        result = emv.calculate(large_data)
        
        if result is not None and len(emv_columns) > 0:
            # 过滤出数值列
            numeric_columns = [col for col in emv_columns if col not in ['date', 'code'] and result[col].dtype in ['float64', 'int64']]
            ...
            if len(numeric_columns) > 0:
                ...
                if len(values) > 20:
                    # 趋势识别测试（EMV用于识别价格变动的难易程度）
                    positive_movement = (values > 0).sum()  # 正向移动
                    negative_movement = (values < 0).sum()  # 负向移动
                    total_signals = positive_movement + negative_movement
                    
                    signal_ratio = total_signals / len(values)
                    print(f"  - 正向移动: {positive_movement}个")
                    print(f"  - 负向移动: {negative_movement}个")
                    print(f"  - 信号比例: {signal_ratio:.4f}")
                    
                    # EMV应该能够识别价格移动的难易程度
                    if signal_ratio >= 0.6:  # 至少60%的移动信号
                        signal_score = 100
        ...
        architecture_checks = {
            'has_calculate_method': hasattr(emv, 'calculate'),
            'has_set_parameters_method': hasattr(emv, 'set_parameters_Emv'),
            'inherits_from_base': isinstance(emv, BaseIndicator),  # 修复后的检查
            'proper_naming': 'EMV' in emv.__class__.__name__ or 'Emv' in emv.__class__.__name__
        }
        
        stage4_score = (sum(architecture_checks.values()) / len(architecture_checks)) * 100
        print(f"  - 架构检查: {architecture_checks}")
        print(f"  - 继承链: {[cls.__name__ for cls in emv.__class__.__mro__[:5]]}")
        ...
        result = emv.calculate(real_data)
        end_time = time.time()
