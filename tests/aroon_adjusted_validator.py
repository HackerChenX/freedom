...
def create_realistic_stock_data(n_periods=1000):
    ...
    for i in range(n_periods):
        ...
        if i % 60 == 0 and i > 0:
            # 上升趋势：连续创新高
            trend_pattern = [1.01, 1.03, 1.05, 1.07, 1.09, 1.11, 1.13, 1.15]
...
def quick_aroon_test_adjusted():
    """快速测试AROON指标（调整后的验证标准）"""
    print("🚀 AROON指标调整后的验证测试")
    print("=" * 50)
    ...
    try:
        ...
        result = aroon.calculate_Aroon(test_data.head(100))
        
        if result is not None:
            # 查找AROON相关的列
            aroon_columns = [col for col in result.columns if any(keyword in col for keyword in ['aroon', 'AROON'])]
            print(f"  - 找到AROON相关列: {aroon_columns[:10]}...")  # 只显示前10个
            
            if len(aroon_columns) >= 3:  # 至少应该有3个AROON分析列（up, down, oscillator）
                # 过滤出数值列
                numeric_columns = [col for col in aroon_columns if col not in ['date', 'code'] and result[col].dtype in ['float64', 'int64']]
                ...
                if len(numeric_columns) >= 3:
                    ...
                    if 'aroon_up' in result.columns and 'aroon_down' in result.columns:
                        ...
                        if len(aroon_up_values) > 0 and len(aroon_down_values) > 0:
                            print(f"    - ✅ AROON UP验证通过: 均值{aroon_up_values.mean():.2f}")
                            print(f"    - ✅ AROON DOWN验证通过: 均值{aroon_down_values.mean():.2f}")
                        else:
                            print(f"    - ⚠️ AROON值验证异常: 无有效值")
                    
                    if algorithm_correct:
                        print(f"    - ✅ AROON算法验证通过: 严格符合阿隆指标理论")
                        stage1_score = 100.0
                    else:
                        print(f"    - ❌ AROON算法验证失败: 不符合标准公式")
                        stage1_score = 0
        ...
        # 参数管理测试
        param_tests = {
            'has_calculate_method': hasattr(aroon, 'calculate_Aroon'),
            'has_set_parameters': hasattr(aroon, 'set_parameters_Aroon'),
            'has_minimum_periods': hasattr(aroon, 'minimum_periods'),
            'has_period': hasattr(aroon, 'period')
        }
        ...
        error_handled = 0
        try:
            empty_result = aroon.calculate_Aroon(pd.DataFrame())
            error_handled += 1
        except:
            error_handled += 1
        
        try:
            invalid_result = aroon.calculate_Aroon(pd.DataFrame({'invalid': [1, 2, 3]}))
            error_handled += 1
        except:
            error_handled += 1
        ...
        result = aroon.calculate_Aroon(large_data)
        ...
        if result is not None:
            ...
            # 统计所有AROON信号
            aroon_columns = [col for col in result.columns if col not in ['date', 'code']]
            for col in aroon_columns:
                if result[col].dtype == 'bool':
                    # 对于布尔列，统计True值
                    true_signals = result[col].sum()
                    aroon_signals += true_signals
                elif result[col].dtype in ['float64', 'int64']:
                    # 对于数值列，统计非零值
                    non_zero_signals = (result[col] != 0).sum()
                    aroon_signals += non_zero_signals
            
            signal_ratio = aroon_signals / len(large_data)
            
            print(f"  - AROON信号识别: {aroon_signals}个")
        ...
        architecture_checks = {
            'has_calculate_method': hasattr(aroon, 'calculate_Aroon'),
            'has_minimum_periods_property': hasattr(aroon.__class__, 'minimum_periods'),
            'inherits_from_base': isinstance(aroon, BaseIndicator),  # 修复后的检查
            'proper_naming': 'Aroon' in aroon.__class__.__name__
        }
        
        stage4_score = (sum(architecture_checks.values()) / len(architecture_checks)) * 100
        print(f"  - 架构检查: {architecture_checks}")
        print(f"  - 继承链: {[cls.__name__ for cls in aroon.__class__.__mro__[:5]]}")
        ...
        # 预热运行，避免首次运行的初始化开销
        _ = aroon.calculate_Aroon(test_data.head(10))
        
        # 多次测试取平均值，避免单次测试的偶然性
        times = []
        for _ in range(3):
            start_time = time.perf_counter()  # 使用更精确的计时器
            result = aroon.calculate_Aroon(test_data)
...
if __name__ == "__main__":
    success, details = quick_aroon_test_adjusted()
