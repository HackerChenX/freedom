...
def quick_stochrsi_test():
    """快速测试STOCHRSI指标"""
    print("🚀 STOCHRSI指标快速验证测试")
    print("=" * 50)
    ...
    try:
        ...
        if result is not None:
            ...
            if len(stochrsi_columns) >= 2:  # 至少应该有K和D线
                # 检查数据有效性
                valid_data_count = 0
                for col in stochrsi_columns[:2]:  # 检查前2个主要列
                    values = result[col].dropna()
                    if len(values) > 0:
                        valid_data_count += 1
                        print(f"    - {col}: {len(values)}个有效值")
                
                stage1_score = (valid_data_count / 2) * 100
        ...
        architecture_checks = {
            ...
            'inherits_from_base': hasattr(stochrsi, '__bases__') and len(stochrsi.__class__.__bases__) > 0,
