...
def create_realistic_stock_data(n_periods=1000):
    ...
    for i in range(n_periods):
        ...
        if i % 80 == 0 and i > 0:
            # 趋势转向：先上涨后下跌
            trend_pattern = [1.02, 1.04, 1.06, 1.08, 1.10, 1.08, 1.06, 1.04, 1.02, 1.00]
...
def quick_sar_test_adjusted():
    ...
    try:
        ...
        result = sar.calculate_Sar(test_data.head(100))
        
        if result is not None:
            # 查找SAR相关的列
            sar_columns = [col for col in result.columns if any(keyword in col for keyword in ['sar', 'SAR', 'parabolic'])]
            print(f"  - 找到SAR相关列: {sar_columns[:10]}...")  # 只显示前10个
            
            if len(sar_columns) >= 1:  # 至少应该有1个SAR分析列
                # 过滤出数值列
                numeric_columns = [col for col in sar_columns if col not in ['date', 'code'] and result[col].dtype in ['float64', 'int64']]
                ...
                if len(numeric_columns) > 0:
                    ...
                    if 'SAR' in result.columns:
                        ...
                        if len(sar_values) > 0:
                            print(f"    - ✅ SAR值验证通过: 均值{sar_values.mean():.2f}")
                        else:
                            print(f"    - ⚠️ SAR值验证异常: 无有效值")
                    
                    if algorithm_correct:
                        print(f"    - ✅ SAR算法验证通过: 严格符合抛物线转向系统理论")
                        stage1_score = 100.0
                    else:
                        print(f"    - ❌ SAR算法验证失败: 不符合标准公式")
                        stage1_score = 0
        ...
        # 参数管理测试
        param_tests = {
            'has_calculate_method': hasattr(sar, 'calculate_Sar'),
            'has_set_parameters': hasattr(sar, 'set_parameters_Sar'),
            'has_minimum_periods': hasattr(sar, 'minimum_periods'),
            'has_acceleration': hasattr(sar, 'acceleration')
        }
        ...
        error_handled = 0
        try:
            empty_result = sar.calculate_Sar(pd.DataFrame())
            error_handled += 1
        except:
            error_handled += 1
        
        try:
            invalid_result = sar.calculate_Sar(pd.DataFrame({'invalid': [1, 2, 3]}))
            error_handled += 1
        except:
            error_handled += 1
        ...
        # 测试阶段3: 信号识别（调整标准：趋势跟踪指标）
        print(f"\n🎯 阶段3: 信号识别测试（趋势跟踪指标标准，{data_type}）")
        
        # 使用更多数据
        large_data = test_data.head(500) if len(test_data) >= 500 else test_data
        result = sar.calculate_Sar(large_data)
        ...
        if result is not None:
            ...
            # 统计所有SAR信号
            sar_columns = [col for col in result.columns if col not in ['date', 'code']]
            for col in sar_columns:
                if result[col].dtype == 'bool':
                    # 对于布尔列，统计True值
                    true_signals = result[col].sum()
                    sar_signals += true_signals
                elif result[col].dtype in ['float64', 'int64']:
                    # 对于数值列，统计非零值
                    non_zero_signals = (result[col] != 0).sum()
                    sar_signals += non_zero_signals
            
            signal_ratio = sar_signals / len(large_data)
            
            print(f"  - SAR信号识别: {sar_signals}个")
            print(f"  - 信号比例: {signal_ratio:.4f}")
            
            # 趋势跟踪指标，调整信号识别标准（2-5%要求）
        ...
        architecture_checks = {
            'has_calculate_method': hasattr(sar, 'calculate_Sar'),
            'has_minimum_periods_property': hasattr(sar.__class__, 'minimum_periods'),
            'inherits_from_base': isinstance(sar, BaseIndicator),  # 修复后的检查
            'proper_naming': 'Parabolic' in sar.__class__.__name__ or 'Sar' in sar.__class__.__name__
        }
        
        stage4_score = (sum(architecture_checks.values()) / len(architecture_checks)) * 100
        print(f"  - 架构检查: {architecture_checks}")
        print(f"  - 继承链: {[cls.__name__ for cls in sar.__class__.__mro__[:5]]}")
        ...
        # 预热运行，避免首次运行的初始化开销
        _ = sar.calculate_Sar(test_data.head(10))
        
        # 多次测试取平均值，避免单次测试的偶然性
        times = []
        for _ in range(3):
            start_time = time.perf_counter()  # 使用更精确的计时器
            result = sar.calculate_Sar(test_data)
...
if __name__ == "__main__":
    success, details = quick_sar_test_adjusted()
