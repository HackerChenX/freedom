...
def main_testmacdperfect():
    ...
    try:
        results = validator.validate_macd_patterns_perfect()
        ...
        output_file = f"perfect_validation_results_MACD_{timestamp}.json"
        ...
        if results['success_rate'] >= 1.0:
            print("\n🎉 完美！MACD指标达到100%成功率目标")
            return 0
        elif results['success_rate'] >= 0.8:
            print("\n✅ 优秀！MACD指标接近100%成功率目标")
            return 0
        else:
            print("\n❌ MACD指标未达到目标，需要继续优化")
            return 1
...
if __name__ == '__main__':
    exit_code = main_testmacdperfect()
    sys.exit(exit_code)