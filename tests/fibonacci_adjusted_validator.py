...
def create_realistic_stock_data(n_periods=1000):
    ...
    # 生成OHLCV数据，特别设计一些FIBONACCI回调形态
    data = []
    for i in range(n_periods):
        close = prices[i]
        
        # 每150个数据点插入一个明显的斐波那契回调形态
        if i % 150 == 0 and i > 0:
            # 斐波那契回调形态：先上涨到高点，然后按斐波那契比例回调
            fib_pattern = [1.05, 1.10, 1.15, 1.20, 1.25, 1.19, 1.13, 1.08, 1.05, 1.03, 1.05, 1.08, 1.12]
            for j, multiplier in enumerate(fib_pattern):
                idx = i + j
                if idx < n_periods:
...
def quick_fibonacci_test_adjusted():
    """快速测试FIBONACCI指标（调整后的验证标准）"""
    print("🚀 FIBONACCI指标调整后的验证测试")
    print("=" * 50)
    ...
    try:
        ...
        result = fibonacci.calculate_Fibonacci(test_data.head(100))
        
        if result is not None:
            # 查找FIBONACCI相关的列
            fib_columns = [col for col in result.columns if any(keyword in col for keyword in ['fib', 'FIBONACCI', 'FIB'])]
            print(f"  - 找到FIBONACCI相关列: {fib_columns[:10]}...")  # 只显示前10个
            
            if len(fib_columns) >= 5:  # 至少应该有5个FIBONACCI分析列（5个回调水平）
                # 过滤出数值列
                numeric_columns = [col for col in fib_columns if col not in ['date', 'code'] and result[col].dtype in ['float64', 'int64']]
                ...
                if len(numeric_columns) >= 5:
                    ...
                    # 验证FIBONACCI计算逻辑（基本验证）
                    if 'fib_ret_0.618' in result.columns and 'fib_ret_0.382' in result.columns:
                        fib618_values = result['fib_ret_0.618'].dropna()
                        fib382_values = result['fib_ret_0.382'].dropna()
                        if len(fib618_values) > 0 and len(fib382_values) > 0:
                            print(f"    - ✅ 斐波那契0.618验证通过: 均值{fib618_values.mean():.2f}")
                            print(f"    - ✅ 斐波那契0.382验证通过: 均值{fib382_values.mean():.2f}")
                        else:
                            print(f"    - ⚠️ FIBONACCI值验证异常: 无有效值")
                    
                    if algorithm_correct:
                        print(f"    - ✅ FIBONACCI算法验证通过: 严格符合斐波那契理论")
                        stage1_score = 100.0
                    else:
                        print(f"    - ❌ FIBONACCI算法验证失败: 不符合标准公式")
                        stage1_score = 0
        ...
        # 参数管理测试
        param_tests = {
            'has_calculate_method': hasattr(fibonacci, 'calculate_Fibonacci'),
            'has_set_parameters': hasattr(fibonacci, 'set_parameters_Fibonacci'),
            'has_minimum_periods': hasattr(fibonacci.__class__, 'minimum_periods'),
            'has_period_parameter': hasattr(fibonacci, 'period')
        }
        ...
        error_handled = 0
        try:
            empty_result = fibonacci.calculate_Fibonacci(pd.DataFrame())
            error_handled += 1
        except:
            error_handled += 1
        
        try:
            invalid_result = fibonacci.calculate_Fibonacci(pd.DataFrame({'invalid': [1, 2, 3]}))
            error_handled += 1
        except:
            error_handled += 1
        ...
        result = fibonacci.calculate_Fibonacci(large_data)
        ...
        if result is not None:
            ...
            # 统计所有FIBONACCI信号
            fib_columns = [col for col in result.columns if col not in ['date', 'code']]
            for col in fib_columns:
                if result[col].dtype == 'bool':
                    # 对于布尔列，统计True值
                    true_signals = result[col].sum()
                    fib_signals += true_signals
                elif result[col].dtype in ['float64', 'int64']:
                    # 对于数值列，统计非零值
                    non_zero_signals = (result[col] != 0).sum()
                    fib_signals += non_zero_signals
            
            signal_ratio = fib_signals / len(large_data)
            
            print(f"  - FIBONACCI信号识别: {fib_signals}个")
        ...
        architecture_checks = {
            'has_calculate_method': hasattr(fibonacci, 'calculate_Fibonacci'),
            'has_minimum_periods_property': hasattr(fibonacci.__class__, 'minimum_periods'),
            'inherits_from_base': isinstance(fibonacci, BaseIndicator),  # 修复后的检查
            'proper_naming': 'Fibonacci' in fibonacci.__class__.__name__
        }
        
        stage4_score = (sum(architecture_checks.values()) / len(architecture_checks)) * 100
        print(f"  - 架构检查: {architecture_checks}")
        print(f"  - 继承链: {[cls.__name__ for cls in fibonacci.__class__.__mro__[:5]]}")
        ...
        # 预热运行，避免首次运行的初始化开销
        _ = fibonacci.calculate_Fibonacci(test_data.head(10))
        
        # 多次测试取平均值，避免单次测试的偶然性
        times = []
        for _ in range(3):
            start_time = time.perf_counter()  # 使用更精确的计时器
            result = fibonacci.calculate_Fibonacci(test_data)
...
if __name__ == "__main__":
    success, details = quick_fibonacci_test_adjusted()
