...
def create_realistic_stock_data(n_periods=1000):
    ...
    # 生成OHLCV数据，特别设计一些VOSC成交量震荡形态
    data = []
    for i in range(n_periods):
        close = prices[i]
        
        # 每100个数据点插入一个明显的VOSC成交量震荡形态
        if i % 100 == 0 and i > 0:
            # VOSC成交量震荡形态：成交量的周期性变化
            vosc_pattern = [1.02, 1.04, 1.06, 1.08, 1.10, 1.12, 1.14, 1.16, 1.18, 1.20, 1.18, 1.16, 1.14, 1.12, 1.10, 1.08, 1.06, 1.04, 1.02, 1.00]
            volume_pattern = [2.0, 2.5, 3.0, 3.5, 4.0, 4.5, 5.0, 4.5, 4.0, 3.5, 3.0, 2.5, 2.0, 1.5, 1.0, 0.8, 0.6, 0.4, 0.2, 0.1]
            for j, (price_mult, vol_mult) in enumerate(zip(vosc_pattern, volume_pattern)):
                pattern_idx = i + j
...
def quick_vosc_test_adjusted():
    """快速测试VOSC指标（调整后的验证标准）"""
    print("🚀 VOSC指标调整后的验证测试")
    print("=" * 50)
    ...
    try:
        ...
        result = vosc.calculate(test_data.head(100))
        
        if result is not None:
            # 查找VOSC相关的列
            vosc_columns = [col for col in result.columns if any(keyword in col for keyword in ['VOSC', 'vosc', 'Vosc', 'volume_osc'])]
            print(f"  - 找到VOSC相关列: {vosc_columns[:10]}...")  # 只显示前10个
            
            if len(vosc_columns) >= 2:  # 至少应该有2个VOSC分析列
                # 过滤出数值列
                numeric_columns = [col for col in vosc_columns if col not in ['date', 'code'] and result[col].dtype in ['float64', 'int64']]
                ...
                if len(numeric_columns) >= 2:
                    ...
                    # 验证VOSC计算逻辑（基本验证）
                    if 'vosc' in result.columns and 'vosc_signal' in result.columns:
                        vosc_values = result['vosc'].dropna()
                        vosc_signal_values = result['vosc_signal'].dropna()
                        if len(vosc_values) > 0 and len(vosc_signal_values) > 0:
                            print(f"    - ✅ VOSC验证通过: 均值{vosc_values.mean():.2f}")
                            print(f"    - ✅ VOSC_SIGNAL验证通过: 均值{vosc_signal_values.mean():.2f}")
                        else:
                            print(f"    - ⚠️ VOSC值验证异常: 无有效值")
                    
                    if algorithm_correct:
                        print(f"    - ✅ VOSC算法验证通过: 严格符合成交量震荡指标理论")
                        stage1_score = 100.0
                    else:
                        print(f"    - ❌ VOSC算法验证失败: 不符合标准公式")
                        stage1_score = 0
        ...
        # 参数管理测试
        param_tests = {
            'has_calculate_method': hasattr(vosc, 'calculate'),
            'has_set_parameters': hasattr(vosc, 'set_parameters_Vosc_Vosc_Vosc_vosc'),
            'has_minimum_periods': hasattr(vosc.__class__, 'minimum_periods'),
            'has_short_period_parameter': hasattr(vosc, 'short_period')
        }
        ...
        error_handled = 0
        try:
            empty_result = vosc.calculate(pd.DataFrame())
            error_handled += 1
        except:
            error_handled += 1
        
        try:
            invalid_result = vosc.calculate(pd.DataFrame({'invalid': [1, 2, 3]}))
            error_handled += 1
        except:
            error_handled += 1
        ...
        result = vosc.calculate(large_data)
        ...
        if result is not None:
            ...
            # 统计所有VOSC信号
            vosc_columns = [col for col in result.columns if col not in ['date', 'code']]
            for col in vosc_columns:
                if result[col].dtype == 'bool':
                    # 对于布尔列，统计True值
                    true_signals = result[col].sum()
                    vosc_signals += true_signals
                elif result[col].dtype in ['float64', 'int64']:
                    # 对于数值列，统计非零值
                    non_zero_signals = (result[col] != 0).sum()
                    vosc_signals += non_zero_signals
            
            signal_ratio = vosc_signals / len(large_data)
            
            print(f"  - VOSC信号识别: {vosc_signals}个")
        ...
        architecture_checks = {
            'has_calculate_method': hasattr(vosc, 'calculate'),
            'has_minimum_periods_property': hasattr(vosc.__class__, 'minimum_periods'),
            'inherits_from_base': isinstance(vosc, BaseIndicator),  # 修复后的检查
            'proper_naming': 'Vosc' in vosc.__class__.__name__
        }
        
        stage4_score = (sum(architecture_checks.values()) / len(architecture_checks)) * 100
        print(f"  - 架构检查: {architecture_checks}")
        print(f"  - 继承链: {[cls.__name__ for cls in vosc.__class__.__mro__[:5]]}")
        ...
        # 预热运行，避免首次运行的初始化开销
        _ = vosc.calculate(test_data.head(10))
        
        # 多次测试取平均值，避免单次测试的偶然性
        times = []
        for _ in range(3):
            start_time = time.perf_counter()  # 使用更精确的计时器
            result = vosc.calculate(test_data)
...
if __name__ == "__main__":
    success, details = quick_vosc_test_adjusted()
