...
class TestDateManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch('analysis.engines.date_manager.get_clickhouse_db') as mock_db:
            ...
            self.date_manager = DateManager(cache_size=100, cache_ttl=3600)
    
    def test_current_datetime_and_date(self):
        """测试获取当前日期时间"""
        # 测试获取当前datetime
        current_dt = self.date_manager.get_current_datetime()
        self.assertIsInstance(current_dt, datetime.datetime)
        ...
        current_date_yyyymmdd = self.date_manager.get_current_date(DateFormat.YYYYMMDD)
        self.assertRegex(current_date_yyyymmdd, r'\d{8}')
        
        # 测试获取当前日期字符串 - 时间戳格式
        current_timestamp = self.date_manager.get_current_date(DateFormat.TIMESTAMP)
        self.assertIsInstance(int(current_timestamp), int)
    ...
    def test_parse_date_datetime_input(self):
        ...
        self.assertEqual(parsed_dt, test_dt)
    ...
    def test_parse_date_pandas_timestamp(self):
        ...
        self.assertEqual(parsed_dt, test_ts.to_pydatetime())
    ...
    def test_parse_date_string_with_format(self):
        ...
        # YYYYMMDD格式
        date_str = "20240115"
        parsed_dt = self.date_manager.parse_date(date_str, DateFormat.YYYYMMDD)
        expected_dt = datetime.datetime(2024, 1, 15)
        self.assertEqual(parsed_dt, expected_dt)
        
        # YYYY-MM-DD格式
        date_str = "2024-01-15"
        parsed_dt = self.date_manager.parse_date(date_str, DateFormat.YYYY_MM_DD)
        expected_dt = datetime.datetime(2024, 1, 15)
        self.assertEqual(parsed_dt, expected_dt)
        
        # 时间戳字符串格式
        timestamp_str = "1705315800"
        parsed_dt = self.date_manager.parse_date(timestamp_str, DateFormat.TIMESTAMP)
    ...
    def test_format_date(self):
        ...
        # YYYYMMDD格式
        formatted = self.date_manager.format_date(test_dt, DateFormat.YYYYMMDD)
        self.assertEqual(formatted, "20240115")
        
        # 时间戳格式
        formatted = self.date_manager.format_date(test_dt, DateFormat.TIMESTAMP)
        expected_timestamp = str(int(test_dt.timestamp()))
        self.assertEqual(formatted, expected_timestamp)
    ...
    def test_get_latest_trading_date_from_cache(self):
        ...
        self.assertEqual(result, cached_date)
    ...
    def test_get_previous_trading_dates(self):
        ...
        self.assertEqual(len(previous_dates), 3)
        # 应该是周五、周四、周三
        expected_dates = ["2024-01-12", "2024-01-11", "2024-01-10"]
        self.assertEqual(previous_dates, expected_dates)
    ...
    def test_get_next_trading_dates(self):
        ...
        self.assertEqual(len(next_dates), 3)
        # 应该是周四、周五、下周一
        expected_dates = ["2024-01-11", "2024-01-12", "2024-01-15"]
        self.assertEqual(next_dates, expected_dates)
    ...
    def test_get_predefined_date_range(self):
        ...
        end_date, date_list = self.date_manager.get_predefined_date_range(
            DateRange.LAST_7_DAYS, base_date, trading_days_only=True
        )
        
        self.assertEqual(end_date, "2024-01-15")
        self.assertIsInstance(date_list, list)
        self.assertGreater(len(date_list), 0)
    ...
    def test_get_trading_days_between(self):
        ...
        self.assertEqual(trading_days, 5)
    ...
    def test_add_trading_days_zero(self):
        ...
        self.assertEqual(result_date, base_date)
    ...
    def test_validate_date_range_valid(self):
        ...
        # 有效范围
        self.assertTrue(self.date_manager.validate_date_range(start_date, end_date))
        
        # 有效范围且在最大天数限制内
        self.assertTrue(self.date_manager.validate_date_range(start_date, end_date, max_days=10))
    ...
    def test_validate_date_range_invalid(self):
        ...
        # 无效范围（开始日期晚于结束日期）
        self.assertFalse(self.date_manager.validate_date_range(start_date, end_date))
        
        # 超出最大天数限制
        start_date = "2024-01-01"
        end_date = "2024-01-31"
        self.assertFalse(self.date_manager.validate_date_range(start_date, end_date, max_days=10))
    ...
    def test_get_time_series_dates_trading_days(self):
        ...
        # 应该包含5个交易日
        self.assertEqual(len(date_index), 5)
