...
2.  **单元测试修复**：修复与该指标相关的所有单元测试，确保逻辑正确。
3.  **集成测试验证**：确保指标在组合或策略中能正确工作。
4.  **文档更新**：更新指标相关的文档和注释。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **MACD** | `indicators/macd.py` | `test_macd.py`, `test_macd_simple.py` | **已完成** | **是** | 通过 `pytest` 验证了核心测试。 |
| **KDJ** | `indicators/kdj.py` | `tests/unit/test_kdj.py` | **已完成** | **是** | 通过所有单元测试。 |
| **RSI** | `indicators/rsi.py` | `tests/unit/test_rsi.py` (待确认) | **进行中** | 否 | **当前焦点** |
