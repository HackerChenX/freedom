...
class KdjKdj(BaseIndicator, PatternSignalMixin, MinimumPeriodsMixin):
    ...
    def get_patterns_Kdj(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        if len(data) > 0:
            ...
            patterns_df["KDJ_BEARISH_DIVERGENCE"] = (
                patterns_df["KDJ_BEARISH_DIVERGENCE"].astype("boolean").fillna(False)
            )
    ...
    def calculate_confidence_Kdj(self, score: pd.Series, patterns: pd.DataFrame, signals: dict) -> float:
        ...
        # 3. 三线排列因素: K,D,J同向发散则置信度高  # TODO: 将魔法数字提取到配置中
        if len(self.result) >= 3:  # TODO: 将魔法数字提取到配置中
    ...
    def calculate_score_Kdj(self, data: pd.DataFrame, **kwargs) -> dict:
        ...
        return {"score": final_score, "confidence": confidence, "patterns": patterns, "raw_score": raw_score}

    def _detect_overbought(self, data: pd.DataFrame) -> pd.Series:
        """检测KDJ超买形态"""
        if "K" not in data.columns:
            return pd.Series([False] * len(data), index=data.index)
        return data["K"] > 80.0  # TODO: 将魔法数字提取到配置中

    def _detect_oversold(self, data: pd.DataFrame) -> pd.Series:
        """检测KDJ超卖形态"""
        if "K" not in data.columns:
            return pd.Series([False] * len(data), index=data.index)
        return data["K"] < 20.0  # TODO: 将魔法数字提取到配置中
    ...
    def _detect_golden_cross(self, series1: pd.Series, series2: pd.Series) -> pd.Series:
        """检测金叉"""
        return self._detect_robust_crossover(
            series1, series2, window=3, cross_type="above"
        )  # TODO: 将魔法数字提取到配置中

    def _detect_death_cross(self, series1: pd.Series, series2: pd.Series) -> pd.Series:
        """检测死叉"""
        return self._detect_robust_crossover(
            series1, series2, window=3, cross_type="below"
        )  # TODO: 将魔法数字提取到配置中
    ...
    def _detect_bullish_divergence(self, data: pd.DataFrame) -> bool:
        ...
        try:
            ...
            k_values = data["K"].iloc[-20:].values  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            ...
            # 确保两个低点之间的距离足够
            if abs(idx1 - idx2) < 3:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
                return False
    ...
    def add_signals(self, data: pd.DataFrame, k_col: str = "K", d_col: str = "D", j_col: str = "J") -> pd.DataFrame:
        ...
        # 计算超买超卖信号
        result["kdj_overbought"] = (result[k_col] > 80) & (
            result[d_col] > 80
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        result["kdj_oversold"] = (result[k_col] < 20) & (
            result[d_col] < 20
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
    ...
    def generate_trading_signals_Kdj(self, data: pd.DataFrame, **kwargs) -> Dict[str, pd.Series]:
        ...
        if result is None or result.empty:
            return {"buy_signal": False, "sell_signal": False, "overbought": False, "oversold": False}

        signals = {}

        # 基本信号
        signals["buy_signal"] = (
            result["kdj_buy_signal"] if "kdj_buy_signal" in result.columns else pd.Series(False, index=result.index)
        )
        signals["sell_signal"] = (
            result["kdj_sell_signal"] if "kdj_sell_signal" in result.columns else pd.Series(False, index=result.index)
        )

        # 超买超卖信号
        signals["overbought"] = (
            result["kdj_overbought"] if "kdj_overbought" in result.columns else pd.Series(False, index=result.index)
        )
        signals["oversold"] = (
            result["kdj_oversold"] if "kdj_oversold" in result.columns else pd.Series(False, index=result.index)
        )

        # 趋势信号
        signals["bull_trend"] = (
            result["kdj_uptrend"] if "kdj_uptrend" in result.columns else pd.Series(False, index=result.index)
        )
        signals["bear_trend"] = (
            result["kdj_downtrend"] if "kdj_downtrend" in result.columns else pd.Series(False, index=result.index)
        )

        # J值超买超卖信号
        signals["j_overbought"] = (
            result["kdj_j_overbought"] if "kdj_j_overbought" in result.columns else pd.Series(False, index=result.index)
        )
        signals["j_oversold"] = (
            result["kdj_j_oversold"] if "kdj_j_oversold" in result.columns else pd.Series(False, index=result.index)
        )
    ...
    def _calculate_signal_strength(self, data: pd.DataFrame, is_buy: bool = True) -> pd.Series:
        ...
        if is_buy:
            # 买入信号强度
            # 1. 超卖区间加分
            strength = np.where(k < 20, strength + 1, strength)  # TODO: 将魔法数字提取到配置中
    ...
    def calculate_raw_score_Kdj(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        position_score = np.where(
            k <= 20, 40 + (20 - k) * 1.5, position_score
        )  # 超卖区加分  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        position_score = np.where(
    ...
    def _detect_stagnation(
        ...
        if low_threshold is not None:
            low_position = (
                (k < low_threshold) | (d < low_threshold) | (j < low_threshold * 0.8)
            )  # TODO: 将魔法数字提取到配置中
    ...
    def _detect_divergence(self, price: pd.Series, k: pd.Series) -> Optional[str]:
        ...
        # 找出最近的两个低点
        min_periods = 5  # TODO: 将魔法数字提取到配置中
        ...
        # 找出最近30个周期内的低点和高点
        recent = slice(-30, None)  # TODO: 将魔法数字提取到配置中
    ...
    def _calculate_divergence_strength(
        self, price: pd.Series, indicator: pd.Series, is_bullish: bool = True, lookback: int = 20
    ) -> float:  # TODO: 将魔法数字提取到配置中
        """
        计算背离强度
        ...
        if is_bullish:
            ...
            if price_min_idx != indicator_min_idx:
                ...
                price_prev_min = price_window.iloc[:-5].min()  # TODO: 将魔法数字提取到配置中
                price_decline = (price_prev_min - price_latest_min) / price_prev_min if price_prev_min > 0 else 0

                # 计算指标背离的程度
                indicator_latest_min = indicator_window.iloc[-5:].min()  # TODO: 将魔法数字提取到配置中
                indicator_prev_min = indicator_window.iloc[:-5].min()  # TODO: 将魔法数字提取到配置中
            ...
            if price_max_idx != indicator_max_idx:
                ...
                price_prev_max = price_window.iloc[:-5].max()  # TODO: 将魔法数字提取到配置中
                price_rise = (price_latest_max - price_prev_max) / price_prev_max if price_prev_max > 0 else 0

                # 计算指标背离的程度
                indicator_latest_max = indicator_window.iloc[-5:].max()  # TODO: 将魔法数字提取到配置中
                indicator_prev_max = indicator_window.iloc[:-5].max()  # TODO: 将魔法数字提取到配置中
    ...
    def get_pattern_info_Kdj(self, pattern_id: str) -> dict:
        ...
        return pattern_info_map.get(
            pattern_id,
            {"name": pattern_id, "description": f"KDJ形态: {pattern_id}", "strength": "medium", "type": "neutral"},
        )
    ...
    def _get_default_parameters(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "k_period": 9,  # TODO: 将魔法数字提取到配置中
            "k_slowing": 3,  # TODO: 将魔法数字提取到配置中
            "d_period": 3,  # TODO: 将魔法数字提取到配置中
        }
    ...
    def set_parameters_Indicator_Base_Indicator(self, **kwargs):
        ...
        if any(key in kwargs for key in ["n", "m1", "m2"]):
            ...
            self._m1 = self._parameters.get("m1", 3)  # TODO: 将魔法数字提取到配置中
            self._m2 = self._parameters.get("m2", 3)  # TODO: 将魔法数字提取到配置中
    ...
    def get_patterns(self, data: pd.DataFrame = None, **kwargs) -> pd.DataFrame:
        ...
        # 3. 超买形态 (K>80, D>80)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        patterns_df["KDJ_OVERBOUGHT"] = (k_values > 80) & (
            d_values > 80
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        # 4. 超卖形态 (K<20, D<20)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        patterns_df["KDJ_OVERSOLD"] = (k_values < 20) & (
            d_values < 20
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        # 5. J值极端超买 (J>100)  # TODO: 将魔法数字提取到配置中
        patterns_df["KDJ_J_EXTREME_OVERBOUGHT"] = j_values > 100

        # 6. J值极端超卖 (J<0)  # TODO: 将魔法数字提取到配置中
    ...
    def calculate_raw_score(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        # 1. 基于KDJ位置的评分
        # 超卖区域加分 (K<30, D<30)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        oversold_condition = k_values < 30  # TODO: 将魔法数字提取到配置中 & (d_values < 30)
        score += oversold_condition * 15  # TODO: 将魔法数字提取到配置中

        # 超买区域减分 (K>70, D>70)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        overbought_condition = k_values > 70  # TODO: 将魔法数字提取到配置中 & (d_values > 70)
        ...
        # 3. 基于J值的评分  # TODO: 将魔法数字提取到配置中
        # J值极端超卖时加分 (J<0)
        score += (j_values < 0) * 12  # TODO: 将魔法数字提取到配置中

        # J值极端超买时减分 (J>100)
        score -= (j_values > 100) * 12  # TODO: 将魔法数字提取到配置中

        # 4. 基于趋势的评分  # TODO: 将魔法数字提取到配置中
        # K值上升趋势加分
        k_rising = k_values > k_values.shift(1)
        score += k_rising * 5  # TODO: 将魔法数字提取到配置中

        # D值上升趋势加分
        d_rising = d_values > d_values.shift(1)
        score += d_rising * 3  # TODO: 将魔法数字提取到配置中
    ...
    def calculate_score(self, data: pd.DataFrame, **kwargs) -> dict:
        ...
        # 计算置信度
        kdj_data = self.calculate_Kdj(data)
        k_value = kdj_data["K"].iloc[-1] if not kdj_data.empty else 50.0  # TODO: 将魔法数字提取到配置中
        d_value = kdj_data["D"].iloc[-1] if not kdj_data.empty else 50.0  # TODO: 将魔法数字提取到配置中

        # 基于KDJ位置计算置信度
        if abs(k_value - d_value) > 10:  # K和D差距较大
            confidence = 0.8  # TODO: 将魔法数字提取到配置中
        elif abs(k_value - d_value) > 5:  # TODO: 将魔法数字提取到配置中
            confidence = 0.6  # TODO: 将魔法数字提取到配置中
        else:
            confidence = 0.4  # TODO: 将魔法数字提取到配置中

        # 极端位置提高置信度
        if k_value < 20 or k_value > 80:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
    ...
    def set_parameters(self, **kwargs):
        ...
        if "n" in kwargs:
            n = kwargs["n"]
            if isinstance(n, int) and 1 <= n <= 50:  # TODO: 将魔法数字提取到配置中
                self.n = n
            else:
