...
class PatternStrengthPatternRegistry(Enum):
    ...
    VERY_STRONG = 5  # 非常强  # TODO: 将魔法数字提取到配置中
    STRONG = 4  # 强  # TODO: 将魔法数字提取到配置中
    MEDIUM = 3  # 中等  # TODO: 将魔法数字提取到配置中
    WEAK = 2  # 弱
    VERY_WEAK = 1  # 非常弱


class PatternInfo(BaseIndicator):
    """形态信息类"""
...
class PatternRegistry(BaseIndicator):
    ...
    def _infer_polarity(
        self, pattern_type: PatternTypePatternRegistry, score_impact: float, display_name: str
    ) -> PatternPolarity:
        """
        自动推断形态极性
        ...
        # 基于评分影响推断
        if score_impact > 5:  # TODO: 将魔法数字提取到配置中
            return PatternPolarity.POSITIVE
        elif score_impact < -5:  # TODO: 将魔法数字提取到配置中
            return PatternPolarity.NEGATIVE
    ...
    def register_indicator_pattern(
        self,
        indicator_type: str,
        pattern_id: str,
        display_name: str,
    ...
    def calculate_combined_score_impact(self, patterns: List[str]) -> float:
        ...
        """
        total_impact = 0.0
        weights = {"bullish": 1.0, "bearish": 1.0, "neutral": 0.5}  # TODO: 将魔法数字提取到配置中
        ...
        logger.debug(
            f"计算评分影响 - 多头: {bullish_impact}, 空头: {bearish_impact}, 中性: {neutral_impact}, 总计: {total_impact}"
        )

        # 限制总影响范围
        return np.clip(total_impact, -25.0, 25.0)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
    ...
    def register_patterns_from_config(self, config_file: str) -> None:
        ...
        try:
            with open(config_file, "r", encoding="utf-8") as f:  # TODO: 将魔法数字提取到配置中
                config = json.load(f)
