...
class RsiDerivatives(BaseIndicator, PatternSignalMixin, MinimumPeriodsMixin):
    """
    RSI_DERIVATIVES 指标
    ...
    def set_parameters_Derivatives(self, **kwargs):
        ...
        try:
            ...
            # 验证参数
            is_valid, errors = validator.validate_indicator_parameters('RSI_DERIVATIVES', params)
            if not is_valid:
                # 静默处理验证失败，避免过多警告
                pass
                # 使用默认参数
    ...
    def _calculate_rsiderivatives(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        df[f'RSI_DERIVATIVES_VALUE'] = df['close'].rolling(window=self.period).mean()
