...
Numeric_array = numeric_array  # 中间命名别名以保持兼容性
...
def wma(series: Numeric_array, periods: int) -> np.ndarray:
    ...
    return pd.Series(series).rolling(periods).apply(lambda x: weighted_average(x.values), raw=True).values
...
def macd(
    close: Numeric_array, fast_period: int = 12, slow_period: int = 26, signal_period: int = 9
) -> Tuple[
...
def kdj(
    close: Numeric_array, high: Numeric_array, low: Numeric_array, n: int = 9, m1: int = 3, m2: int = 3
) -> Tuple[
    np.ndarray, np.ndarray, np.ndarray
]:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
    """
    ...
    k = ema(rsv, (m1 * 2 - 1))
    d = ema(k, (m2 * 2 - 1))
    j = k * 3 - d * 2  # TODO: 将魔法数字提取到配置中
    return k, d, j


def boll(
    close: Numeric_array, periods: int = 20, std_dev: float = 2.0
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:  # TODO: 将魔法数字提取到配置中
    """
    计算布林带指标
...
def rsi(close: Numeric_array, periods: int = 14) -> np.ndarray:  # TODO: 将魔法数字提取到配置中
    """
    计算RSI指标
...
def atr(
    close: Numeric_array, high: Numeric_array, low: Numeric_array, periods: int = 14
) -> np.ndarray:  # TODO: 将魔法数字提取到配置中
    """
    计算ATR指标
