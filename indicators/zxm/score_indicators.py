from utils.container import container

"""
ZXM体系评分指标模块

实现ZXM体系的评分指标，包括弹性评分和买点评分
"""

from utils.logger import get_logger

import numpy as np
import pandas as pd
from typing import Dict, List, Union, Optional, Any, Tuple

from indicators.base_indicator import BaseIndicator
from indicators.base.pattern_signal_mixin import PatternSignalMixin
from indicators.base.minimum_periods_mixin import MinimumPeriodsMixin
from indicators.zxm.zxm_abstract_methods_mixin import ZXMAbstractMethodsMixin
from indicators.zxm.elasticity_indicators import AmplitudeElasticity, ZxmriseElasticity
from indicators.zxm.buy_point_indicators import ZXMDailyMACD, ZXMTurnover, ZXMMACallback
from utils.logger import get_logger

logger = get_logger(__name__)


class ZxmelasticityScore(BaseIndicator, PatternSignalMixin, MinimumPeriodsMixin, ZXMAbstractMethodsMixin):
    """
    ZXM弹性评分指标

    综合评估股票的弹性表现，包括振幅弹性和涨幅弹性
    """

    def __init__(self, threshold: float = 75):  # TODO: 将魔法数字提取到配置中
        # 依赖注入示例:
        # self.data_access = container.resolve("DataAccessInterface")
        # self.cache_service = container.resolve("ICacheService")
        self.REQUIRED_COLUMNS = ["open", "high", "low", "close", "volume"]
        """
        初始化ZXM弹性评分指标
        
        Args:
            threshold: 弹性评分阈值，默认为75分
        """
        super().__init__(name="ZXMElasticityScore", description="ZXM弹性评分指标，综合评估股票的弹性表现")
        self.threshold = threshold

        # 初始化弹性子指标
        self.amplitude_elasticity = AmplitudeElasticity()
        self.rise_elasticity = ZxmriseElasticity()

    def _calculate_scoreindicators(self, data: pd.DataFrame, *args, **kwargs) -> pd.DataFrame:
        """
        计算ZXM弹性评分

        Args:
            data: 输入数据，包含OHLCV数据

        Returns:
            pd.DataFrame: 计算结果，包含弹性评分和信号
        """
        # 初始化结果数据框
        result = data.copy()

        # 计算振幅弹性
        try:
            amplitude_result = self.amplitude_elasticity.calculate(data)
            amplitude_signal = amplitude_result["XG"]
        except Exception as e:
            logger.error(f"计算振幅弹性出错: {e}")
            amplitude_signal = pd.Series(False, index=data.index)

        # 计算涨幅弹性
        try:
            rise_result = self.rise_elasticity.calculate(data)
            rise_signal = rise_result["XG"]
        except Exception as e:
            logger.error(f"计算涨幅弹性出错: {e}")
            rise_signal = pd.Series(False, index=data.index)

        # 统计弹性指标
        elasticity_indicators = [amplitude_signal, rise_signal]

        # 计算弹性指标满足数量
        elasticity_count = pd.Series(0, index=data.index)
        for indicator in elasticity_indicators:
            elasticity_count += indicator.astype(int)

        # 计算弹性评分（0-100分）
        elasticity_score = (elasticity_count / len(elasticity_indicators)) * 100

        # 生成满足弹性条件的信号
        signal = elasticity_score >= self.threshold

        # 添加计算结果到数据框
        result.loc[:, "AmplitudeElasticity"] = amplitude_signal
        result.loc[:, "RiseElasticity"] = rise_signal
        result.loc[:, "ElasticityCount"] = elasticity_count
        result.loc[:, "ElasticityScore"] = elasticity_score
        result.loc[:, "Signal"] = signal

        # 添加形态识别和信号生成
        result = self.add_pattern_detection(result)
        result = self.add_signal_generation(result)

        # 重写buy_signal逻辑，基于Signal值而不是通用逻辑
        result.loc[:, "buy_signal"] = result["Signal"] == True
        result.loc[:, "sell_signal"] = result["Signal"] == False
        result.loc[:, "hold_signal"] = result["Signal"] == False

        return result

    def calculate_raw_score_Indicators_scoreindicators(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        """
        计算ZXM弹性评分指标的原始评分

        Args:
            data: 输入数据，包含OHLCV数据
            **kwargs: 其他参数

        Returns:
            pd.Series: 评分结果，0-100分
        """
        # 计算指标
        result = self.calculate(data)

        # 直接使用计算的弹性评分作为原始评分
        return result["ElasticityScore"]

    def identify_patterns_Indicators_scoreindicators(self, data: pd.DataFrame, **kwargs) -> List[str]:
        """
        识别ZXM弹性评分指标相关的技术形态

        Args:
            data: 输入数据
            **kwargs: 其他参数

        Returns:
            List[str]: 识别的形态列表
        """
        # 计算指标
        result = self.calculate(data)

        # 只关注最后一个交易日的形态
        patterns = []
        if len(result) > 0:
            last_row = result.iloc[-1]

            # 基础形态判断
            if last_row["Signal"]:
                patterns.append("弹性评分信号")

            # 弹性评分等级判断
            score = last_row["ElasticityScore"]
            if score >= 90:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
                patterns.append("极高弹性评分(≥90)")  # TODO: 将魔法数字提取到配置中
            elif score >= 75:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
                patterns.append("高弹性评分(75-90)")  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            elif score >= 50:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
                patterns.append("中等弹性评分(50-75)")  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            elif score >= 25:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
                patterns.append("低弹性评分(25-50)")  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            else:
                patterns.append("极低弹性评分(<25)")  # TODO: 将魔法数字提取到配置中

            # 弹性指标满足情况
            count = last_row["ElasticityCount"]
            if count == 2:
                patterns.append("全部弹性指标满足")
            elif count == 1:
                patterns.append("部分弹性指标满足")
            else:
                patterns.append("无弹性指标满足")

            # 具体弹性指标判断
            if last_row["AmplitudeElasticity"]:
                patterns.append("振幅弹性满足")
            if last_row["RiseElasticity"]:
                patterns.append("涨幅弹性满足")

        return patterns

    def calculate_confidence_Indicators_scoreindicators(
        self, score: pd.Series, patterns: List[str], signals: Dict[str, pd.Series]
    ) -> float:
        """
        计算置信度

        Args:
            score: 评分序列
            patterns: 形态列表
            signals: 信号字典

        Returns:
            float: 置信度值，0-1之间
        """
        if score.empty:
            return 0.5  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        latest_score = score.iloc[-1]

        # 基础置信度基于评分
        base_confidence = min(
            0.9, max(0.1, latest_score / 100)
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        # 根据形态调整置信度
        pattern_boost = 0.0
        if "弹性评分信号" in patterns:
            pattern_boost += 0.15  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        if "极高弹性评分(≥90)" in patterns:  # TODO: 将魔法数字提取到配置中
            pattern_boost += 0.2
        elif "高弹性评分(75-90)" in patterns:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            pattern_boost += 0.15  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        if "全部弹性指标满足" in patterns:
            pattern_boost += 0.15  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        elif "部分弹性指标满足" in patterns:
            pattern_boost += 0.1

        # 最终置信度
        final_confidence = min(1.0, base_confidence + pattern_boost)
        return final_confidence

    def get_patterns_Indicators_scoreindicators(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        """
        获取技术形态

        Args:
            data: 输入数据
            **kwargs: 其他参数

        Returns:
            pd.DataFrame: 包含形态信号的Data_frame
        """
        # 计算指标
        result = self.calculate(data)

        # 初始化形态DataFrame
        patterns_df = pd.DataFrame(index=data.index)

        # 基础形态
        patterns_df.loc[:, "弹性评分信号"] = result["Signal"]
        patterns_df.loc[:, "振幅弹性满足"] = result["AmplitudeElasticity"]
        patterns_df.loc[:, "涨幅弹性满足"] = result["RiseElasticity"]

        # 弹性评分等级形态
        score = result["ElasticityScore"]
        patterns_df.loc[:, "极高弹性评分"] = score >= 90  # TODO: 将魔法数字提取到配置中
        patterns_df.loc[:, "高弹性评分"] = (score >= 75) & (
            score < 90
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        patterns_df.loc[:, "中等弹性评分"] = (score >= 50) & (
            score < 75
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        patterns_df.loc[:, "低弹性评分"] = (score >= 25) & (
            score < 50
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        patterns_df.loc[:, "极低弹性评分"] = score < 25  # TODO: 将魔法数字提取到配置中

        # 弹性指标满足情况形态
        count = result["ElasticityCount"]
        patterns_df.loc[:, "全部弹性指标满足"] = count == 2
        patterns_df.loc[:, "部分弹性指标满足"] = count == 1
        patterns_df.loc[:, "无弹性指标满足"] = count == 0

        return patterns_df

    def set_parameters_Indicators_scoreindicators(self, **kwargs):
        """
        设置指标参数

        Args:
            **kwargs: 参数字典，可包含：
                - threshold: 弹性评分阈值，默认75
        """
        self.threshold = kwargs.get("threshold", 75)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中


class ZxmbuyPointScore(BaseIndicator, PatternSignalMixin):
    """
    ZXM买点评分指标

    综合评估股票的买点表现，包括MACD买点、换手买点和回踩均线买点
    """

    def calculate_raw_score_Indicators_scoreindicators_duplicate(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        """
        计算ZXM买点评分指标的原始评分

        Args:
            data: 输入数据，包含OHLCV数据
            **kwargs: 其他参数

        Returns:
            pd.Series: 评分结果，0-100分
        """
        # 计算指标
        result = self.calculate(data)

        # 直接使用计算的买点评分作为原始评分
        return result["BuyPointScore"]

    def identify_patterns_Indicators_scoreindicators_duplicate(self, data: pd.DataFrame, **kwargs) -> List[str]:
        """
        识别ZXM买点评分指标相关的技术形态

        Args:
            data: 输入数据
            **kwargs: 其他参数

        Returns:
            List[str]: 识别的形态列表
        """
        # 计算指标
        result = self.calculate(data)

        # 只关注最后一个交易日的形态
        patterns = []
        if len(result) > 0:
            last_row = result.iloc[-1]

            # 基础形态判断
            if last_row["Signal"]:
                patterns.append("买点评分信号")

            # 买点评分等级判断
            score = last_row["BuyPointScore"]
            if score >= 90:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
                patterns.append("极高买点评分(≥90)")  # TODO: 将魔法数字提取到配置中
            elif score >= 75:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
                patterns.append("高买点评分(75-90)")  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            elif score >= 50:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
                patterns.append("中等买点评分(50-75)")  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            elif score >= 25:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
                patterns.append("低买点评分(25-50)")  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            else:
                patterns.append("极低买点评分(<25)")  # TODO: 将魔法数字提取到配置中

            # 买点指标满足情况
            count = last_row["BuyPointCount"]
            if count == 3:  # TODO: 将魔法数字提取到配置中
                patterns.append("三重买点信号共振")
            elif count == 2:
                patterns.append("多数买点指标满足")
            elif count == 1:
                patterns.append("少数买点指标满足")
            else:
                patterns.append("无买点指标满足")

            # 具体买点指标判断
            if last_row["MACDBuyPoint"]:
                patterns.append("MACD买点满足")
            if last_row["TurnoverBuyPoint"]:
                patterns.append("换手买点满足")
            if last_row["MACallbackBuyPoint"]:
                patterns.append("均线回调买点满足")

        return patterns

    def calculate_confidence_Indicators_scoreindicators_duplicate(
        self, score: pd.Series, patterns: List[str], signals: Dict[str, pd.Series]
    ) -> float:
        """
        计算置信度

        Args:
            score: 评分序列
            patterns: 形态列表
            signals: 信号字典

        Returns:
            float: 置信度值，0-1之间
        """
        if score.empty:
            return 0.5  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        latest_score = score.iloc[-1]

        # 基础置信度基于评分
        base_confidence = min(
            0.9, max(0.1, latest_score / 100)
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        # 根据形态调整置信度
        pattern_boost = 0.0
        if "买点评分信号" in patterns:
            pattern_boost += 0.15  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        if "极高买点评分(≥90)" in patterns:  # TODO: 将魔法数字提取到配置中
            pattern_boost += 0.2
        elif "高买点评分(75-90)" in patterns:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            pattern_boost += 0.15  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        if "三重买点信号共振" in patterns:
            pattern_boost += 0.2
        elif "多数买点指标满足" in patterns:
            pattern_boost += 0.15  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        elif "少数买点指标满足" in patterns:
            pattern_boost += 0.1

        # 最终置信度
        final_confidence = min(1.0, base_confidence + pattern_boost)
        return final_confidence

    def get_patterns_Indicators_scoreindicators_duplicate(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        """
        获取技术形态

        Args:
            data: 输入数据
            **kwargs: 其他参数

        Returns:
            pd.DataFrame: 包含形态信号的Data_frame
        """
        # 计算指标
        result = self.calculate(data)

        # 初始化形态DataFrame
        patterns_df = pd.DataFrame(index=data.index)

        # 基础形态
        patterns_df.loc[:, "买点评分信号"] = result["Signal"]
        patterns_df.loc[:, "MACD买点满足"] = result["MACDBuyPoint"]
        patterns_df.loc[:, "换手买点满足"] = result["TurnoverBuyPoint"]
        patterns_df.loc[:, "均线回调买点满足"] = result["MACallbackBuyPoint"]

        # 买点评分等级形态
        score = result["BuyPointScore"]
        patterns_df.loc[:, "极高买点评分"] = score >= 90  # TODO: 将魔法数字提取到配置中
        patterns_df.loc[:, "高买点评分"] = (score >= 75) & (
            score < 90
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        patterns_df.loc[:, "中等买点评分"] = (score >= 50) & (
            score < 75
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        patterns_df.loc[:, "低买点评分"] = (score >= 25) & (
            score < 50
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        patterns_df.loc[:, "极低买点评分"] = score < 25  # TODO: 将魔法数字提取到配置中

        # 买点指标满足情况形态
        count = result["BuyPointCount"]
        patterns_df.loc[:, "三重买点信号共振"] = count == 3  # TODO: 将魔法数字提取到配置中
        patterns_df.loc[:, "多数买点指标满足"] = count == 2
        patterns_df.loc[:, "少数买点指标满足"] = count == 1
        patterns_df.loc[:, "无买点指标满足"] = count == 0

        return patterns_df

    def set_parameters_Indicators_scoreindicators_duplicate(self, **kwargs):
        """
        设置指标参数

        Args:
            **kwargs: 参数字典，可包含：
                - threshold: 买点评分阈值，默认75
        """
        self.threshold = kwargs.get("threshold", 75)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中


class StockScoreCalculator(BaseIndicator, PatternSignalMixin):
    """
    ZXM股票综合评分指标

    计算股票的综合评分
    """

    def _calculate_trend_score(self, data: pd.DataFrame, result: pd.DataFrame) -> pd.DataFrame:
        """
        计算趋势评分

        Args:
            data: 输入数据
            result: 结果数据框

        Returns:
            pd.DataFrame: 更新后的结果数据框
        """
        # 计算均线系统
        ma5 = data["close"].rolling(window=5).mean()  # TODO: 将魔法数字提取到配置中
        ma10 = data["close"].rolling(window=10).mean()
        ma20 = data["close"].rolling(window=20).mean()  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        ma60 = data["close"].rolling(window=60).mean()  # TODO: 将魔法数字提取到配置中

        # 初始化趋势评分为50分（中性）
        trend_score = pd.Series(50.0, index=data.index)  # TODO: 将魔法数字提取到配置中

        # 1. 均线多头排列得分
        bullish_ma = (ma5 > ma10) & (ma10 > ma20) & (ma20 > ma60)
        trend_score[bullish_ma] += 20  # TODO: 将魔法数字提取到配置中

        # 2. 均线空头排列扣分
        bearish_ma = (ma5 < ma10) & (ma10 < ma20) & (ma20 < ma60)
        trend_score[bearish_ma] -= 20  # TODO: 将魔法数字提取到配置中

        # 3. 价格站上均线得分  # TODO: 将魔法数字提取到配置中
        price_above_ma20 = data["close"] > ma20
        trend_score[price_above_ma20] += 10

        price_above_ma60 = data["close"] > ma60
        trend_score[price_above_ma60] += 10

        # 4. 价格跌破均线扣分  # TODO: 将魔法数字提取到配置中
        price_below_ma20 = data["close"] < ma20
        trend_score[price_below_ma20] -= 10

        price_below_ma60 = data["close"] < ma60
        trend_score[price_below_ma60] -= 10

        # 5. 均线斜率得分  # TODO: 将魔法数字提取到配置中
        ma20_slope = ma20.diff(5) / ma20.shift(5)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        ma60_slope = ma60.diff(5) / ma60.shift(5)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        trend_score[ma20_slope > 0.01] += 5  # 中期均线向上  # TODO: 将魔法数字提取到配置中
        trend_score[ma20_slope < -0.01] -= 5  # 中期均线向下  # TODO: 将魔法数字提取到配置中

        trend_score[
            ma60_slope > 0.005
        ] += 5  # 长期均线向上  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        trend_score[
            ma60_slope < -0.005
        ] -= 5  # 长期均线向下  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        # 确保评分在0-100范围内
        trend_score = trend_score.clip(0, 100)

        # 添加到结果
        result.loc[:, "TrendScore"] = trend_score

        return result

    def _calculate_momentum_score(self, data: pd.DataFrame, result: pd.DataFrame) -> pd.DataFrame:
        """
        计算动量评分

        Args:
            data: 输入数据
            result: 结果数据框

        Returns:
            pd.DataFrame: 更新后的结果数据框
        """
        # 初始化动量评分为50分（中性）
        momentum_score = pd.Series(50.0, index=data.index)  # TODO: 将魔法数字提取到配置中

        # 1. 计算各周期涨跌幅
        returns_5d = data["close"].pct_change(5) * 100  # TODO: 将魔法数字提取到配置中
        returns_10d = data["close"].pct_change(10) * 100
        returns_20d = data["close"].pct_change(20) * 100  # TODO: 将魔法数字提取到配置中

        # 2. 近期涨幅评分
        momentum_score[returns_5d > 5] += 10  # 5日涨幅大于5%  # TODO: 将魔法数字提取到配置中
        momentum_score[returns_5d > 10] += 10  # 5日涨幅大于10%
        momentum_score[returns_5d < -5] -= 10  # 5日跌幅大于5%  # TODO: 将魔法数字提取到配置中
        momentum_score[returns_5d < -10] -= 10  # 5日跌幅大于10%

        # 3. 中期涨幅评分  # TODO: 将魔法数字提取到配置中
        momentum_score[returns_10d > 10] += 10  # 10日涨幅大于10%
        momentum_score[returns_10d < -10] -= 10  # 10日跌幅大于10%

        # 4. 长期涨幅评分  # TODO: 将魔法数字提取到配置中
        momentum_score[returns_20d > 15] += 10  # 20日涨幅大于15%  # TODO: 将魔法数字提取到配置中
        momentum_score[returns_20d < -15] -= 10  # 20日跌幅大于15%  # TODO: 将魔法数字提取到配置中

        # 5. 动量加速/减速评分  # TODO: 将魔法数字提取到配置中
        acceleration = returns_5d - returns_5d.shift(5)  # TODO: 将魔法数字提取到配置中

        momentum_score[acceleration > 3] += 10  # 动量加速（5日涨幅比前5日提高3%以上）  # TODO: 将魔法数字提取到配置中
        momentum_score[acceleration < -3] -= 10  # 动量减速（5日涨幅比前5日减少3%以上）  # TODO: 将魔法数字提取到配置中

        # 确保评分在0-100范围内
        momentum_score = momentum_score.clip(0, 100)

        # 添加到结果
        result.loc[:, "MomentumScore"] = momentum_score

        return result

    def _calculate_volatility_score(self, data: pd.DataFrame, result: pd.DataFrame) -> pd.DataFrame:
        """
        计算波动率评分

        Args:
            data: 输入数据
            result: 结果数据框

        Returns:
            pd.DataFrame: 更新后的结果数据框
        """
        # 初始化波动率评分为50分（中性）
        volatility_score = pd.Series(50.0, index=data.index)  # TODO: 将魔法数字提取到配置中

        # 1. 计算真实波幅（TR）
        high_low = data["high"] - data["low"]
        high_close = (data["high"] - data["close"].shift(1)).abs()
        low_close = (data["low"] - data["close"].shift(1)).abs()

        tr = pd.concat([high_low, high_close, low_close], axis=1).max(axis=1)

        # 2. 计算平均真实波幅（ATR）
        atr_14 = tr.rolling(window=14).mean()  # TODO: 将魔法数字提取到配置中

        # 3. 计算波动率（ATR/收盘价）  # TODO: 将魔法数字提取到配置中
        volatility = atr_14 / data["close"] * 100

        # 4. 波动率评分  # TODO: 将魔法数字提取到配置中
        # 高波动率（>3%）扣分，低波动率（<1%）加分  # TODO: 将魔法数字提取到配置中
        volatility_score[volatility > 3] -= 15  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        volatility_score[
            volatility > 5
        ] -= 15  # 极端波动率（>5%）进一步扣分  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        volatility_score[volatility < 1] += 15  # TODO: 将魔法数字提取到配置中

        # 5. 波动率变化评分  # TODO: 将魔法数字提取到配置中
        volatility_change = volatility - volatility.shift(5)  # TODO: 将魔法数字提取到配置中

        volatility_score[volatility_change > 1] -= 10  # 波动率上升扣分
        volatility_score[volatility_change < -1] += 10  # 波动率下降加分

        # 6. 价格稳定性评分  # TODO: 将魔法数字提取到配置中
        close_std = (
            data["close"].rolling(window=20).std() / data["close"].rolling(window=20).mean() * 100
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        volatility_score[close_std < 2] += 10  # 价格稳定加分
        volatility_score[close_std > 5] -= 10  # 价格不稳定扣分  # TODO: 将魔法数字提取到配置中

        # 确保评分在0-100范围内
        volatility_score = volatility_score.clip(0, 100)

        # 添加到结果
        result.loc[:, "VolatilityScore"] = volatility_score

        return result

    def _calculate_volume_score(self, data: pd.DataFrame, result: pd.DataFrame) -> pd.DataFrame:
        """
        计算成交量评分

        Args:
            data: 输入数据
            result: 结果数据框

        Returns:
            pd.DataFrame: 更新后的结果数据框
        """
        # 初始化成交量评分为50分（中性）
        volume_score = pd.Series(50.0, index=data.index)  # TODO: 将魔法数字提取到配置中

        # 1. 计算成交量变化率
        volume_change = data["volume"].pct_change() * 100

        # 2. 计算相对成交量（与20日均量比较）
        volume_ratio = data["volume"] / data["volume"].rolling(window=20).mean()  # TODO: 将魔法数字提取到配置中

        # 3. 成交量放大评分  # TODO: 将魔法数字提取到配置中
        volume_score[volume_ratio > 2] += 15  # 成交量是20日均量的2倍以上  # TODO: 将魔法数字提取到配置中
        volume_score[volume_ratio > 3] += 10  # 成交量是20日均量的3倍以上  # TODO: 将魔法数字提取到配置中

        # 4. 成交量萎缩评分  # TODO: 将魔法数字提取到配置中
        volume_score[
            volume_ratio < 0.5
        ] -= 15  # 成交量不足20日均量的50%  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        # 5. 量价配合评分  # TODO: 将魔法数字提取到配置中
        price_up = data["close"] > data["open"]
        price_down = data["close"] < data["open"]

        # 放量上涨加分
        volume_score[
            (price_up) & (volume_ratio > 1.5)
        ] += 15  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        # 放量下跌扣分
        volume_score[
            (price_down) & (volume_ratio > 1.5)
        ] -= 15  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        # 缩量上涨加分（潜在突破前蓄势）
        volume_score[(price_up) & (volume_ratio < 0.8)] += 10  # TODO: 将魔法数字提取到配置中

        # 缩量下跌加分（可能企稳）
        volume_score[
            (price_down) & (volume_ratio < 0.8)
        ] += 5  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        # 6. 连续成交量变化评分  # TODO: 将魔法数字提取到配置中
        volume_increase_days = pd.Series(0, index=data.index)
        volume_decrease_days = pd.Series(0, index=data.index)

        for i in range(5, len(data)):  # TODO: 将魔法数字提取到配置中
            if all(
                data["volume"].iloc[i - j] > data["volume"].iloc[i - j - 1] for j in range(3)
            ):  # TODO: 将魔法数字提取到配置中
                volume_increase_days.iloc[i] = 3  # TODO: 将魔法数字提取到配置中
            if all(
                data["volume"].iloc[i - j] < data["volume"].iloc[i - j - 1] for j in range(3)
            ):  # TODO: 将魔法数字提取到配置中
                volume_decrease_days.iloc[i] = 3  # TODO: 将魔法数字提取到配置中

        # 连续放量加分（可能突破）
        volume_score[volume_increase_days == 3] += 10  # TODO: 将魔法数字提取到配置中

        # 连续缩量扣分（可能无人问津）
        volume_score[volume_decrease_days == 3] -= 10  # TODO: 将魔法数字提取到配置中

        # 确保评分在0-100范围内
        volume_score = volume_score.clip(0, 100)

        # 添加到结果
        result.loc[:, "VolumeScore"] = volume_score

        return result

    def _calculate_value_score(self, data: pd.DataFrame, result: pd.DataFrame) -> pd.DataFrame:
        """
        计算价值评分（基于基本面指标）

        Args:
            data: 输入数据
            result: 结果数据框

        Returns:
            pd.DataFrame: 更新后的结果数据框
        """
        # 初始化价值评分为50分（中性）
        value_score = pd.Series(50.0, index=data.index)  # TODO: 将魔法数字提取到配置中

        # 1. PE评分
        pe_ratio = data["pe_ratio"]

        value_score[pe_ratio < 15] += 10  # PE低于15  # TODO: 将魔法数字提取到配置中
        value_score[pe_ratio < 10] += 10  # PE低于10
        value_score[pe_ratio > 30] -= 10  # PE高于30  # TODO: 将魔法数字提取到配置中
        value_score[pe_ratio > 50] -= 10  # PE高于50  # TODO: 将魔法数字提取到配置中

        # 2. PB评分
        pb_ratio = data["pb_ratio"]

        value_score[pb_ratio < 1.5] += 10  # PB低于1.5  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        value_score[pb_ratio < 1] += 10  # PB低于1
        value_score[pb_ratio > 3] -= 10  # PB高于3  # TODO: 将魔法数字提取到配置中
        value_score[pb_ratio > 5] -= 10  # PB高于5  # TODO: 将魔法数字提取到配置中

        # 3. 如果有其他基本面指标，可以继续添加  # TODO: 将魔法数字提取到配置中
        if "roe" in data.columns:
            roe = data["roe"]
            value_score[roe > 15] += 10  # ROE高于15%  # TODO: 将魔法数字提取到配置中
            value_score[roe > 20] += 10  # ROE高于20%  # TODO: 将魔法数字提取到配置中

        if "dividend_yield" in data.columns:
            dividend_yield = data["dividend_yield"]
            value_score[dividend_yield > 2] += 10  # 股息率高于2%
            value_score[dividend_yield > 4] += 10  # 股息率高于4%  # TODO: 将魔法数字提取到配置中

        # 确保评分在0-100范围内
        value_score = value_score.clip(0, 100)

        # 添加到结果
        result.loc[:, "ValueScore"] = value_score

        return result

    def generate_signals_Indicators_scoreindicators(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        """
        生成标准化的信号输出

        Args:
            data: 输入数据
            **kwargs: 其他参数

        Returns:
            pd.DataFrame: 包含标准化信号的Data_frame
        """
        # 计算指标和评分
        result = self.calculate(data)
        score = result["FinalScore"]  # 使用FinalScore

        # 初始化信号DataFrame
        signals = pd.DataFrame(index=data.index)

        # 设置买卖信号
        signals.loc[:, "buy_signal"] = result["BuySignal"]
        signals.loc[:, "sell_signal"] = result["SellSignal"]
        signals.loc[:, "neutral_signal"] = ~(result["BuySignal"] | result["SellSignal"])

        # 设置趋势
        signals.loc[:, "trend"] = 0  # 默认中性
        signals.loc[result["FinalScore"] >= 60, "trend"] = 1  # 高分看涨  # TODO: 将魔法数字提取到配置中
        signals.loc[result["FinalScore"] <= 40, "trend"] = -1  # 低分看跌  # TODO: 将魔法数字提取到配置中

        # 设置评分
        signals.loc[:, "score"] = score

        # 设置信号类型
        signals.loc[:, "signal_type"] = "neutral"
        signals.loc[result["BuySignal"], "signal_type"] = "zxm_high_score_buy"
        signals.loc[result["SellSignal"], "signal_type"] = "zxm_low_score_sell"

        # 设置信号描述
        signals.loc[:, "signal_desc"] = ""

        # 为每个信号设置详细描述
        for i in signals.index:
            desc_parts = []

            # 添加总分
            desc_parts.append(f"总分{result.loc[i, 'FinalScore']:.1f}")

            # 添加评分等级
            if "ScoreGrade" in result.columns:
                desc_parts.append(f"等级{result.loc[i, 'ScoreGrade']}")

            # 添加优势项
            strengths = []
            if result.loc[i, "TrendScore"] >= 70:  # TODO: 将魔法数字提取到配置中
                strengths.append("趋势")
            if result.loc[i, "MomentumScore"] >= 70:  # TODO: 将魔法数字提取到配置中
                strengths.append("动量")
            if result.loc[i, "VolatilityScore"] >= 70:  # TODO: 将魔法数字提取到配置中
                strengths.append("稳定性")
            if result.loc[i, "VolumeScore"] >= 70:  # TODO: 将魔法数字提取到配置中
                strengths.append("成交量")
            if "ValueScore" in result.columns and result.loc[i, "ValueScore"] >= 70:  # TODO: 将魔法数字提取到配置中
                strengths.append("价值")

            if strengths:
                desc_parts.append("优势:" + "/".join(strengths))

            # 添加劣势项
            weaknesses = []
            if result.loc[i, "TrendScore"] <= 30:  # TODO: 将魔法数字提取到配置中
                weaknesses.append("趋势")
            if result.loc[i, "MomentumScore"] <= 30:  # TODO: 将魔法数字提取到配置中
                weaknesses.append("动量")
            if result.loc[i, "VolatilityScore"] <= 30:  # TODO: 将魔法数字提取到配置中
                weaknesses.append("稳定性")
            if result.loc[i, "VolumeScore"] <= 30:  # TODO: 将魔法数字提取到配置中
                weaknesses.append("成交量")
            if "ValueScore" in result.columns and result.loc[i, "ValueScore"] <= 30:  # TODO: 将魔法数字提取到配置中
                weaknesses.append("价值")

            if weaknesses:
                desc_parts.append("劣势:" + "/".join(weaknesses))

            signals.loc[i, "signal_desc"] = "，".join(desc_parts)

        # 置信度设置
        signals.loc[:, "confidence"] = 60  # 基础置信度  # TODO: 将魔法数字提取到配置中

        # 根据分数和项目一致性调整置信度
        for i in signals.index:
            # 高分或低分的置信度更高
            if result.loc[i, "FinalScore"] >= 80:  # TODO: 将魔法数字提取到配置中
                signals.loc[i, "confidence"] = 80  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            elif result.loc[i, "FinalScore"] <= 20:  # TODO: 将魔法数字提取到配置中
                signals.loc[i, "confidence"] = 80  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

            # 分项一致性高的置信度更高
            if signals.loc[i, "buy_signal"]:
                consistency_score = sum(
                    [
                        1 if result.loc[i, "TrendScore"] >= 60 else 0,  # TODO: 将魔法数字提取到配置中
                        1 if result.loc[i, "MomentumScore"] >= 60 else 0,  # TODO: 将魔法数字提取到配置中
                        1 if result.loc[i, "VolatilityScore"] >= 60 else 0,  # TODO: 将魔法数字提取到配置中
                        1 if result.loc[i, "VolumeScore"] >= 60 else 0,  # TODO: 将魔法数字提取到配置中
                    ]
                )
                if consistency_score >= 3:  # 至少3个分项都支持买入  # TODO: 将魔法数字提取到配置中
                    signals.loc[i, "confidence"] = min(
                        90, signals.loc[i, "confidence"] + 20
                    )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

            elif signals.loc[i, "sell_signal"]:
                consistency_score = sum(
                    [
                        1 if result.loc[i, "TrendScore"] <= 40 else 0,  # TODO: 将魔法数字提取到配置中
                        1 if result.loc[i, "MomentumScore"] <= 40 else 0,  # TODO: 将魔法数字提取到配置中
                        1 if result.loc[i, "VolatilityScore"] <= 40 else 0,  # TODO: 将魔法数字提取到配置中
                        1 if result.loc[i, "VolumeScore"] <= 40 else 0,  # TODO: 将魔法数字提取到配置中
                    ]
                )
                if consistency_score >= 3:  # 至少3个分项都支持卖出  # TODO: 将魔法数字提取到配置中
                    signals.loc[i, "confidence"] = min(
                        90, signals.loc[i, "confidence"] + 20
                    )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        # 风险等级
        signals.loc[:, "risk_level"] = "中"  # 默认中等风险
        signals.loc[result["VolatilityScore"] <= 30, "risk_level"] = (
            "高"  # 高波动性，高风险  # TODO: 将魔法数字提取到配置中
        )
        signals.loc[result["VolatilityScore"] >= 70, "risk_level"] = (
            "低"  # 低波动性，低风险  # TODO: 将魔法数字提取到配置中
        )

        # 建议仓位
        signals.loc[:, "position_size"] = 0.0
        signals.loc[signals["buy_signal"], "position_size"] = 0.3  # 基础仓位  # TODO: 将魔法数字提取到配置中

        # 根据总分和置信度调整仓位
        high_score_high_conf = (result["FinalScore"] >= 80) & (
            signals["confidence"] >= 80
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        signals.loc[high_score_high_conf, "position_size"] = (
            0.5  # 高分高置信度，加大仓位  # TODO: 将魔法数字提取到配置中
        )

        exceptional_score = result["FinalScore"] >= 90  # TODO: 将魔法数字提取到配置中
        signals.loc[exceptional_score, "position_size"] = 0.7  # 极高分，最大仓位  # TODO: 将魔法数字提取到配置中

        # 止损位 - 使用近期低点或移动平均线
        signals.loc[:, "stop_loss"] = 0.0
        ma20 = data["close"].rolling(window=20).mean()  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        for i in signals.index[signals["buy_signal"]]:
            try:
                idx = data.index.get_loc(i)
                if idx >= 20:  # TODO: 将魔法数字提取到配置中
                    # 使用20日均线作为止损位基准
                    ma20_val = ma20.iloc[idx]
                    # 如果价格远高于均线，使用均线作为止损；否则使用近期低点
                    if data["close"].iloc[idx] > ma20_val * 1.1:
                        signals.loc[i, "stop_loss"] = ma20_val * 0.95  # 均线下方5%  # TODO: 将魔法数字提取到配置中
                    else:
                        # 使用近期低点
                        low_price = data.iloc[idx - 20 : idx + 1]["low"].min()  # TODO: 将魔法数字提取到配置中
                        signals.loc[i, "stop_loss"] = low_price * 0.97  # 最低点下方3%  # TODO: 将魔法数字提取到配置中
            except:
                continue

        # 市场环境和成交量确认
        signals.loc[:, "market_env"] = "normal"

        # 判断市场环境
        # 使用市场趋势得分作为参考
        market_trend = pd.Series("normal", index=data.index)
        market_trend[result["TrendScore"] >= 75] = "bull_market"  # TODO: 将魔法数字提取到配置中
        market_trend[result["TrendScore"] <= 25] = "bear_market"  # TODO: 将魔法数字提取到配置中
        signals.loc[:, "market_env"] = market_trend

        # 成交量确认
        signals.loc[:, "volume_confirmation"] = result["VolumeScore"] >= 60  # TODO: 将魔法数字提取到配置中

        return signals

    def get_pattern_info_Indicators_Score_Indicators(self, pattern_id: str) -> dict:
        """
        获取指定形态的详细信息

        Args:
            pattern_id: 形态ID

        Returns:
            dict: 形态详细信息
        """
        # 默认形态信息
        default_pattern = {
            "id": pattern_id,
            "name": pattern_id,
            "description": f"{pattern_id}形态",
            "type": "NEUTRAL",
            "strength": "MEDIUM",
            "score_impact": 0.0,
        }

        # ZXMScoreIndicator指标特定的形态信息映射
        pattern_info_map = {
            # 基础形态
            "超买区域": {
                "id": "超买区域",
                "name": "超买区域",
                "description": "指标进入超买区域，可能面临回调压力",
                "type": "BEARISH",
                "strength": "MEDIUM",
                "score_impact": -10.0,
            },
            "超卖区域": {
                "id": "超卖区域",
                "name": "超卖区域",
                "description": "指标进入超卖区域，可能出现反弹机会",
                "type": "BULLISH",
                "strength": "MEDIUM",
                "score_impact": 10.0,
            },
            "中性区域": {
                "id": "中性区域",
                "name": "中性区域",
                "description": "指标处于中性区域，趋势不明确",
                "type": "NEUTRAL",
                "strength": "WEAK",
                "score_impact": 0.0,
            },
            # 趋势形态
            "上升趋势": {
                "id": "上升趋势",
                "name": "上升趋势",
                "description": "指标显示上升趋势，看涨信号",
                "type": "BULLISH",
                "strength": "STRONG",
                "score_impact": 15.0,  # TODO: 将魔法数字提取到配置中
            },
            "下降趋势": {
                "id": "下降趋势",
                "name": "下降趋势",
                "description": "指标显示下降趋势，看跌信号",
                "type": "BEARISH",
                "strength": "STRONG",
                "score_impact": -15.0,  # TODO: 将魔法数字提取到配置中
            },
            # 信号形态
            "买入信号": {
                "id": "买入信号",
                "name": "买入信号",
                "description": "指标产生买入信号，建议关注",
                "type": "BULLISH",
                "strength": "STRONG",
                "score_impact": 20.0,  # TODO: 将魔法数字提取到配置中
            },
            "卖出信号": {
                "id": "卖出信号",
                "name": "卖出信号",
                "description": "指标产生卖出信号，建议谨慎",
                "type": "BEARISH",
                "strength": "STRONG",
                "score_impact": -20.0,  # TODO: 将魔法数字提取到配置中
            },
        }

        return pattern_info_map.get(pattern_id, default_pattern)

    @property
    def minimum_periods(self) -> int:
        """
        ZxmelasticityScore指标所需的最少数据周期数

        计算逻辑：使用默认值

        Returns:
            int: 最少需要的数据周期数
        """
        return 30  # TODO: 将魔法数字提取到配置中

    def __init__(self):
        # 依赖注入示例:
        # self.data_access = container.resolve("DataAccessInterface")
        # self.cache_service = container.resolve("ICacheService")
        """初始化ZXM股票综合评分指标"""
        # 移除super().__init__调用，直接设置属性
        self.name = "ZXMTrendScore"
        self.description = "ZXM股票综合评分指标，计算股票的综合评分"

    @property
    def minimum_periods(self) -> int:
        """
        ZXM股票综合评分指标所需的最少数据周期数

        Returns:
            int: 最少需要的数据周期数
        """
        return 60  # 需要足够数据计算各种评分  # TODO: 将魔法数字提取到配置中

    def calculate(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        """
        计算ZXM股票综合评分指标的主要入口方法

        Args:
            data: 包含OHLCV数据的DataFrame
            **kwargs: 其他参数

        Returns:
            包含ZXM股票综合评分指标的DataFrame
        """
        result = data.copy()

        # 计算各项评分
        result = self._calculate_trend_score(data, result)
        result = self._calculate_momentum_score(data, result)
        result = self._calculate_volatility_score(data, result)
        result = self._calculate_volume_score(data, result)

        # 如果有基本面数据，计算价值评分
        if any(col in data.columns for col in ["pe_ratio", "pb_ratio", "roe", "dividend_yield"]):
            result = self._calculate_value_score(data, result)

        # 计算综合评分
        score_columns = ["TrendScore", "MomentumScore", "VolatilityScore", "VolumeScore"]
        if "ValueScore" in result.columns:
            score_columns.append("ValueScore")

        # 加权平均计算综合评分
        weights = [
            0.3,
            0.25,
            0.2,
            0.25,
        ]  # 趋势30%，动量25%，波动20%，成交量25%  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        if "ValueScore" in result.columns:
            weights = [
                0.25,
                0.2,
                0.15,
                0.2,
                0.2,
            ]  # 加入价值评分20%  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        final_score = pd.Series(0.0, index=data.index)
        for i, col in enumerate(score_columns):
            final_score += result[col] * weights[i]

        result["FinalScore"] = final_score

        # 生成买卖信号
        result["BuySignal"] = final_score >= 75  # TODO: 将魔法数字提取到配置中
        result["SellSignal"] = final_score <= 25  # TODO: 将魔法数字提取到配置中

        # 评分等级
        result["ScoreGrade"] = "C"
        result.loc[final_score >= 80, "ScoreGrade"] = "A"  # TODO: 将魔法数字提取到配置中
        result.loc[(final_score >= 60) & (final_score < 80), "ScoreGrade"] = (
            "B"  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        )
        result.loc[(final_score >= 40) & (final_score < 60), "ScoreGrade"] = (
            "C"  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        )
        result.loc[final_score < 40, "ScoreGrade"] = "D"  # TODO: 将魔法数字提取到配置中

        return result

    def _calculate_baseindicator(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        """
        BaseIndicator要求的抽象方法实现

        Args:
            data: 包含OHLCV数据的DataFrame
            **kwargs: 其他参数

        Returns:
            包含ZXM股票综合评分指标的DataFrame
        """
        return self.calculate(data, **kwargs)

    def calculate_confidence_Indicator_Base_Indicator(
        self, score: pd.Series, patterns: pd.DataFrame, signals: dict
    ) -> float:
        """
        BaseIndicator要求的置信度计算方法

        Args:
            score: 得分序列
            patterns: 检测到的形态DataFrame
            signals: 生成的信号字典

        Returns:
            float: 置信度分数 (0-1)
        """
        return 0.8  # ZXM综合评分置信度较高  # TODO: 将魔法数字提取到配置中

    def calculate_raw_score_Indicator_Base_Indicator(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        """
        BaseIndicator要求的原始评分计算方法

        Args:
            data: 输入数据
            **kwargs: 其他参数

        Returns:
            pd.Series: 原始评分序列
        """
        result = self.calculate(data, **kwargs)
        return result["FinalScore"]

    def get_patterns_Indicator_Base_Indicator(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        """
        BaseIndicator要求的形态获取方法

        Args:
            data: 输入数据
            **kwargs: 其他参数

        Returns:
            pd.DataFrame: 形态DataFrame
        """
        result = self.calculate(data, **kwargs)
        patterns = pd.DataFrame(index=data.index)

        # ZXM评分形态
        final_score = result["FinalScore"]
        patterns["ZXM_HIGH_SCORE"] = final_score >= 80  # TODO: 将魔法数字提取到配置中
        patterns["ZXM_MEDIUM_HIGH_SCORE"] = (final_score >= 60) & (
            final_score < 80
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        patterns["ZXM_MEDIUM_SCORE"] = (final_score >= 40) & (
            final_score < 60
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        patterns["ZXM_LOW_SCORE"] = final_score < 40  # TODO: 将魔法数字提取到配置中

        # 买卖信号形态
        patterns["ZXM_BUY_SIGNAL"] = result["BuySignal"]
        patterns["ZXM_SELL_SIGNAL"] = result["SellSignal"]

        return patterns

    def set_parameters_Indicator_Base_Indicator(self, **kwargs):
        """
        BaseIndicator要求的参数设置方法

        Args:
            **kwargs: 参数字典
        """
        # ZXM综合评分使用固定权重，无需设置参数
        pass

    def _get_default_parameters_stockscorecalculator(self) -> dict:
        """
        获取ZXM股票综合评分指标的默认参数

        Returns:
            dict: 默认参数字典
        """
        return {
            "trend_weight": 0.3,  # TODO: 将魔法数字提取到配置中
            "momentum_weight": 0.25,  # TODO: 将魔法数字提取到配置中
            "volatility_weight": 0.2,
            "volume_weight": 0.25,  # TODO: 将魔法数字提取到配置中
            "value_weight": 0.2,
        }
