...
class ZXMStrategyCombination(BaseIndicator, PatternSignalMixin, MinimumPeriodsMixin):
    ...
    def set_parameters_Strategy_Combination(self, **kwargs):
        ...
        Args:
            **kwargs: 参数字典
        """
        self.strategy_count = kwargs.get('strategy_count', 5)
        self.lookback_period = kwargs.get('lookback_period', 30)
        self.rebalance_period = kwargs.get('rebalance_period', 10)
        self.correlation_threshold = kwargs.get('correlation_threshold', 0.7)
        self.performance_weight = kwargs.get('performance_weight', 0.4)
        self.risk_weight = kwargs.get('risk_weight', 0.3)
        self.correlation_weight = kwargs.get('correlation_weight', 0.3)
    ...
    def _calculate_base_strategies(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        mean_reversion_signal = (z_score < -1.5).astype(float)
        
        # 策略3: 动量策略
        momentum = close.pct_change(10)
        momentum_signal = (momentum > momentum.rolling(window=20).quantile(0.8)).astype(float)
        
        # 策略4: 波动率突破策略
        atr = ((high - low) + (high - close.shift(1)).abs() + (low - close.shift(1)).abs()) / 3
        atr_ma = atr.rolling(window=14).mean()
        volatility_signal = (atr > atr_ma * 1.5).astype(float)
    ...
    def _calculate_strategy_correlation(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        strategy_columns = ['Strategy1_Trend', 'Strategy2_MeanReversion', 'Strategy3_Momentum', 
                           'Strategy4_Volatility', 'Strategy5_Volume']
        ...
        result['CorrelationScore'] = correlation_score.fillna(50)
    ...
    def _calculate_combination_performance(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 滚动性能指标
        rolling_return = combination_return.rolling(window=self.lookback_period).mean() * 252
        rolling_volatility = combination_return.rolling(window=self.lookback_period).std() * np.sqrt(252)
        ...
        # 组合评分
        combination_score = (
            np.clip(rolling_sharpe * 25, 0, 50) +  # 夏普比率评分
            np.clip((-max_drawdown) * 100, 0, 30) +  # 回撤控制评分
            np.clip(result['CorrelationScore'] * 0.2, 0, 20)  # 分散化评分
        )
        ...
        result['CombinationScore'] = combination_score.fillna(50)
    ...
    def _generate_combination_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 策略组合信号
        result['StrategyCombinationSignal'] = weighted_signal > 0.6
        
        # 策略优化信号
        result['StrategyOptimizationSignal'] = (
            (combination_score >= 80) |  # 高组合评分
            (abs(avg_correlation) <= 0.3)  # 低相关性
        )
        ...
        # 综合组合判断
        result['IsOptimalCombination'] = combination_score >= 75
        result['IsSuboptimalCombination'] = combination_score <= 35
    ...
    def get_patterns_Indicator_Base_Indicator(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        # 策略组合形态
        combination_score = result['CombinationScore']
        patterns['ZXM_OPTIMAL_COMBINATION'] = combination_score >= 85
        patterns['ZXM_GOOD_COMBINATION'] = (combination_score >= 65) & (combination_score < 85)
        patterns['ZXM_NEUTRAL_COMBINATION'] = (combination_score >= 45) & (combination_score < 65)
        patterns['ZXM_POOR_COMBINATION'] = (combination_score >= 25) & (combination_score < 45)
        patterns['ZXM_SUBOPTIMAL_COMBINATION'] = combination_score < 25
