...
class ZXMLiquidityAnalysis(BaseIndicator):
    ...
    def calculate(self, data: pd.DataFrame, *args, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(data) >= 20:
                ...
                # 流动性评分：波动性越低，流动性越好
                if volatility < 0.2:
                    result['liquidity_score'] = 90
                elif volatility < 0.4:
                    result['liquidity_score'] = 70
                elif volatility < 0.6:
                    result['liquidity_score'] = 50
                else:
                    result['liquidity_score'] = 30
            else:
                result['volatility'] = 0.3
            ...
            if len(data) >= 5:
                ...
                result['price_continuity'] = 0.8
            
            # 4. 综合流动性指数
            liquidity_components = [
                result.get('liquidity_score', 60) / 100,
                result.get('volume_stability', 0.7),
                result.get('price_continuity', 0.8)
            ]
            ...
            # 6. 流动性风险评估
            if result['liquidity_index'] < 40:
                result['liquidity_risk'] = 'high'
            elif result['liquidity_index'] < 60:
    ...
    def calculate_raw_score_Indicator_Base_Indicator(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        try:
            ...
            # 基于流动性指数计算评分
            liquidity_index = result.get('liquidity_index', 60.0)
