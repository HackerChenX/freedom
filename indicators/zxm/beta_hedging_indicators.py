...
class ZXMBetaHedging(BaseIndicator, PatternSignalMixin, MinimumPeriodsMixin):
    ...
    def set_parameters_Beta_Hedging(self, **kwargs):
        ...
        self.hedge_period = kwargs.get('hedge_period', 30)
        self.beta_threshold = kwargs.get('beta_threshold', 1.2)
        self.hedge_threshold = kwargs.get('hedge_threshold', 0.8)
        self.rebalance_frequency = kwargs.get('rebalance_frequency', 5)
    ...
    def minimum_periods(self) -> int:
        ...
        Returns:
            int: 最少需要的数据周期数
        """
        return max(self.lookback_period, self.hedge_period) + 20
    ...
    def _calculate_hedging_effectiveness(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 对冲评分
        hedging_score = (
            (1 - abs(hedged_beta - self.target_beta)) * 40 +  # Beta目标达成度
            volatility_reduction * 100 * 35 +  # 波动率降低效果
            hedge_efficiency * 25  # 对冲效率
        )
        ...
        result['HedgingScore'] = hedging_score.fillna(50)
    ...
    def _generate_hedging_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 对冲信号
        result['HedgingSignal'] = (
            (beta_value >= self.beta_threshold) |  # Beta过高
            (hedging_score <= 40)  # 对冲效果差
        )
        ...
        # 再平衡信号
        result['RebalanceSignal'] = (
            (result.index % self.rebalance_frequency == 0) &  # 定期再平衡
            (abs(beta_trend) >= 0.1)  # Beta趋势变化
        )
        
        # 综合对冲判断
        result['IsWellHedged'] = hedging_score >= 70
        result['IsPoorlyHedged'] = hedging_score <= 30
    ...
    def get_patterns_Indicator_Base_Indicator(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        # Beta对冲形态
        hedging_score = result['HedgingScore']
        patterns['ZXM_EXCELLENT_HEDGING'] = hedging_score >= 85
        patterns['ZXM_GOOD_HEDGING'] = (hedging_score >= 65) & (hedging_score < 85)
        patterns['ZXM_NEUTRAL_HEDGING'] = (hedging_score >= 45) & (hedging_score < 65)
        patterns['ZXM_POOR_HEDGING'] = (hedging_score >= 25) & (hedging_score < 45)
        patterns['ZXM_BAD_HEDGING'] = hedging_score < 25
