...
class ZXMPortfolioOptimization(BaseIndicator, PatternSignalMixin, MinimumPeriodsMixin):
    ...
    def minimum_periods(self) -> int:
        ...
        Returns:
            int: 最少需要的数据周期数
        """
        return max(self.lookback_period, self.rebalance_period) + 20
    ...
    def _calculate_return_risk_metrics(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 滚动收益率
        rolling_returns = daily_returns.rolling(window=self.lookback_period).mean() * 252
        
        # 滚动波动率
        rolling_volatility = daily_returns.rolling(window=self.lookback_period).std() * np.sqrt(252)
    ...
    def _calculate_optimal_weights(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 综合权重
        optimal_weight = (risk_parity_weight * 0.6 + sharpe_weight * 0.4).fillna(0.5)
        ...
        result['OptimalWeight'] = normalized_weight.fillna(0.5)
    ...
    def _calculate_portfolio_performance(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 投资组合波动率
        portfolio_volatility = portfolio_returns.rolling(window=self.lookback_period).std() * np.sqrt(252)
        
        # 投资组合夏普比率
        portfolio_annual_returns = portfolio_returns.rolling(window=self.lookback_period).mean() * 252
        ...
        tracking_error = active_returns.rolling(window=self.lookback_period).std() * np.sqrt(252)
        information_ratio = (active_returns.rolling(window=self.lookback_period).mean() * 252) / tracking_error
    ...
    def _calculate_optimization_score(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 夏普比率评分 (0-40分)
        sharpe_score = np.clip(portfolio_sharpe * 20, 0, 40)
        
        # 信息比率评分 (0-30分)
        info_ratio_score = np.clip(information_ratio * 15, 0, 30)
        
        # 波动率控制评分 (0-20分)
        volatility_score = np.clip((self.target_volatility - portfolio_volatility) / self.target_volatility * 20 + 10, 0, 20)
        
        # 回撤控制评分 (0-10分)
        drawdown_score = np.clip((-max_drawdown) * 50, 0, 10)
        ...
        result['OptimizationScore'] = optimization_score.fillna(50)
    ...
    def _generate_optimization_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        result['RebalanceSignal'] = abs(weight_change) > 0.05
        
        # 优化信号
        result['OptimizationSignal'] = (
            (optimization_score >= 80) |  # 高优化评分
            (portfolio_sharpe >= 1.5)     # 高夏普比率
        )
        
        # 风险控制信号
        result['RiskControlSignal'] = (
            (portfolio_volatility >= self.target_volatility * 1.2) |  # 波动率过高
            (optimization_score <= 30)  # 优化评分过低
        )
        ...
        result['WeightAdjustSignal'] = (
            (optimal_weight >= self.max_weight * 0.9) |  # 接近最大权重
            (optimal_weight <= self.min_weight * 1.1)    # 接近最小权重
        )
        
        # 综合优化判断
        result['IsOptimalPortfolio'] = optimization_score >= 75
        result['IsSuboptimalPortfolio'] = optimization_score <= 35
    ...
    def get_patterns_Indicator_Base_Indicator(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        # 投资组合优化形态
        optimization_score = result['OptimizationScore']
        patterns['ZXM_OPTIMAL_PORTFOLIO'] = optimization_score >= 85
        patterns['ZXM_GOOD_PORTFOLIO'] = (optimization_score >= 65) & (optimization_score < 85)
        patterns['ZXM_NEUTRAL_PORTFOLIO'] = (optimization_score >= 45) & (optimization_score < 65)
        patterns['ZXM_POOR_PORTFOLIO'] = (optimization_score >= 25) & (optimization_score < 45)
        patterns['ZXM_SUBOPTIMAL_PORTFOLIO'] = optimization_score < 25
