...
class ZXMCyclePosition(BaseIndicator, PatternSignalMixin, MinimumPeriodsMixin):
    ...
    def set_parameters_Cycle_Position(self, **kwargs):
        ...
        Args:
            **kwargs: 参数字典
        """
        self.cycle_period = kwargs.get('cycle_period', 60)
        self.trend_period = kwargs.get('trend_period', 20)
    ...
    def _calculate_cycle_position(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        result['CyclePosition'] = cycle_position.fillna(0.5)
        result['TrendPosition'] = trend_position.fillna(0.5)
    ...
    def _calculate_cycle_strength(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        cycle_strength = (
            price_strength * 0.4 +
            (volume_strength - 1).clip(0, 2) * 0.3 +
            volatility_strength.fillna(1) * 0.3
        )
        
        result['PriceMomentum'] = price_momentum
        result['CycleStrength'] = cycle_strength.fillna(0.5)
    ...
    def _calculate_cycle_phase(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 底部阶段
        cycle_phase[cycle_position <= 0.25] = 1
        
        # 顶部阶段
        cycle_phase[cycle_position >= 0.75] = 3
        
        # 下降阶段（价格下跌且位置较高）
        cycle_phase[(cycle_position > 0.5) & (price_momentum < -0.02)] = 4
    ...
    def _calculate_composite_cycle_score(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 强度调整
        strength_adjustment = (cycle_strength - 0.5) * 20
        ...
        phase_adjustment[cycle_phase == 3] = 10   # 顶部阶段加分
        phase_adjustment[cycle_phase == 4] = -20  # 下降阶段减分
    ...
    def _generate_cycle_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        result['CycleTurningSignal'] = (
            (abs(position_change) > 0.1) |  # 位置快速变化
            (cycle_phase.diff() != 0)       # 阶段转换
        )
        
        # 综合周期判断
        result['IsCycleTop'] = composite_score >= 80
        result['IsCycleBottom'] = composite_score <= 20
    ...
    def get_patterns_Indicator_Base_Indicator(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        # 周期位置形态
        cycle_position = result['CyclePosition']
        patterns['ZXM_CYCLE_TOP'] = cycle_position >= 0.8
        patterns['ZXM_CYCLE_HIGH'] = (cycle_position >= 0.6) & (cycle_position < 0.8)
        patterns['ZXM_CYCLE_MIDDLE'] = (cycle_position >= 0.4) & (cycle_position < 0.6)
        patterns['ZXM_CYCLE_LOW'] = (cycle_position >= 0.2) & (cycle_position < 0.4)
