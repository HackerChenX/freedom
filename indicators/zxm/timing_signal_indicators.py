...
class ZXMTimingSignal(BaseIndicator, PatternSignalMixin, MinimumPeriodsMixin):
    ...
    def set_parameters_Timing_Signal(self, **kwargs):
        ...
        self.medium_period = kwargs.get('medium_period', 20)
        self.long_period = kwargs.get('long_period', 60)
        self.signal_threshold = kwargs.get('signal_threshold', 0.6)
    ...
    def _calculate_technical_strength(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 综合技术强度
        technical_strength = (
            price_strength.fillna(0) * 0.5 +
            (volume_strength.fillna(1) - 1) * 50 * 0.3 +
            volatility_strength.fillna(1) * 20 * 0.2
        )
    ...
    def _calculate_market_timing(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        market_timing = (
            trend_timing * 0.4 +
            (position_timing - 0.5) * 2 * 0.3 +
            momentum_timing.fillna(0) * 0.3
        )
    ...
    def _calculate_composite_timing_score(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        composite_score = (
            ...
            market_timing * 30 * 0.35 +
            momentum_strength * 100 * 0.25
        )
    ...
    def _generate_timing_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        result['TimingStrengthSignal'] = (
            (composite_score >= 80) |
            (technical_strength > 15) |
            (abs(market_timing) > 0.8)
        )
        
        # 市场时机信号
        result['MarketTimingSignal'] = (
            (market_timing > 0.7) |  # 强势时机
            (market_timing < -0.7)   # 弱势时机
        )
        
        # 综合择时判断
        result['IsOptimalTiming'] = composite_score >= 70
        result['IsPoorTiming'] = composite_score <= 30
    ...
    def get_patterns_Indicator_Base_Indicator(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        # 择时形态
        composite_score = result['CompositeTimingScore']
        patterns['ZXM_OPTIMAL_TIMING'] = composite_score >= 80
        patterns['ZXM_GOOD_TIMING'] = (composite_score >= 60) & (composite_score < 80)
        patterns['ZXM_NEUTRAL_TIMING'] = (composite_score >= 40) & (composite_score < 60)
        patterns['ZXM_POOR_TIMING'] = (composite_score >= 20) & (composite_score < 40)
        patterns['ZXM_VERY_POOR_TIMING'] = composite_score < 20
