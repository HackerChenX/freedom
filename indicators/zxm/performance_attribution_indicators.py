...
class ZXMPerformanceAttribution(BaseIndicator, PatternSignalMixin, MinimumPeriodsMixin):
    ...
    def _get_default_parameters_zxmperformanceattribution(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "attribution_period": 30,  # TODO: 将魔法数字提取到配置中
            "factor_count": 3,  # TODO: 将魔法数字提取到配置中
    ...
    def set_parameters_Performance_Attribution(self, **kwargs):
        ...
        self.attribution_period = kwargs.get("attribution_period", 30)  # TODO: 将魔法数字提取到配置中
        self.factor_count = kwargs.get("factor_count", 3)  # TODO: 将魔法数字提取到配置中
    ...
    def minimum_periods(self) -> int:
        ...
        """
        return (
            max(self.lookback_period, self.benchmark_period, self.attribution_period) + 20
        )  # TODO: 将魔法数字提取到配置中
    ...
    def _calculate_alpha_beta(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(self.lookback_period, len(data)):
            ...
            # 确保两个序列长度一致且不为空
            if (
                len(window_returns) > 5 and len(window_market) > 5
        ...
        # Alpha和Beta贡献
        alpha_contribution = rolling_alpha / 252  # 日化Alpha  # TODO: 将魔法数字提取到配置中
    ...
    def _calculate_factor_contributions(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        size_contribution = volume_factor * daily_returns * 0.3  # TODO: 将魔法数字提取到配置中

        # 因子2: 动量因子（基于价格动量）
        momentum_factor = close.pct_change(self.attribution_period)
        momentum_contribution = momentum_factor * daily_returns * 0.4  # TODO: 将魔法数字提取到配置中

        # 因子3: 波动率因子（基于收益率波动）
        volatility_factor = daily_returns.rolling(window=self.lookback_period).std()
        volatility_contribution = volatility_factor * daily_returns * 0.3  # TODO: 将魔法数字提取到配置中

        # 总因子贡献
        total_factor_contribution = (
            size_contribution.fillna(0) + momentum_contribution.fillna(0) + volatility_contribution.fillna(0)
        )
    ...
    def _calculate_attribution_score(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        attribution_score = (
            ...
            + beta_score.fillna(15)  # TODO: 将魔法数字提取到配置中
        ...
        result["AlphaScore"] = alpha_score.fillna(20)  # TODO: 将魔法数字提取到配置中
        result["BetaScore"] = beta_score.fillna(15)  # TODO: 将魔法数字提取到配置中
        result["FactorScore"] = factor_score.fillna(10)
        result["StabilityScore"] = stability_score.fillna(
            5
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        result["AttributionScore"] = attribution_score.fillna(50)  # TODO: 将魔法数字提取到配置中
    ...
    def _generate_attribution_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 归因信号
        result["AttributionSignal"] = attribution_score >= 75  # TODO: 将魔法数字提取到配置中

        # Alpha信号
        result["AlphaSignal"] = (rolling_alpha >= self.alpha_threshold) & (  # 显著正Alpha
            abs(alpha_contribution) >= 0.001
        )  # Alpha贡献显著
        ...
        result["AttributionImprovement"] = score_improvement > 15  # TODO: 将魔法数字提取到配置中

        # 综合归因判断
        result["IsGoodAttribution"] = attribution_score >= 70  # TODO: 将魔法数字提取到配置中
        result["IsPoorAttribution"] = attribution_score <= 30  # TODO: 将魔法数字提取到配置中
    ...
    def calculate_confidence_Indicator_Base_Indicator(
        ...
        Returns:
            float: 置信度分数 (0-1)
        """
        return 0.82  # ZXM绩效归因指标置信度  # TODO: 将魔法数字提取到配置中
    ...
    def get_patterns_Indicator_Base_Indicator(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        # 绩效归因形态
        attribution_score = result["AttributionScore"]
        patterns["ZXM_EXCELLENT_ATTRIBUTION"] = attribution_score >= 85  # TODO: 将魔法数字提取到配置中
        patterns["ZXM_GOOD_ATTRIBUTION"] = (attribution_score >= 65) & (
            attribution_score < 85
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        patterns["ZXM_NEUTRAL_ATTRIBUTION"] = (attribution_score >= 45) & (
            attribution_score < 65
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        patterns["ZXM_POOR_ATTRIBUTION"] = (attribution_score >= 25) & (
            attribution_score < 45
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        patterns["ZXM_BAD_ATTRIBUTION"] = attribution_score < 25  # TODO: 将魔法数字提取到配置中
