...
class ZXMHotSpot(BaseIndicator, PatternSignalMixin, MinimumPeriodsMixin):
    ...
    def set_parameters_Hot_Spot(self, **kwargs):
        ...
        self.price_threshold = kwargs.get('price_threshold', 0.03)
        self.momentum_period = kwargs.get('momentum_period', 5)
    ...
    def _calculate_hot_spot_strength(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 价格热点强度
        price_hot_strength = (
            (short_momentum > self.price_threshold).astype(int) * 30 +
            (medium_momentum > self.price_threshold * 2).astype(int) * 20 +
            (short_momentum > medium_momentum).astype(int) * 10
        )
        
        # 成交量热点强度
        volume_hot_strength = (
            (volume_amplification > self.volume_threshold).astype(int) * 30 +
            (volume_amplification > self.volume_threshold * 1.5).astype(int) * 20
        )
    ...
    def _calculate_composite_hot_spot_score(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 加权计算综合评分
        composite_score = (
            hot_spot_strength * 0.5 +
            (short_momentum * 100).clip(0, 50) * 0.3 +
            (volume_amplification * 20).clip(0, 50) * 0.2
        )
    ...
    def _generate_hot_spot_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        result['ModerateHotSpotSignal'] = (
            ...
            ((short_momentum > self.price_threshold * 0.5) | 
        ...
        # 综合热点判断
        result['IsHotSpot'] = composite_score >= 70
    ...
    def get_patterns_Indicator_Base_Indicator(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        # 热点形态
        composite_score = result['CompositeHotSpotScore']
        patterns['ZXM_STRONG_HOT_SPOT'] = composite_score >= 80
        patterns['ZXM_MODERATE_HOT_SPOT'] = (composite_score >= 60) & (composite_score < 80)
        patterns['ZXM_WEAK_HOT_SPOT'] = (composite_score >= 40) & (composite_score < 60)
        patterns['ZXM_COLD_SPOT'] = composite_score < 40
