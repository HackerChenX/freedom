...
class SelectionModel(BaseIndicator, PatternSignalMixin, MinimumPeriodsMixin, ZXMAbstractMethodsMixin):
    ...
    def _calculate_selectionmodel(self, data: pd.DataFrame, *args, **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            if "ValueScore" in score_result.columns:
                ...
                result.loc[:, "ValueScore"] = 50.0  # 默认中性评分  # TODO: 将魔法数字提取到配置中

            result.loc[:, "FinalScore"] = score_result["FinalScore"]

            # 5. 计算买点信号  # TODO: 将魔法数字提取到配置中
            ...
            # 7. 计算整合选股信号  # TODO: 将魔法数字提取到配置中
            # a. 强趋势上涨股
            strong_uptrend = result["TrendDirection"] == 1

            # b. 放量突破股
            volume_breakout = result["BreakoutBuyPoint"] & (result["VolumeScore"] >= 70)  # TODO: 将魔法数字提取到配置中
            ...
            # d. 洗盘后启动股
            washplate_start = (
                result["AnyWashPlate"] & (result["TrendScore"] >= 60) & (result["VolumeScore"] >= 60)
            )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

            # e. 低吸高弹性股
            low_buy_elastic = (result["ElasticityBuySignal"] | result["BottomVolumeBuyPoint"]) & (
                result["BounceStrength"] >= 50
            )  # TODO: 将魔法数字提取到配置中
            ...
            result.loc[:, "FinalSelect"] = (
                strong_uptrend | volume_breakout | pullback_buy | washplate_start | low_buy_elastic
            )

            # 8. 计算选股综合得分  # TODO: 将魔法数字提取到配置中
            # 将各维度得分加权平均
            ...
            for i in range(len(data)):
                ...
                if result["AnyBuyPoint"].iloc[i]:
                    if result["BreakoutBuyPoint"].iloc[i]:
                        buy_point_bonus = 15  # 突破买点价值最高  # TODO: 将魔法数字提取到配置中
                    elif result["PullbackStabilizeBuyPoint"].iloc[i]:
                        buy_point_bonus = 12  # 回调企稳买点次之  # TODO: 将魔法数字提取到配置中
                    else:
                ...
                if result["AnyWashPlate"].iloc[i]:
                    washplate_bonus = 8  # TODO: 将魔法数字提取到配置中
                else:
                    washplate_bonus = 0

                # 趋势强度加减分
                if result["TrendDirection"].iloc[i] == 1:
                    trend_bonus = 15  # TODO: 将魔法数字提取到配置中
                ...
                if result["ElasticityBuySignal"].iloc[i]:
                    ...
                    elasticity_bonus = 7  # TODO: 将魔法数字提取到配置中
                else:
                    elasticity_bonus = 0

                # 综合计算
                selection_score[i] = min(
                    100, base_score + buy_point_bonus + washplate_bonus + trend_bonus + elasticity_bonus
                )

            result.loc[:, "SelectionScore"] = selection_score

            # 9. 计算买入优先级  # TODO: 将魔法数字提取到配置中
            # 将选中的股票按优先级排序（1-5，1为最高）  # TODO: 将魔法数字提取到配置中
            ...
            for i in range(len(data)):
                ...
                # 计算基础优先级
                if (
                    result["StrongUptrendSelect"].iloc[i] and result["SelectionScore"].iloc[i] >= 85
                ):  # TODO: 将魔法数字提取到配置中
                    priority[i] = 1  # 最高优先级
                elif result["VolumeBreakoutSelect"].iloc[i] or (
                    result["PullbackBuySelect"].iloc[i] and result["TrendScore"].iloc[i] >= 70
                ):  # TODO: 将魔法数字提取到配置中
                    priority[i] = 2  # 次高优先级
                elif result["WashplateStartSelect"].iloc[i]:
                    priority[i] = 3  # 中等优先级  # TODO: 将魔法数字提取到配置中
                elif result["PullbackBuySelect"].iloc[i]:
                    priority[i] = 4  # 次低优先级  # TODO: 将魔法数字提取到配置中
                elif result["LowBuyElasticSelect"].iloc[i]:
                    priority[i] = 5  # 最低优先级  # TODO: 将魔法数字提取到配置中

                # 调整优先级
                # 如果综合得分特别高，提升优先级
                if result["SelectionScore"].iloc[i] >= 90 and priority[i] > 1:  # TODO: 将魔法数字提取到配置中
                    priority[i] -= 1
            ...
            result.loc[:, "SelectionScore"] = pd.Series(50, index=data.index)  # TODO: 将魔法数字提取到配置中
    ...
    def identify_patterns_Model(self, data: pd.DataFrame, **kwargs) -> List[str]:
        ...
        if len(result) > 0:
            ...
            if last_row["FinalSelect"]:
                ...
                elif priority == 3:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
                    patterns.append("中等优先级选股")
                elif priority == 4:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
                    patterns.append("低优先级选股")
                elif priority == 5:  # TODO: 将魔法数字提取到配置中
    ...
    def generate_signals_Model(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        for i in signals.index:
            if result.loc[i, "FinalSelect"]:
                # 根据选股得分调整（最多±20）  # TODO: 将魔法数字提取到配置中
                score_adj = (
                    (score.iloc[i] - 70) / 30 * 20
                )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

                # 根据优先级调整（最多±10）
                priority = result.loc[i, "BuyPriority"]
                priority_adj = (6 - priority) * 2.5  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

                signals.loc[i, "confidence"] = min(
                    95, max(50, 60 + score_adj + priority_adj)
                )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        ...
        # 低优先级选股风险较高
        signals.loc[result["BuyPriority"] >= 4, "risk_level"] = "高"  # TODO: 将魔法数字提取到配置中
        ...
        for i in signals.index[signals["buy_signal"]]:
            ...
            # 基础仓位 - 根据优先级
            if priority == 1:
                base_position = 0.5  # TODO: 将魔法数字提取到配置中
            elif priority == 2:
                base_position = 0.4  # TODO: 将魔法数字提取到配置中
            elif priority == 3:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
                base_position = 0.3  # TODO: 将魔法数字提取到配置中
            elif priority == 4:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
                base_position = 0.2
            else:
                base_position = 0.1
            ...
            # 得分调整 - 高分加仓
            if score_val >= 90:  # TODO: 将魔法数字提取到配置中
                score_adj = 0.2
            elif score_val >= 80:  # TODO: 将魔法数字提取到配置中
                score_adj = 0.1
            elif score_val >= 70:  # TODO: 将魔法数字提取到配置中
                score_adj = 0.05  # TODO: 将魔法数字提取到配置中
            else:
                score_adj = 0

            signals.loc[i, "position_size"] = min(0.7, base_position + score_adj)  # TODO: 将魔法数字提取到配置中
        ...
        for i in signals.index[signals["buy_signal"]]:
            try:
                ...
                if result.loc[i, "StrongUptrendSelect"]:
                    # 强趋势上涨 - 使用20日均线作为止损
                    ma20 = data["close"].rolling(window=20).mean().iloc[idx]  # TODO: 将魔法数字提取到配置中
                    signals.loc[i, "stop_loss"] = ma20 * 0.95  # TODO: 将魔法数字提取到配置中

                elif result.loc[i, "VolumeBreakoutSelect"]:
                    # 放量突破 - 使用突破点位作为止损
                    # 简单以当前价格的5%作为止损
                    signals.loc[i, "stop_loss"] = data["close"].iloc[idx] * 0.95  # TODO: 将魔法数字提取到配置中
                    ...
                    if idx >= 20:  # TODO: 将魔法数字提取到配置中
                        low_price = data["low"].iloc[idx - 20 : idx + 1].min()  # TODO: 将魔法数字提取到配置中
                        signals.loc[i, "stop_loss"] = (
                            low_price * 0.97
                        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
                    ...
                    # 默认止损策略 - 当前价格的7%止损
                    signals.loc[i, "stop_loss"] = data["close"].iloc[idx] * 0.93  # TODO: 将魔法数字提取到配置中
    ...
    def calculate_confidence_Model(self, score: pd.Series, patterns: List[str], signals: Dict[str, pd.Series]) -> float:
        ...
        if "最高优先级选股" in patterns:
            pattern_boost += 0.25  # TODO: 将魔法数字提取到配置中
        ...
        if "超强上升趋势" in patterns:
            ...
            pattern_boost += 0.08  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
    ...
    def get_patterns_Model(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        patterns_df.loc[:, "中等优先级选股"] = result["BuyPriority"] == 3  # TODO: 将魔法数字提取到配置中
        patterns_df.loc[:, "低优先级选股"] = result["BuyPriority"] == 4  # TODO: 将魔法数字提取到配置中
        patterns_df.loc[:, "最低优先级选股"] = result["BuyPriority"] == 5  # TODO: 将魔法数字提取到配置中
    ...
    def set_parameters_Model(self, **kwargs):
        ...
        # 设置选股模型自身的参数
        self.selection_threshold = kwargs.get("selection_threshold", 70)  # TODO: 将魔法数字提取到配置中
        self.priority_threshold = kwargs.get("priority_threshold", 85)  # TODO: 将魔法数字提取到配置中
