...
class WrWr(BaseIndicator, PatternSignalMixin, MinimumPeriodsMixin):
    ...
    def calculate_raw_score_Wr_Wr(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        )  # 基础分50分  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        ...
        # 2. WR穿越关键位置评分
        # WR从超卖区上穿-80+25分  # TODO: 将魔法数字提取到配置中
        wr_cross_up_oversold = crossover(wr, -80)  # TODO: 将魔法数字提取到配置中
        score += wr_cross_up_oversold * 25  # TODO: 将魔法数字提取到配置中

        # WR从超买区下穿-20-25分  # TODO: 将魔法数字提取到配置中
        wr_cross_down_overbought = crossunder(wr, -20)  # TODO: 将魔法数字提取到配置中
        score -= wr_cross_down_overbought * 25  # TODO: 将魔法数字提取到配置中

        # 3. 中线穿越评分  # TODO: 将魔法数字提取到配置中
        # WR上穿-50+15分  # TODO: 将魔法数字提取到配置中
        wr_cross_up_middle = crossover(wr, -50)  # TODO: 将魔法数字提取到配置中
        score += wr_cross_up_middle * 15  # TODO: 将魔法数字提取到配置中

        # WR下穿-50-15分  # TODO: 将魔法数字提取到配置中
        wr_cross_down_middle = crossunder(wr, -50)  # TODO: 将魔法数字提取到配置中
        score -= wr_cross_down_middle * 15  # TODO: 将魔法数字提取到配置中

        # 4. WR背离评分  # TODO: 将魔法数字提取到配置中
        if (
            len(data) >= 20
        ):  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
    ...
    def identify_patterns_Wr(self, data: pd.DataFrame, **kwargs) -> List[str]:
        ...
        # 1. 超买超卖形态
        if current_wr <= -90:  # TODO: 将魔法数字提取到配置中
            patterns.append("WR极度超卖")
        elif current_wr <= -80:  # TODO: 将魔法数字提取到配置中
            patterns.append("WR超卖")
        elif current_wr >= -10:
            patterns.append("WR极度超买")
        elif current_wr >= -20:  # TODO: 将魔法数字提取到配置中
            patterns.append("WR超买")

        # 2. 穿越形态
        if crossover(recent_wr, -80).any():  # TODO: 将魔法数字提取到配置中
            patterns.append("WR上穿超卖线")
        if crossunder(recent_wr, -20).any():  # TODO: 将魔法数字提取到配置中
            patterns.append("WR下穿超买线")
        if crossover(recent_wr, -50).any():  # TODO: 将魔法数字提取到配置中
            patterns.append("WR上穿中线")
        if crossunder(recent_wr, -50).any():  # TODO: 将魔法数字提取到配置中
            patterns.append("WR下穿中线")

        # 3. 背离形态  # TODO: 将魔法数字提取到配置中
        if (
            len(data) >= 20
        ):  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        ...
        # 4. 钝化形态  # TODO: 将魔法数字提取到配置中
        if self._detect_wr_stagnation(
            recent_wr, threshold=-80, periods=5, direction="low"
        ):  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            patterns.append("WR低位钝化")
        if self._detect_wr_stagnation(
            recent_wr, threshold=-20, periods=5, direction="high"
        ):  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            patterns.append("WR高位钝化")

        # 5. 反转形态  # TODO: 将魔法数字提取到配置中
    ...
    def _calculate_wr_divergence_Wr(self, price: pd.Series, wr: pd.Series) -> pd.Series:
        ...
        if len(price) < 20:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        ...
        for i in range(window, len(price) - window):
            ...
            if price.iloc[i] == price_window.max():  # 价格峰值
                if wr.iloc[i] != wr_window.max():  # WR未创新高
                    divergence_score.iloc[i : i + 10] -= 25  # 负背离  # TODO: 将魔法数字提取到配置中
            elif price.iloc[i] == price_window.min():  # 价格谷值
                if wr.iloc[i] != wr_window.min():  # WR未创新低
                    divergence_score.iloc[i : i + 10] += 25  # 正背离  # TODO: 将魔法数字提取到配置中
    ...
    def _calculate_wr_trend_score(self, wr: pd.Series) -> pd.Series:
        ...
        # 计算WR斜率
        wr_slope = wr.diff(3)  # TODO: 将魔法数字提取到配置中

        # 趋势评分
        trend_score += np.where(wr_slope > 5, 10, 0)  # 强烈上升+10分  # TODO: 将魔法数字提取到配置中
        trend_score += np.where(wr_slope > 2, 5, 0)  # 温和上升+5分  # TODO: 将魔法数字提取到配置中
        trend_score -= np.where(wr_slope < -5, 10, 0)  # 强烈下降-10分  # TODO: 将魔法数字提取到配置中
        trend_score -= np.where(wr_slope < -2, 5, 0)  # 温和下降-5分  # TODO: 将魔法数字提取到配置中
    ...
    def _detect_wr_divergence_pattern(self, price: pd.Series, wr: pd.Series) -> Optional[str]:
        ...
        # 寻找最近的峰值和谷值
        recent_price = price.tail(20)  # TODO: 将魔法数字提取到配置中
        recent_wr = wr.tail(20)  # TODO: 将魔法数字提取到配置中
        ...
        for i in range(2, len(recent_price) - 2):
            ...
            elif recent_price.iloc[i] < recent_price.iloc[i - 1] and recent_price.iloc[i] < recent_price.iloc[i + 1]:
                price_extremes.append(recent_price.iloc[i])
                wr_extremes.append(recent_wr.iloc[i])
    ...
    def _detect_wr_stagnation(self, wr: pd.Series, threshold: float, periods: int, direction: str) -> bool:
        """
        检测WR钝化
    ...
    def _detect_wr_reversal_pattern(self, wr: pd.Series) -> bool:
        ...
        if (
            recent_wr.iloc[0] < -80
            and recent_wr.iloc[-1] > -50  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            and (recent_wr.iloc[-1] - recent_wr.iloc[0]) > 20
        ):  # TODO: 将魔法数字提取到配置中
            return True
        ...
        if (
            recent_wr.iloc[0] > -20
            and recent_wr.iloc[-1] < -50  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            and (recent_wr.iloc[0] - recent_wr.iloc[-1]) > 20
        ):  # TODO: 将魔法数字提取到配置中
            return True
    ...
    def get_signals_Wr(self, df: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        # 获取参数
        overbought = kwargs.get("overbought", -20)  # 超买阈值  # TODO: 将魔法数字提取到配置中
        oversold = kwargs.get("oversold", -80)  # 超卖阈值  # TODO: 将魔法数字提取到配置中
    ...
    def get_patterns_Wr_Wr(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        # 1. WR超买超卖形态
        patterns_df["WR_EXTREME_OVERSOLD"] = wr < -90  # TODO: 将魔法数字提取到配置中
        patterns_df["WR_OVERSOLD"] = (wr >= -90) & (
            wr < -80
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        patterns_df["WR_NORMAL"] = (wr >= -80) & (
            wr <= -20
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        patterns_df["WR_OVERBOUGHT"] = (wr > -20) & (wr <= -10)  # TODO: 将魔法数字提取到配置中
        patterns_df["WR_EXTREME_OVERBOUGHT"] = wr > -10

        # 2. WR穿越形态
        patterns_df["WR_CROSS_ABOVE_OVERSOLD"] = crossover(wr, -80)  # TODO: 将魔法数字提取到配置中
        patterns_df["WR_CROSS_BELOW_OVERBOUGHT"] = crossunder(wr, -20)  # TODO: 将魔法数字提取到配置中
        patterns_df["WR_CROSS_ABOVE_MID"] = crossover(wr, -50)  # TODO: 将魔法数字提取到配置中
        patterns_df["WR_CROSS_BELOW_MID"] = crossunder(wr, -50)  # TODO: 将魔法数字提取到配置中

        # 3. WR趋势形态  # TODO: 将魔法数字提取到配置中
        patterns_df["WR_RISING"] = wr > wr.shift(1)
        patterns_df["WR_FALLING"] = wr < wr.shift(1)
        patterns_df["WR_UPTREND"] = (
            (wr > wr.shift(1))
            & (wr.shift(1) > wr.shift(2))
            & (wr.shift(2) > wr.shift(3))  # TODO: 将魔法数字提取到配置中
        )
        ...
        # 4. WR钝化形态  # TODO: 将魔法数字提取到配置中
        patterns_df["WR_LOW_STAGNATION"] = wr.rolling(5).apply(
            lambda x: (x < -80).all(), raw=False
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        patterns_df["WR_HIGH_STAGNATION"] = wr.rolling(5).apply(
            lambda x: (x > -20).all(), raw=False
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        # 5. WR反转形态  # TODO: 将魔法数字提取到配置中
        # 从超卖区快速反转
        wr_change_5 = wr - wr.shift(4)  # TODO: 将魔法数字提取到配置中
        patterns_df["WR_BULLISH_REVERSAL"] = (
            (wr.shift(4) < -80) & (wr > -50) & (wr_change_5 > 20)
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        patterns_df["WR_BEARISH_REVERSAL"] = (
    ...
    def calculate_confidence_Wr_Wr(self, score: pd.Series, patterns: pd.DataFrame, signals: dict) -> float:
        ...
        # 中性评分置信度中等
            ...
            confidence += (
                0.15  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            )
    ...
    def get_patterns(self, data: pd.DataFrame = None, **kwargs) -> pd.DataFrame:
        ...
        # 1. 超买形态 (WR > -20)  # TODO: 将魔法数字提取到配置中
        patterns_df["WR_OVERBOUGHT"] = wr_values > -20  # TODO: 将魔法数字提取到配置中

        # 2. 超卖形态 (WR < -80)  # TODO: 将魔法数字提取到配置中
        patterns_df["WR_OVERSOLD"] = wr_values < -80  # TODO: 将魔法数字提取到配置中

        # 3. 极端超买形态 (WR > -10)  # TODO: 将魔法数字提取到配置中
        patterns_df["WR_EXTREME_OVERBOUGHT"] = wr_values > -10

        # 4. 极端超卖形态 (WR < -90)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        patterns_df["WR_EXTREME_OVERSOLD"] = wr_values < -90  # TODO: 将魔法数字提取到配置中

        # 5. 从超卖区域反弹形态  # TODO: 将魔法数字提取到配置中
        patterns_df["WR_OVERSOLD_BOUNCE"] = (wr_values > -80) & (
            wr_values.shift(1) <= -80
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        # 6. 从超买区域回落形态  # TODO: 将魔法数字提取到配置中
        patterns_df["WR_OVERBOUGHT_FALL"] = (wr_values < -20) & (
            wr_values.shift(1) >= -20
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        # 7. 中性区域突破形态  # TODO: 将魔法数字提取到配置中
        patterns_df["WR_NEUTRAL_BREAK_UP"] = (
            wr_values > -50
        )  # TODO: 将魔法数字提取到配置中 & (wr_values.shift(1) <= -50)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        patterns_df["WR_NEUTRAL_BREAK_DOWN"] = (
            wr_values < -50
        )  # TODO: 将魔法数字提取到配置中 & (wr_values.shift(1) >= -50)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
    ...
    def calculate_raw_score(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        # 1. 基于WR位置的评分
        # 超卖区域加分 (WR < -80)  # TODO: 将魔法数字提取到配置中
        oversold_condition = wr_values < -80  # TODO: 将魔法数字提取到配置中
        score += oversold_condition * 20  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        # 极端超卖加分 (WR < -90)  # TODO: 将魔法数字提取到配置中
        extreme_oversold_condition = wr_values < -90  # TODO: 将魔法数字提取到配置中
        score += extreme_oversold_condition * 15  # TODO: 将魔法数字提取到配置中

        # 超买区域减分 (WR > -20)  # TODO: 将魔法数字提取到配置中
        overbought_condition = wr_values > -20  # TODO: 将魔法数字提取到配置中
        score -= overbought_condition * 20  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        ...
        # 2. 基于WR反弹和回落的评分
        oversold_bounce = (wr_values > -80) & (
            wr_values.shift(1) <= -80
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        overbought_fall = (wr_values < -20) & (
            wr_values.shift(1) >= -20
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中

        # 超卖反弹加分
        score += oversold_bounce * 15  # TODO: 将魔法数字提取到配置中

        # 超买回落减分
        score -= overbought_fall * 15  # TODO: 将魔法数字提取到配置中

        # 3. 基于WR趋势的评分  # TODO: 将魔法数字提取到配置中
        # WR上升趋势加分
        wr_rising = wr_values > wr_values.shift(1)
        score += wr_rising * 5  # TODO: 将魔法数字提取到配置中
    ...
    def get_signals(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        result_df.loc[oversold_bounce, "wr_strength"] = 0.8  # TODO: 将魔法数字提取到配置中
        result_df.loc[oversold_bounce, "wr_confidence"] = 0.9  # TODO: 将魔法数字提取到配置中
        ...
        result_df.loc[overbought_fall, "wr_strength"] = 0.8  # TODO: 将魔法数字提取到配置中
        result_df.loc[overbought_fall, "wr_confidence"] = 0.9  # TODO: 将魔法数字提取到配置中

        # 3. 中性区域突破信号  # TODO: 将魔法数字提取到配置中
        neutral_break_up = wr_values > -50  # TODO: 将魔法数字提取到配置中 & (wr_values.shift(1) <= -50)
        result_df.loc[neutral_break_up, "wr_signal"] = 1
        result_df.loc[neutral_break_up, "wr_strength"] = 0.6  # TODO: 将魔法数字提取到配置中
        result_df.loc[neutral_break_up, "wr_confidence"] = 0.7  # TODO: 将魔法数字提取到配置中

        neutral_break_down = wr_values < -50  # TODO: 将魔法数字提取到配置中 & (wr_values.shift(1) >= -50)
        result_df.loc[neutral_break_down, "wr_signal"] = -1
        result_df.loc[neutral_break_down, "wr_strength"] = 0.6  # TODO: 将魔法数字提取到配置中
        result_df.loc[neutral_break_down, "wr_confidence"] = 0.7  # TODO: 将魔法数字提取到配置中

        # 4. 极端超卖/超买信号  # TODO: 将魔法数字提取到配置中
        extreme_oversold = wr_values < -90  # TODO: 将魔法数字提取到配置中
        result_df.loc[extreme_oversold, "wr_signal"] = 1
        result_df.loc[extreme_oversold, "wr_strength"] = 0.9  # TODO: 将魔法数字提取到配置中
        result_df.loc[extreme_oversold, "wr_confidence"] = 0.8  # TODO: 将魔法数字提取到配置中

        extreme_overbought = wr_values > -10
        result_df.loc[extreme_overbought, "wr_signal"] = -1
        result_df.loc[extreme_overbought, "wr_strength"] = 0.9  # TODO: 将魔法数字提取到配置中
        result_df.loc[extreme_overbought, "wr_confidence"] = 0.8  # TODO: 将魔法数字提取到配置中
    ...
    def calculate_score(self, data: pd.DataFrame, **kwargs) -> dict:
        ...
        # 基于WR位置计算置信度
        if wr_value < -80 or wr_value > -20:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            confidence = 0.9  # 极端位置置信度高  # TODO: 将魔法数字提取到配置中
        elif wr_value < -70 or wr_value > -30:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            confidence = 0.7  # TODO: 将魔法数字提取到配置中
        elif wr_value < -60 or wr_value > -40:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            confidence = (
                0.5  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            )
        else:
            confidence = 0.3  # 中性区域置信度低  # TODO: 将魔法数字提取到配置中
    ...
    def set_parameters(self, **kwargs):
        ...
        if "overbought" in kwargs:
            ...
            if isinstance(overbought, (int, float)) and -50 <= overbought <= 0:  # TODO: 将魔法数字提取到配置中
        ...
        if "oversold" in kwargs:
            ...
            if isinstance(oversold, (int, float)) and -100 <= oversold <= -50:  # TODO: 将魔法数字提取到配置中
