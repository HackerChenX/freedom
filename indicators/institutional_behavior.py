...
class InstitutionalBehavior(BaseIndicator, PatternSignalMixin, MinimumPeriodsMixin):
    ...
    def calculate_raw_score_Behavior(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        stable_price = price_volatility < price_volatility.rolling(window=30, min_periods=1).mean()
        ...
        money_flow_ma = money_flow.rolling(window=20, min_periods=1).mean()
        ...
        # 🔧 Ultra Think修复：正确处理NaN值
        gentle_accumulation = (
            (df['close'] > df['close'].shift(5).fillna(df['close'])) &  # 5日上涨
            (price_volatility < price_volatility.rolling(window=20, min_periods=1).mean()) &  # 波动率低
...
class FundFlow(BaseIndicator, PatternSignalMixin, MinimumPeriodsMixin):
    ...
    def _calculate_institutional_behavior(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 价涨量缩（散户跟风）
        institutional_behavior[(price_change > 0) & (volume_change < 0)] = 0.5
    ...
    def _calculate_flow_strength(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 计算流向强度
        flow_strength = (money_flow_ratio * 0.6 + institutional_behavior * 40 * 0.4)

        # 标准化到0-100范围
        flow_strength_normalized = ((flow_strength - flow_strength.min()) / (flow_strength.max() - flow_strength.min()) * 100).fillna(50)
    ...
    def _calculate_composite_flow_score(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        composite_score = (
            ...
            institutional_behavior * 30 * 0.3 +
            flow_strength * 0.3
        )

        # 标准化到0-100范围
        composite_score_normalized = ((composite_score - composite_score.min()) / (composite_score.max() - composite_score.min()) * 100).fillna(50)
    ...
    def _generate_fund_flow_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 强资金流入信号（买入机会）
        result['StrongInflowSignal'] = (composite_score >= 70) & (institutional_behavior > 0.5)

        # 强资金流出信号（卖出警告）
        result['StrongOutflowSignal'] = (composite_score <= 30) & (institutional_behavior < -0.5)

        # 资金流向转折信号
        score_change = composite_score.diff()
        result['FlowReversalSignal'] = (
            (score_change > 15) |  # 资金流向快速好转
            (score_change < -15)   # 资金流向快速恶化
        )
    ...
    def get_patterns_Indicator_Base_Indicator(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        # 资金流向形态
        composite_score = result['CompositeFundFlowScore']
        patterns['ZXM_STRONG_INFLOW'] = composite_score >= 80
        patterns['ZXM_MODERATE_INFLOW'] = (composite_score >= 60) & (composite_score < 80)
        patterns['ZXM_NEUTRAL_FLOW'] = (composite_score >= 40) & (composite_score < 60)
        patterns['ZXM_MODERATE_OUTFLOW'] = (composite_score >= 20) & (composite_score < 40)
        patterns['ZXM_STRONG_OUTFLOW'] = composite_score < 20
