...
class Rectangle(BaseIndicator, PatternSignalMixin, MinimumPeriodsMixin):
    ...
    def __init__(self, period: int = 20, tolerance: float = 0.02):  # TODO: 将魔法数字提取到配置中
        # 依赖注入示例:
        ...
        self._parameters = {"period": period, "tolerance": tolerance, "price_col": "close"}
            ...
            for i in range(self.period, len(data)):
                ...
                if range_height > 0:
                    ...
                    for j in range(len(window)):
                        ...
                        if (
                            abs(low - support_level) <= range_height * self.tolerance
                            or abs(high - resistance_level) <= range_height * self.tolerance
                            or (low >= support_level and high <= resistance_level)
                        ):
                            in_range_count += 1

                    # 如果大部分价格都在矩形范围内，则认为是矩形形态
                    if in_range_count / len(window) >= 0.7:  # TODO: 将魔法数字提取到配置中
    ...
    def _identify_breakout_up(self, data: pd.DataFrame) -> pd.Series:
        ...
        try:
            ...
            for i in range(1, len(data)):
                if data["rectangle"].iloc[i - 1] and data["close"].iloc[i] > data["resistance"].iloc[i - 1] * (
                    1 + self.tolerance
                ):
    ...
    def _identify_breakout_down(self, data: pd.DataFrame) -> pd.Series:
        ...
        try:
            ...
            for i in range(1, len(data)):
                if data["rectangle"].iloc[i - 1] and data["close"].iloc[i] < data["support"].iloc[i - 1] * (
                    1 - self.tolerance
                ):
    ...
    def minimum_periods(self) -> int:
        ...
        Returns:
            int: 最少需要的数据周期数
        """
        return self.period + 5  # TODO: 将魔法数字提取到配置中
    ...
    def calculate_raw_score_Indicator_Base_Indicator(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        if isinstance(result, pd.DataFrame) and not result.empty:
            if "breakout_up" in result.columns:
                score[result["breakout_up"]] = 80.0  # 向上突破，看涨  # TODO: 将魔法数字提取到配置中
            if "breakout_down" in result.columns:
                score[result["breakout_down"]] = 20.0  # 向下突破，看跌  # TODO: 将魔法数字提取到配置中
            if "rectangle" in result.columns:
                score[result["rectangle"]] = 60.0  # 矩形整理，轻微看涨  # TODO: 将魔法数字提取到配置中
    ...
    def calculate_confidence_Indicator_Base_Indicator(
        ...
        """抽象基类要求的置信度方法"""
        base_confidence = 0.7  # 矩形形态相对可靠  # TODO: 将魔法数字提取到配置中
