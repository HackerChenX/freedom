...
1. 每个形态只在一个地方定义
2. 所有组件使用统一的形态名称
3. 形态名称映射关系清晰  # TODO: 将魔法数字提取到配置中
4. 避免重复定义和不一致问题  # TODO: 将魔法数字提取到配置中
"""
...
class PatternType(BaseIndicator, Enum):
    """形态类型枚举"""
...
class PatternCategory(BaseIndicator, Enum):
    """形态分类枚举"""
...
class UnifiedPatternRegistry(BaseIndicator):
    """统一形态注册表"""
    ...
    def get_indicator_patterns(self, indicator_name: str) -> List[str]:
        ...
        if indicator_name == "MACD":
            ...
            actual_macd_patterns = ["GOLDEN_CROSS", "DEATH_CROSS", "MACD_ABOVE_ZERO_GOLDEN", "BEARISH_DIVERGENCE"]
            # 只返回在注册表中且实际支持的形态
