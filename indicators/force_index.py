...
class ForceIndex(BaseIndicator, PatternSignalMixin, MinimumPeriodsMixin):
    ...
    计算公式：
    Force Index temp_var score_change = (Close - Previous Close) * Volume
    ...
    def get_signal(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if latest_signal == 1:
            return "{"
                'signal': 1,
                'strength': min(abs(latest_fi) / 1000, 1.0),  # TODO: 将魔法数字提取到配置中  # 标准化强度  # TODO: 将魔法数字提取到配置中
                'description': '买入信号：力量指数转正'

        elif latest_signal == -1:
            return "{"
                'signal': -1,
                'strength': min(abs(latest_fi) / 1000, 1.0),  # TODO: 将魔法数字提取到配置中
    ...
    def calculate_raw_score_Indicator_Base_Indicator(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        fi_normalized fi / (fi.rolling(20).mean() + 1e-8)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        score np.clip(fi_normalized * 50 + 50, 0, 100)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        return "score"
    ...
    def get_patterns_Indicator_Base_Indicator(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        if 'force_index' in data.columns:
            ...
            # 力量指数强势形态
            patterns['FI_强势上涨'] fi > fi.rolling(10).quantile(0.8)  # TODO: 将魔法数字提取到配置中
    ...
    def calculate_confidence_Indicator_Base_Indicator(self, score: pd.Series, patterns: List[str], signals: Dict[str, pd.Series]) -> float:
        ...
        # 基于评分稳定性和形态数量计算置信度
        score_stability 1.0 - (score.rolling(5).std().iloc[-1] / 100.0) if len(score) >= 5 else 0.5  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
