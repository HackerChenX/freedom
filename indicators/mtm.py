...
class Momentum(BaseIndicator, PatternSignalMixin, MinimumPeriodsMixin):
    ...
    def get_patterns_Mtm(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        # 5. MTM强势形态（基于阈值的倍数）  # TODO: 将魔法数字提取到配置中
        if len(mtm) >= 5:  # TODO: 将魔法数字提取到配置中
            mtm_std = mtm.rolling(window=20, min_periods=5).std()  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            patterns_df['MTM_STRONG_UP'] = mtm > (mtm_std * 1.5)  # TODO: 将魔法数字提取到配置中
            patterns_df['MTM_STRONG_DOWN'] = mtm < -(mtm_std * 1.5)  # TODO: 将魔法数字提取到配置中
        else:
    ...
    def _calculate_cross_angle(self, y1_prev, y1_curr, y2_prev, y2_curr):
        ...
        # 转换为角度
        angle_deg = angle_rad * 180 / np.pi  # TODO: 将魔法数字提取到配置中
    ...
    def _calculate_divergence_strength_Mtm(self, current_price, previous_price, current_mtm, previous_mtm):
        """计算背离强度"""
        # 计算价格变化百分比
        price_change = price_change = abs(current_price - previous_price) / previous_price
        
        # 计算MTM变化百分比，避免除以零
        mtm_denominator = abs(previous_mtm) if previous_mtm != 0 else 1e-6  # TODO: 将魔法数字提取到配置中
        mtm_change = abs(current_mtm - previous_mtm) / mtm_denominator
        
        # 计算背离强度：价格变化和MTM变化的不一致程度
        # 背离越明显，强度越大
        return min(price_change / max(mtm_change, 1e-6), 1.0)  # TODO: 将魔法数字提取到配置中
    ...
    def generate_trading_signals_Mtm(self, data: pd.DataFrame, **kwargs) -> Dict[str, pd.Series]:
        ...
        for i in range(1, len(mtm)):
            ...
            if mtm.iloc[i-1] <= mtmma.iloc[i-1] and mtm.iloc[i] > mtmma.iloc[i]:
                ...
                signals['signal_strength'].iloc[i] = float(50 + min(angle / 90 * 40, 40))  # 50-90  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        ...
        for i in range(1, len(mtm)):
            ...
            if mtm.iloc[i-1] <= self.oversold and mtm.iloc[i] > self.oversold:
                signals['buy_signal'].iloc[i] = True
                # 信号强度基于超卖程度
                signals['signal_strength'].iloc[i] = float(60 + min((self.oversold - mtm.iloc[i-1]) / abs(self.oversold) * 30, 30))  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        ...
        for i in range(1, len(mtm)):
            ...
            if mtm.iloc[i-1] >= self.overbought and mtm.iloc[i] < self.overbought:
                signals['sell_signal'].iloc[i] = True
                # 信号强度基于超买程度
                signals['signal_strength'].iloc[i] = float(60 + min((mtm.iloc[i-1] - self.overbought) / abs(self.overbought) * 30, 30))  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        ...
        if 'MTM_GOLDEN_CROSS' in patterns_df.columns:
            ...
            for i, signal in enumerate(golden_cross_signals):
                if signal and i < len(signals['buy_signal']):
                    signals['buy_signal'].iloc[i] = True
                    if signals['signal_strength'].iloc[i] < 70:  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
                        signals['signal_strength'].iloc[i] = float(70)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
    ...
    def calculate_raw_score_Mtm(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        for i in range(len(mtm)):
            if mtm.iloc[i] < self.oversold:
                # MTM在超卖区，得分增加
                ratio = min((self.oversold - mtm.iloc[i]) / abs(self.oversold), 1.0) if self.oversold != 0 else 0.5  # TODO: 将魔法数字提取到配置中
                score.iloc[i] = 50 + ratio * 30  # 50-80  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            elif mtm.iloc[i] > self.overbought:
                # MTM在超买区，得分降低
                ratio = min((mtm.iloc[i] - self.overbought) / abs(self.overbought), 1.0) if self.overbought != 0 else 0.5  # TODO: 将魔法数字提取到配置中
                score.iloc[i] = 50 - ratio * 30  # 20-50  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            else:
                ...
                if mtm.iloc[i] > mtmma.iloc[i]:
                    ...
                    score.iloc[i] = 50 + ratio * 20  # 50-70  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
                else:
                    # MTM小于MTMMA，动量向下，评分降低
                    score.iloc[i] = 50 - ratio * 20  # 30-50  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        ...
        for i in range(1, len(mtm)):
            ...
            if mtm.iloc[i-1] <= mtmma.iloc[i-1] and mtm.iloc[i] > mtmma.iloc[i]:
                angle = self._calculate_cross_angle(mtm.iloc[i-1], mtm.iloc[i], mtmma.iloc[i-1], mtmma.iloc[i])
                adjust = min(angle / 90 * 15, 15)  # 最多增加15分  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
                score.iloc[i] = min(score.iloc[i] + adjust, 90)  # TODO: 将魔法数字提取到配置中
            
            # 死叉降低评分
            if mtm.iloc[i-1] >= mtmma.iloc[i-1] and mtm.iloc[i] < mtmma.iloc[i]:
                angle = self._calculate_cross_angle(mtm.iloc[i-1], mtm.iloc[i], mtmma.iloc[i-1], mtmma.iloc[i])
                adjust = min(angle / 90 * 15, 15)  # 最多减少15分  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        ...
        if 'MTM_STRONG_UP' in patterns_df.columns:
            ...
            score.loc[strong_up_mask] = score.loc[strong_up_mask].apply(lambda x: min(x + 10, 90))  # TODO: 将魔法数字提取到配置中
    ...
    def _get_default_parameters_mtm(self) -> Dict[str, Any]:
        """获取默认参数"""
        return {"period": 10, "ma_period": 6, "overbought": 0, "oversold": 0}  # TODO: 将魔法数字提取到配置中
    ...
    def calculate_score_mtm(self, data: pd.DataFrame, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'latest_score': float(score.iloc[-1]) if len(score) > 0 and pd.notna(score.iloc[-1]) else 50.0,  # TODO: 将魔法数字提取到配置中
                'confidence': confidence
            }

        except Exception as e:
            logger.warning(f"MTM评分计算失败: {e}")
            return {'latest_score': 50.0, 'confidence': 0.0}  # TODO: 将魔法数字提取到配置中
    ...
    def generate_signals_mtm(self, data: pd.DataFrame, **kwargs) -> Dict[str, pd.Series]:
        ...
        try:
            ...
            if 'mtm' in self._result.columns:
                ...
                signals['signal_strength'] = pd.Series([0.5] * len(self._result), index=self._result.index)  # TODO: 将魔法数字提取到配置中
            ...
            return {
                ...
                'signal_strength': pd.Series([0.5] * length, index=data.index)  # TODO: 将魔法数字提取到配置中
            }
        ...
        try:
            ...
            if 'mtm' in self._result.columns:
                ...
                if pd.notna(mtm_strength):
                    # MTM强度越大，置信度越高（但有上限）
                    confidence += min(mtm_strength * 0.01, 0.3)  # 最多增加0.3  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            ...
            if len(patterns) > 0:
                ...
                confidence += pattern_count * 0.05  # 每个强模式增加0.05  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
    ...
    def minimum_periods(self) -> int:
        ...
        计算逻辑：基于参数 period(10) + ma_period(6) 计算  # TODO: 将魔法数字提取到配置中

        Returns:
            int: 最少需要的数据周期数
        """
        return self.period + self.ma_period + 5  # TODO: 将魔法数字提取到配置中
