...
class COMPOSITE(BaseIndicator):
    ...
    def _calculate_trend_score(self, data: pd.DataFrame) -> pd.Series:
        ...
        # 均线排列 - 增加权重
        ma_alignment_bull = ((ma5 > ma10) & (ma10 > ma20)).astype(int) * 25
        ma_alignment_bear = ((ma5 < ma10) & (ma10 < ma20)).astype(int) * (-25)
        ...
        # 价格动量
        price_momentum = close.pct_change(periods=5).fillna(0) * 1000  # 5日动量

        # 趋势评分 - 更敏感的计算
        trend_score = 50 + trend_strength * 0.8 + ma_alignment + price_position * 0.6 + price_momentum * 0.2
