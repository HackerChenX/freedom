...
from db.sql_manager import SQLManager, QueryType
    ...
    def calculate_raw_score_Obv(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        # 1. OBV趋势评分 (35%)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        # 基于OBV的变化趋势
        ...
        # 连续上升
        trend_score np.where((obv_change_1 > 0) & (obv_change_2 > 0), 15, trend_score)  # TODO: 将魔法数字提取到配置中
        # 连续下降
        trend_score np.where((obv_change_1 < 0) & (obv_change_2 < 0), -15, trend_score)  # TODO: 将魔法数字提取到配置中
        # 单次上升
        trend_score np.where((obv_change_1 > 0) & (obv_change_2 <= 0), 8, trend_score)  # TODO: 将魔法数字提取到配置中
        # 单次下降
        trend_score np.where((obv_change_1 < 0) & (obv_change_2 >= 0), -8, trend_score)  # TODO: 将魔法数字提取到配置中
        
        # OBV与移动平均线的关系
        if len(obv_ma.dropna()) > 0:
            trend_score += np.where(obv > obv_ma, 3, -3)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        
        scores += trend_score * 0.35  # TODO: 将魔法数字提取到配置中
        
        # 2. 量价关系评分 (30%)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        # 基于OBV与价格变化的一致性
        ...
        scores += volume_price_score * 0.3  # TODO: 将魔法数字提取到配置中
        
        # 3. OBV突破评分 (25%)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        # 基于OBV突破重要阻力或支撑
        ...
        if len(obv_ma.dropna()) > 0:
            ...
            breakout_score np.where(obv_breakout_up, 12, breakout_score)  # TODO: 将魔法数字提取到配置中
            
            # 跌破移动平均线
            obv_breakdown (obv < obv_ma) & (obv.shift(1) >= obv_ma.shift(1))
            breakout_score np.where(obv_breakdown, -12, breakout_score)  # TODO: 将魔法数字提取到配置中
            
            # 在移动平均线之上持续
            obv_above_sustained (obv > obv_ma) & (obv.shift(1) > obv_ma.shift(1))
            breakout_score np.where(obv_above_sustained, 5, breakout_score)  # TODO: 将魔法数字提取到配置中
            
            # 在移动平均线之下持续
            obv_below_sustained (obv < obv_ma) & (obv.shift(1) < obv_ma.shift(1))
            breakout_score np.where(obv_below_sustained, -5, breakout_score)  # TODO: 将魔法数字提取到配置中
        
        # OBV创新高/新低
        obv_high obv.rolling(window=20).max()  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        obv_low obv.rolling(window=20).min()  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        
        obv_new_high obv >= obv_high
        obv_new_low obv <= obv_low
        
        breakout_score += np.where(obv_new_high, 8, 0)  # TODO: 将魔法数字提取到配置中
        breakout_score += np.where(obv_new_low, -8, 0)  # TODO: 将魔法数字提取到配置中
        
        scores += breakout_score * 0.25  # TODO: 将魔法数字提取到配置中
        
        # 4. OBV强度评分 (10%)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        # 基于OBV的相对强度
        strength_score pd.Series(0.0, index=data.index)
        
        if len(obv_strength.dropna()) > 0:
            # 强势OBV
            strength_score np.where(obv_strength > 1.5, 8, strength_score)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            strength_score np.where((obv_strength > 0.5) & (obv_strength <= 1.5), 4, strength_score)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            strength_score np.where((obv_strength >= -0.5) & (obv_strength <= 0.5), 0, strength_score)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            strength_score np.where((obv_strength >= -1.5) & (obv_strength < -0.5), -4, strength_score)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            strength_score np.where(obv_strength < -1.5, -8, strength_score)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
    ...
    def calculate_confidence_Obv(self, score: pd.Series, patterns: pd.DataFrame, signals: dict) -> float:
        ...
        recent_volatility obv_volatility.iloc[-1] if len(obv_volatility) > 0 else 5  # TODO: 将魔法数字提取到配置中
        ...
        if len(obv) >= 2:
            ...
            if recent_change > avg_change * 1.5:  # TODO: 将魔法数字提取到配置中
                obv_magnitude 0.2
        ...
        if 2 <= recent_volatility <= 8:  # TODO: 将魔法数字提取到配置中
            volatility_appropriateness 0.15  # TODO: 将魔法数字提取到配置中
        elif 1 <= recent_volatility <= 12:  # TODO: 将魔法数字提取到配置中
            volatility_appropriateness 0.1
        
        base_confidence 0.3 + trend_consistency + obv_magnitude + volatility_appropriateness  # TODO: 将魔法数字提取到配置中
        ...
        try:
            ...
            if 'obv_strength' in self._result.columns:
                ...
                if pd.notna(strength):
                    confidence += min(strength * 0.1, 0.3)  # 最多增加0.3  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
