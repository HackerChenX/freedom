...
class MacdMacd(BaseIndicator, PatternSignalMixin, MinimumPeriodsMixin):
    ...
    def __init__(
        ...
        fast_period: int = 12,
        slow_period: int = 26,
        signal_period: int = 9,  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
    ...
    def minimum_periods(self) -> int:
        ...
        Returns:
            int: 最少需要的数据周期数
        """
        slow_period = self._parameters.get("slow_period", 26)  # TODO: 将魔法数字提取到配置中
        signal_period = self._parameters.get("signal_period", 9)  # TODO: 将魔法数字提取到配置中

        # 慢线需要的最小周期 + 信号线周期 + 额外缓冲
        return slow_period + signal_period + 5  # 默认情况下是40个周期  # TODO: 将魔法数字提取到配置中
    ...
    def _register_macd_patterns(self):
        ...
        self.register_pattern_to_registry(
            ...
            pattern_type="BULLISH",
            default_strength="STRONG",
            score_impact=22.0,  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            polarity="POSITIVE",
        )
    ...
    def _calculate_macd(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        if not isinstance(macd_histogram, pd.Series):
            ...
            if len(macd_histogram) != len(data.index):
                # 如果长度不匹配，用NaN填充或截断
                if len(macd_histogram) < len(data.index):
                    macd_histogram = np.concatenate(
                        [np.full(len(data.index) - len(macd_histogram), np.nan), macd_histogram]
                    )
                else:
    ...
    def get_patterns_Macd(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        # 1. 金叉和死叉
        patterns_df["GOLDEN_CROSS"] = self._detect_robust_crossover_Macd(
            dif, dea, window=3, cross_type="above"
        )  # TODO: 将魔法数字提取到配置中
        patterns_df["DEATH_CROSS"] = self._detect_robust_crossover_Macd(
            dif, dea, window=3, cross_type="below"
        )  # TODO: 将魔法数字提取到配置中
        ...
        # 3. 背离检测  # TODO: 将魔法数字提取到配置中
        bullish_divergence, bearish_divergence = self._detect_divergence_Macd(
            data["close"], macd_df["macd_line"], window=self._parameters["divergence_window"]
        )
        ...
        # 检测连续3个柱状图扩张或收缩
        for i in range(3, len(histogram)):  # TODO: 将魔法数字提取到配置中
            # 扩张: 连续3个柱状图变大
            if (
                histogram.iloc[i] > histogram.iloc[i - 1] > histogram.iloc[i - 2] > histogram.iloc[i - 3]
            ) and histogram.iloc[
                i
            ] > 0:  # TODO: 将魔法数字提取到配置中
    ...
    def calculate_raw_score_Macd(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        # 定义各形态的得分权重
        pattern_scores = {
            "GOLDEN_CROSS": 20.0,  # TODO: 将魔法数字提取到配置中
            "DEATH_CROSS": -20.0,  # TODO: 将魔法数字提取到配置中
            "MACD_BULLISH_DIVERGENCE": 25.0,  # TODO: 将魔法数字提取到配置中
            "MACD_BEARISH_DIVERGENCE": -25.0,  # TODO: 将魔法数字提取到配置中
            "MACD_ZERO_CROSS_ABOVE": 15.0,  # TODO: 将魔法数字提取到配置中
            "MACD_ZERO_CROSS_BELOW": -15.0,  # TODO: 将魔法数字提取到配置中
            "MACD_HISTOGRAM_EXPANDING": 10.0,
            "MACD_HISTOGRAM_CONTRACTING": -10.0,
            "MACD_DOUBLE_BOTTOM": 22.0,  # TODO: 将魔法数字提取到配置中
            "MACD_DOUBLE_TOP": -22.0,  # TODO: 将魔法数字提取到配置中
        }
        ...
        if "macd_line" in macd_df.columns and "macd_signal" in macd_df.columns:
            # 当MACD线在信号线上方，给予正分
            macd_above_signal = (macd_df["macd_line"] > macd_df["macd_signal"]).astype(
                int
            ) * 5.0  # TODO: 将魔法数字提取到配置中
            # 当MACD线在信号线下方，给予负分
            macd_below_signal = (macd_df["macd_line"] < macd_df["macd_signal"]).astype(
                int
            ) * -5.0  # TODO: 将魔法数字提取到配置中
            # 累加得分
        ...
        if "macd_line" in macd_df.columns:
            # 当MACD线在零轴上方，给予正分
            macd_above_zero = (macd_df["macd_line"] > 0).astype(int) * 5.0  # TODO: 将魔法数字提取到配置中
            # 当MACD线在零轴下方，给予负分
            macd_below_zero = (macd_df["macd_line"] < 0).astype(int) * -5.0  # TODO: 将魔法数字提取到配置中
            # 累加得分
            total_score += macd_above_zero + macd_below_zero

        return total_score

    def _detect_robust_crossover_Macd(
        self, series1: pd.Series, series2: Union[pd.Series, float, int], window: int = 3, cross_type: str = "above"
        ...
        # 数据不足以计算交叉
        if len(series1) < 3:  # TODO: 将魔法数字提取到配置中
            return result
    ...
    def _detect_divergence_Macd(
        self, price: pd.Series, indicator: pd.Series, window: int = 14
    ) -> Tuple[pd.Series, pd.Series]:  # TODO: 将魔法数字提取到配置中
        """
        检测价格与指标之间的背离
        ...
        if len(price_peaks) < 2 or len(indicator_peaks) < 2:
            ...
            price_peaks, _ = find_peaks(
                price.values, distance=window // 3, prominence=0.005
            )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            indicator_peaks, _ = find_peaks(
                indicator.values, distance=window // 3, prominence=0.005
            )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        ...
        if len(price_troughs) < 2 or len(indicator_troughs) < 2:
            ...
            indicator_troughs, _ = find_peaks(
                -indicator.values, distance=window // 3, prominence=0.005
            )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        ...
        if len(price_peaks) >= 2 and len(indicator_peaks) >= 2:
            for i in range(1, len(price_peaks)):
                ...
                ind_peaks_between = [
                    idx
                    for idx in indicator_peaks
                    if max(0, p_peak1_idx - window // 2) <= idx <= min(len(indicator) - 1, p_peak2_idx + window // 2)
                ]
                ...
                if len(ind_peaks_between) >= 2:
                    ...
                    if (
                        price.iloc[p_peak2_idx] > price.iloc[p_peak1_idx]
                        and indicator.iloc[ind_peak2_idx] < indicator.iloc[ind_peak1_idx]
                    ):
        ...
        if len(price_troughs) >= 2 and len(indicator_troughs) >= 2:
            for i in range(1, len(price_troughs)):
                ...
                ind_troughs_between = [
                    idx
                    for idx in indicator_troughs
                    if max(0, p_trough1_idx - window // 2)
                    <= idx
                    <= min(len(indicator) - 1, p_trough2_idx + window // 2)
                ]
                ...
                if len(ind_troughs_between) >= 2:
                    ...
                    if (
                        price.iloc[p_trough2_idx] < price.iloc[p_trough1_idx]
                        and indicator.iloc[ind_trough2_idx] > indicator.iloc[ind_trough1_idx]
                    ):
    ...
    def get_signals_Macd(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        for idx in data.index:
            ...
            if triggered_patterns:
                signals["signals_details"].append({"date": idx, "patterns": triggered_patterns})
    ...
    def calculate_confidence_Macd(self, score: pd.Series, patterns: list, signals: dict) -> float:
        ...
        # 3. 信号越强（例如，金叉后价格确实上涨），置信度越高  # TODO: 将魔法数字提取到配置中

        # 归一化得分
        normalized_score = min(last_score / 50, 1.0)  # 假设50是高分  # TODO: 将魔法数字提取到配置中

        # 归一化形态数量
        normalized_patterns = min(num_patterns / 5, 1.0)  # 假设5个形态是很多了  # TODO: 将魔法数字提取到配置中

        # 综合置信度
        confidence = (normalized_score * 0.6) + (
            normalized_patterns * 0.4
        )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
    ...
    def get_pattern_info_Macd(self, pattern_id: str) -> dict:
        ...
        return pattern_info_map.get(
            pattern_id,
            {"name": pattern_id, "description": f"MACD形态: {pattern_id}", "strength": "medium", "type": "neutral"},
        )
    ...
    def _get_default_parameters(self) -> Dict[str, Any]:
        """获取默认参数"""
        return {
            "fast_period": 12,  # TODO: 将魔法数字提取到配置中
            "slow_period": 26,  # TODO: 将魔法数字提取到配置中
    ...
    def calculate_dict(self, data: pd.DataFrame, **kwargs) -> Dict[str, pd.Series]:
        ...
        if isinstance(result_df, pd.DataFrame):
            ...
            return {"macd_line": empty_series, "macd_signal": empty_series, "macd_histogram": empty_series}
    ...
    def calculate_raw_score_Indicator_Base_Indicator(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        if result is None:
            return pd.Series(
                [50.0] * len(data), index=data.index
        ...
        if isinstance(result, dict):
            ...
            macd_line = (
                result.get("macd_line", pd.Series([0] * len(data)))
                if "macd_line" in result.columns
                else pd.Series([0] * len(data))
            )
            macd_signal = (
                result.get("macd_signal", pd.Series([0] * len(data)))
        ...
        # MACD线在信号线上方为正面信号
        score = pd.Series([50.0] * len(data), index=data.index)  # TODO: 将魔法数字提取到配置中
        ...
        if len(macd_line) == len(data) and len(macd_signal) == len(data):
            ...
            score[bullish_mask] = 70.0  # 看涨信号  # TODO: 将魔法数字提取到配置中
            score[bearish_mask] = 30.0  # 看跌信号  # TODO: 将魔法数字提取到配置中
    ...
    def get_patterns_Indicator_Base_Indicator(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        if isinstance(result, dict):
            ...
            macd_histogram = (
                result.get("macd_histogram", pd.Series([0] * len(data)))
        ...
        dynamic_strength_threshold = (
            0.0005 + macd_volatility * 0.5
        )  # 更敏感的动态阈值  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        dynamic_change_threshold = 0.0002 + signal_volatility * 0.3  # TODO: 将魔法数字提取到配置中
        ...
        enhancement_validation = (
            ...
            | (
                macd_line > macd_line.shift(3)
            )  # 3期内上升  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        )

        # 增强噪声过滤层（更严格的真实数据标准）
        noise_filter = (
            (
                abs(macd_line) > dynamic_strength_threshold * 4
            )  # 更严格的MACD线强度要求  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            & (
                abs(macd_signal) > dynamic_strength_threshold * 3
            )  # 更严格的信号线强度要求  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            & (
                abs(macd_histogram) > dynamic_change_threshold * 5
            )  # 更严格的柱状图变化要求  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        )

        # 增强市场环境过滤（避免在极小波动中产生信号）
        market_filter = (
            (
                macd_line.rolling(window=5).std() > dynamic_change_threshold * 2
            )  # 更严格的波动性要求  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        ...
        enhancement_validation_death = (
            ...
            | (macd_line < macd_line.shift(3))  # 3期内下降  # TODO: 将魔法数字提取到配置中
        )
        ...
        # 识别MACD零轴上金叉 (平衡敏感性算法)
        # 零轴阈值（宽松）
        zero_axis_threshold = 0.0005  # 更宽松的零轴判断  # TODO: 将魔法数字提取到配置中
        ...
        zero_axis_noise_filter = (
            (
                abs(macd_line) > dynamic_strength_threshold * 6
            )  # 零轴上方需要最强的MACD信号  # TODO: 将魔法数字提取到配置中
            & (abs(macd_histogram) > dynamic_change_threshold * 8)  # 最强的柱状图变化  # TODO: 将魔法数字提取到配置中
            & (macd_line > dynamic_strength_threshold * 4)  # 明确在零轴上方  # TODO: 将魔法数字提取到配置中
            & (
                macd_line.rolling(window=5).std() > dynamic_change_threshold * 3
            )  # 足够的波动性  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        )

        # 最终验证：基于金叉 + 零轴条件 + 特殊噪声过滤
        above_zero_golden = (
            validated_golden_cross
        ...
        basic_divergence_condition = (
            (macd_histogram.shift(2) > 0.0005)  # OR逻辑：之前柱状图为正  # TODO: 将魔法数字提取到配置中
        ...
        divergence_noise_filter = (
            (
                abs(macd_histogram.shift(2) - macd_histogram) > dynamic_strength_threshold * 8
            )  # 背离需要极明显的柱状图变化  # TODO: 将魔法数字提取到配置中
            & (abs(macd_line) > dynamic_strength_threshold * 4)  # MACD线有很强强度  # TODO: 将魔法数字提取到配置中
            & (
                macd_line.rolling(window=10).std() > dynamic_change_threshold * 4
            )  # 有很强的历史波动性  # TODO: 将魔法数字提取到配置中
            & (
                abs(macd_line.shift(5) - macd_line) > dynamic_strength_threshold * 3
            )  # 5期内有明显变化  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        )
    ...
    def set_parameters_Indicator_Base_Indicator(self, **kwargs):
        ...
        if any(key in kwargs for key in ["fast_period", "slow_period", "signal_period"]):
            # 重新设置内部参数
            self._fast_period = self._parameters.get("fast_period", 12)  # TODO: 将魔法数字提取到配置中
            self._slow_period = self._parameters.get(
                "slow_period", 26
            )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            self._signal_period = self._parameters.get(
                "signal_period", 9
            )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
