...
class ChaikinVolatility(BaseIndicator, PatternSignalMixin, MinimumPeriodsMixin):
    ...
    3. Chaikin_Volatility = (EMA_HL - EMA_HL[lookback]) / EMA_HL[lookback] * 100
    ...
    def _generate_signals(self, df: pd.DataFrame) -> pd.Series:
        ...
        # 波动率突然增加信号（可能预示趋势变化）
        high_volatility = cv > cv.rolling(20).quantile(0.8)
        signals[high_volatility] = 1
        
        # 波动率突然减少信号（可能预示盘整）
        low_volatility = cv < cv.rolling(20).quantile(0.2)
    ...
    def _calculate_volatility_level(self, cv: pd.Series) -> pd.Series:
        ...
        # 使用滚动分位数定义等级
        rolling_window = min(50, len(cv))
        if rolling_window > 10:
            high_threshold = cv.rolling(rolling_window).quantile(0.75)
            low_threshold = cv.rolling(rolling_window).quantile(0.25)
    ...
    def get_signal(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if latest_signal == 1:
            return {
                'signal': 1,
                'strength': min(abs(latest_cv) / 50, 1.0),  # 标准化强度
                'description': f'波动率增加信号，当前等级：{latest_level}'
            }
        elif latest_signal == -1:
            return {
                'signal': -1,
                'strength': min(abs(latest_cv) / 50, 1.0),
            ...
            return {
                'signal': 0,
                'strength': 0,
                'description': f'波动率正常，当前等级：{latest_level}'
            }
    ...
    def calculate_raw_score_Indicator_Base_Indicator(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        cv_abs = cv.abs()
        cv_normalized = cv_abs / (cv_abs.rolling(20).mean() + 1e-8)
        score = np.clip(cv_normalized * 30 + 50, 0, 100)
        return score
    ...
    def get_patterns_Indicator_Base_Indicator(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        if 'chaikin_volatility' in data.columns:
            cv = data['chaikin_volatility']
            # 波动率突破形态
            patterns['CV_波动率突增'] = cv > cv.rolling(20).quantile(0.8)
            patterns['CV_波动率骤减'] = cv < cv.rolling(20).quantile(0.2)
            # 波动率趋势形态
    ...
    def calculate_confidence_Indicator_Base_Indicator(self, score: pd.Series, patterns: List[str], signals: Dict[str, pd.Series]) -> float:
        ...
        pattern_strength = min(len(patterns) * 0.25, 1.0)
    ...
    def minimum_periods(self) -> int:
        """最小周期数"""
        return self.period + self.lookback + 5
