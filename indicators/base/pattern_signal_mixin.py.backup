...
class PatternSignalMixin:
    ...
    def _add_rsi_patterns(self, result: pd.DataFrame) -> pd.DataFrame:
        ...
        # RSI超买超卖形态
        result['pattern_bullish'] = result[rsi_col] < 30  # 超卖，看涨形态
        result['pattern_bearish'] = result[rsi_col] > 70  # 超买，看跌形态
        result['pattern_neutral'] = (result[rsi_col] >= 30) & (result[rsi_col] <= 70)
    ...
    def _add_rsi_signals(self, result: pd.DataFrame) -> pd.DataFrame:
        ...
        # RSI买卖信号
        result['buy_signal'] = (result[rsi_col] < 30) & (result[rsi_col].shift(1) >= 30)  # 从超卖区域向上突破
        result['sell_signal'] = (result[rsi_col] > 70) & (result[rsi_col].shift(1) <= 70)  # 从超买区域向下突破
    ...
    def _add_macd_patterns(self, result: pd.DataFrame) -> pd.DataFrame:
        ...
        if dif_col and dea_col:
            # MACD金叉死叉形态
            result['pattern_bullish'] = (result[dif_col] > result[dea_col]) & (result[dif_col].shift(1) <= result[dea_col].shift(1))  # 金叉
            result['pattern_bearish'] = (result[dif_col] < result[dea_col]) & (result[dif_col].shift(1) >= result[dea_col].shift(1))  # 死叉
    ...
    def _add_macd_signals(self, result: pd.DataFrame) -> pd.DataFrame:
        ...
        if dif_col and dea_col:
            # MACD买卖信号
            result['buy_signal'] = (result[dif_col] > result[dea_col]) & (result[dif_col].shift(1) <= result[dea_col].shift(1))  # 金叉买入
            result['sell_signal'] = (result[dif_col] < result[dea_col]) & (result[dif_col].shift(1) >= result[dea_col].shift(1))  # 死叉卖出
            result['hold_signal'] = ~(result['buy_signal'] | result['sell_signal'])
    ...
    def _add_kdj_patterns(self, result: pd.DataFrame) -> pd.DataFrame:
        ...
        if k_col and d_col:
            # KDJ金叉死叉形态
            result['pattern_bullish'] = (result[k_col] > result[d_col]) & (result[k_col].shift(1) <= result[d_col].shift(1))  # 金叉
            result['pattern_bearish'] = (result[k_col] < result[d_col]) & (result[k_col].shift(1) >= result[d_col].shift(1))  # 死叉
            result['pattern_neutral'] = ~(result['pattern_bullish'] | result['pattern_bearish'])
    ...
    def _add_kdj_signals(self, result: pd.DataFrame) -> pd.DataFrame:
        ...
        if k_col and d_col:
            # KDJ买卖信号
            result['buy_signal'] = (result[k_col] > result[d_col]) & (result[k_col].shift(1) <= result[d_col].shift(1)) & (result[k_col] < 20)  # 低位金叉买入
            result['sell_signal'] = (result[k_col] < result[d_col]) & (result[k_col].shift(1) >= result[d_col].shift(1)) & (result[k_col] > 80)  # 高位死叉卖出
            result['hold_signal'] = ~(result['buy_signal'] | result['sell_signal'])
    ...
    def _add_boll_signals(self, result: pd.DataFrame) -> pd.DataFrame:
        ...
        if upper_col and lower_col:
            ...
            if hasattr(self, 'data') and 'close' in self.data.columns:
                close_price = self.data['close']
                # BOLL买卖信号
                result['buy_signal'] = (close_price < result[lower_col]) & (close_price.shift(1) >= result[lower_col].shift(1))  # 跌破下轨后回升
                result['sell_signal'] = (close_price > result[upper_col]) & (close_price.shift(1) <= result[upper_col].shift(1))  # 突破上轨后回落
                result['hold_signal'] = ~(result['buy_signal'] | result['sell_signal'])
