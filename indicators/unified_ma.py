...
from db.sql_manager import SQLManager, QueryType
    ...
    def calculate_raw_score_Ma_Unified_Ma(self, data: pd.DataFrame, **kwargs) -> pd.Series:
        ...
        # 计算多个周期的移动平均线
        ma5 = df['close'].rolling(window=5).mean()  # TODO: 将魔法数字提取到配置中
        ma10 = df['close'].rolling(window=10).mean()
        ma20 = df['close'].rolling(window=20).mean()  # TODO: 将魔法数字提取到配置中
        ma30 = df['close'].rolling(window=30).mean()  # TODO: 将魔法数字提取到配置中
        ma60 = df['close'].rolling(window=60).mean()  # TODO: 将魔法数字提取到配置中
        
        # 指数移动平均线
        ema12 = df['close'].ewm(span=12).mean()  # TODO: 将魔法数字提取到配置中
        ema26 = df['close'].ewm(span=26).mean()  # TODO: 将魔法数字提取到配置中
        
        # 复合评分计算
        scores = pd.Series(50.0, index=data.index)  # 基准分  # TODO: 将魔法数字提取到配置中
        
        # 短期MA排列 (30%)  # TODO: 将魔法数字提取到配置中
        short_bullish = (df['close'] > ma5) & (ma5 > ma10) & (ma10 > ma20)
        short_bearish = (df['close'] < ma5) & (ma5 < ma10) & (ma10 < ma20)
        scores += np.where(short_bullish, 20, 0)  # TODO: 将魔法数字提取到配置中
        scores += np.where(short_bearish, -15, 0)  # TODO: 将魔法数字提取到配置中
        
        # 中长期MA排列 (25%)  # TODO: 将魔法数字提取到配置中
        long_bullish = (ma20 > ma30) & (ma30 > ma60)
        long_bearish = (ma20 < ma30) & (ma30 < ma60)
        scores += np.where(long_bullish, 15, 0)  # TODO: 将魔法数字提取到配置中
        scores += np.where(long_bearish, -15, 0)  # TODO: 将魔法数字提取到配置中
        
        # MA金叉死叉 (25%)  # TODO: 将魔法数字提取到配置中
        ...
        scores += np.where(ma5_cross_ma10, 15, 0)  # TODO: 将魔法数字提取到配置中
        scores += np.where(ma5_death_ma10, -10, 0)
        scores += np.where(ma10_cross_ma20, 12, 0)  # TODO: 将魔法数字提取到配置中
        scores += np.where(ma10_death_ma20, -12, 0)  # TODO: 将魔法数字提取到配置中
        
        # EMA信号 (15%)  # TODO: 将魔法数字提取到配置中
        ema_bullish = ema12 > ema26
        ema_cross = (ema12 > ema26) & (ema12.shift(1) <= ema26.shift(1))
        scores += np.where(ema_bullish, 8, -5)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        scores += np.where(ema_cross, 10, 0)
        
        # 价格与MA关系 (5%)  # TODO: 将魔法数字提取到配置中
        above_all_ma = (df['close'] > ma5) & (df['close'] > ma10) & (df['close'] > ma20)
        below_all_ma = (df['close'] < ma5) & (df['close'] < ma10) & (df['close'] < ma20)
        scores += np.where(above_all_ma, 8, 0)  # TODO: 将魔法数字提取到配置中
        scores += np.where(below_all_ma, -8, 0)  # TODO: 将魔法数字提取到配置中
        
        # MA斜率分析
        ma20_slope = (ma20 - ma20.shift(5)) / ma20.shift(5)  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        ...
        expanding = ma_spread > ma_spread.shift(5)  # 发散  # TODO: 将魔法数字提取到配置中
        contracting = ma_spread < ma_spread.shift(5)  # 收敛  # TODO: 将魔法数字提取到配置中
        scores += np.where(expanding & (ma_spread > 0), 5, 0)  # 上升发散  # TODO: 将魔法数字提取到配置中
        scores += np.where(contracting & (ma_spread < 0), 5, 0)  # 下跌收敛  # TODO: 将魔法数字提取到配置中
