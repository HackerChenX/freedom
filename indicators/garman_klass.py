...
class GarmanKlass(BaseIndicator, PatternSignalMixin, MinimumPeriodsMixin):
    ...
    def __init__(self, period: int = 20, annualize: bool = True, **kwargs):  # TODO: 将魔法数字提取到配置中
        # 依赖注入示例:
    ...
    def _get_default_parameters(self) -> Dict[str, Any]:
        """获取默认参数"""
        return {"period": 20, "annualize": True}  # TODO: 将魔法数字提取到配置中
    ...
    def _calculate_percentile(self, volatility: pd.Series) -> pd.Series:
        ...
        Returns:
            pd.Series: 分位数序列
        """
        rolling_window = min(252, len(volatility))  # 最多使用一年的数据  # TODO: 将魔法数字提取到配置中
        if rolling_window < 20:  # TODO: 将魔法数字提取到配置中
            return pd.Series(50, index=volatility.index)  # 默认中位数  # TODO: 将魔法数字提取到配置中

        percentiles = volatility.rolling(rolling_window).apply(
            lambda x: (x.iloc[-1] <= x).mean() * 100 if len(x) > 0 else 50  # TODO: 将魔法数字提取到配置中
        )

        return percentiles.fillna(50)  # TODO: 将魔法数字提取到配置中
    ...
    def _classify_volatility_regime(self, percentiles: pd.Series) -> pd.Series:
        ...
        regimes[percentiles >= 80] = "极高"  # TODO: 将魔法数字提取到配置中
        regimes[(percentiles >= 60) & (percentiles < 80)] = (
            "高"  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        )
        regimes[(percentiles >= 40) & (percentiles < 60)] = (
            "中等"  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        )
        regimes[(percentiles >= 20) & (percentiles < 40)] = (
            "低"  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
        )
        regimes[percentiles < 20] = "极低"  # TODO: 将魔法数字提取到配置中
    ...
    def get_signal(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        latest_volatility = (
            data["gk_volatility_annualized"].iloc[-1] if "gk_volatility_annualized" in data.columns else 0
        )
        latest_regime = data["volatility_regime"].iloc[-1] if "volatility_regime" in data.columns else "未知"
        latest_percentile = (
            data["volatility_percentile"].iloc[-1] if "volatility_percentile" in data.columns else 50
        )  # TODO: 将魔法数字提取到配置中
    ...
    def get_pattern_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "type": "volatility",
            "parameters": {"period": self.period, "annualize": self.annualize},
        }
    ...
    def get_patterns_Indicator_Base_Indicator(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        if "volatility_percentile" in data.columns:
            ...
            # 波动率状态形态
            patterns["GK_极高波动"] = percentiles >= 80  # TODO: 将魔法数字提取到配置中
            patterns["GK_高波动"] = (percentiles >= 60) & (
                percentiles < 80
            )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            patterns["GK_低波动"] = (percentiles >= 20) & (
                percentiles < 40
            )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            patterns["GK_极低波动"] = percentiles < 20  # TODO: 将魔法数字提取到配置中
            # 波动率转换形态
            patterns["GK_波动率突破"] = (percentiles > 70) & (
                percentiles.shift(1) < 30
            )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
            patterns["GK_波动率回落"] = (percentiles < 30) & (
                percentiles.shift(1) > 70
            )  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
    ...
    def calculate_confidence_Indicator_Base_Indicator(
        ...
        # 基于波动率分位数的稳定性计算置信度
        score_range = (
            score.rolling(10).max().iloc[-1] - score.rolling(10).min().iloc[-1] if len(score) >= 10 else 50
        )  # TODO: 将魔法数字提取到配置中
        stability = 1.0 - (score_range / 100.0)
        pattern_strength = min(len(patterns) * 0.15, 1.0)  # TODO: 将魔法数字提取到配置中
    ...
    def minimum_periods(self) -> int:
        ...
        return max(
            self.period + 20, 50
        )  # 需要足够的历史数据计算分位数  # TODO: 将魔法数字提取到配置中  # TODO: 将魔法数字提取到配置中
