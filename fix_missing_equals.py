...
def find_missing_equals_patterns(content: str) -> list:
    ...
    for i, line in enumerate(lines, 1):
        ...
        # 模式1: variable_name function_call() (缺少等号)
        ...
        # 模式2: variable_name expression (缺少等号)
        pattern2 = re.match(r'^(\s*)([a-zA-Z_][a-zA-Z0-9_]*)\s+([^=\s].*[^=\s])\s*$', line)
        if pattern2:
            indent, var_name, expression = pattern2.groups()
            # 排除特殊情况
            if (not any(keyword in line for keyword in ['class ', 'def ', 'import ', 'from ', 'if ', 'elif ', 'else', 'try:', 'except', 'finally', 'with ', 'for ', 'while ']) 
        ...
        # 模式3: return "string" 应该是 return variable
...
def main():
    ...
    parser = argparse.ArgumentParser(description='批量修复缺少等号的语法错误')
