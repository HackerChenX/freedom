...
"""
诊断ZXM_LIQUIDITY_ANALYSIS指标失败原因
"""
...
def diagnose_zxm_liquidity_analysis():
    """诊断ZXM_LIQUIDITY_ANALYSIS指标问题"""
    logger.info("🔍 开始诊断ZXM_LIQUIDITY_ANALYSIS指标...")
    
    try:
        # 尝试导入指标类
        logger.info("📦 尝试导入ZXM_LIQUIDITY_ANALYSIS指标...")
        
        from indicators.zxm.zxm_liquidity_analysis import ZXMLiquidityAnalysis
        
        indicator = ZXMLiquidityAnalysis()
        logger.info(f"✅ 成功导入: ZXMLiquidityAnalysis")
...
def main():
    """主函数"""
    success = diagnose_zxm_liquidity_analysis()
    
    if success:
        logger.info("✅ ZXM_LIQUIDITY_ANALYSIS诊断完成")
    else:
        logger.error("❌ ZXM_LIQUIDITY_ANALYSIS诊断失败")
