...
def diagnose_zxm_market_sentiment():
    """诊断ZXM_MARKET_SENTIMENT指标问题"""
    logger.info("🔍 开始诊断ZXM_MARKET_SENTIMENT指标...")
    
    try:
        # 尝试导入指标类
        logger.info("📦 尝试导入ZXM_MARKET_SENTIMENT指标...")
        
        # 检查可能的导入路径
        import_attempts = [
            ("indicators.sentiment_analysis", "MarketSentiment"),
            ("indicators.zxm.market_breadth", "ZxmmarketBreadth"),
            ("indicators.zxm.zxm_market_sentiment", "ZXMMarketSentiment"),
        ]
        ...
        for module_path, class_name in import_attempts:
            try:
                ...
                successful_import = f"{module_path}.{class_name}"
                logger.info(f"✅ 成功导入: {successful_import}")
                break
            except Exception as e:
                logger.warning(f"⚠️ 导入失败 {module_path}.{class_name}: {e}")
        
        if indicator is None:
            logger.error("❌ 无法导入任何ZXM_MARKET_SENTIMENT相关类")
            return False
...
def main():
    """主函数"""
    success = diagnose_zxm_market_sentiment()
    
    if success:
        logger.info("✅ ZXM_MARKET_SENTIMENT诊断完成")
    else:
        logger.error("❌ ZXM_MARKET_SENTIMENT诊断失败")
