...
class OptimizedZXMCalculator:
    ...
    def calculate_optimized_zxm(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 1. 计算LLV和HHV（向量化）
        llv_55, hhv_55 = self._fast_rolling_min_max(low_vals, 55)
        ...
        # 3. 计算V11（优化的SMA）
        sma_rsv_5 = self._fast_sma(rsv_55, 5, 1)
        sma_sma_3 = self._fast_sma(sma_rsv_5, 3, 1)
        v11 = 3 * sma_rsv_5 - 2 * sma_sma_3
        
        # 4. 计算V11的EMA（使用pandas的向量化EMA）
        v11_series = pd.Series(v11, index=data.index)
        ema_v11_3 = v11_series.ewm(span=3, adjust=False).mean().values
        ...
        if len(valid_prev) > 0:
            v12[1:][valid_prev] = (
                (ema_v11_3[1:] - ema_v11_3[:-1])[valid_prev] / 
                ema_v11_3[:-1][valid_prev] * 100
            )
        
        # 6. 计算AA和BB条件（优化的布尔运算）
        aa_base = ema_v11_3 <= 13
        aa_filter = self._fast_filter_condition(aa_base, 15)
        ...
        xg = self._fast_count_condition(combined_condition, 6)
