...
def test_adx_instantiation():
    ...
    try:
        ...
        adx_indicator = manager.create_indicator('ADX')
        
        if adx_indicator is not None:
            print("✅ ADX指标实例化成功")
            print(f"   类名: {adx_indicator.__class__.__name__}")
            print(f"   指标名: {adx_indicator.name}")
            print(f"   默认周期: {adx_indicator.params.get('period', 'N/A')}")
            return True, adx_indicator
        else:
            print("❌ ADX指标实例化失败")
            return False, None
            
    except Exception as e:
        print(f"❌ ADX指标实例化异常: {e}")
        return False, None

def test_adx_calculate(adx_indicator, test_data):
    """测试ADX指标calculate()方法"""
    print("\n" + "="*60)
    print("📊 ADX指标calculate()测试")
    print("="*60)
    
    try:
        result = adx_indicator.calculate(test_data)
        
        if result is not None and not result.empty:
            print("✅ ADX指标calculate()方法成功")
            print(f"   结果行数: {len(result)}")
            print(f"   结果列数: {len(result.columns)}")
            print(f"   列名: {list(result.columns)}")
            
            # 检查关键的ADX列
            adx_columns = [col for col in result.columns if 'ADX' in col.upper() or 'PDI' in col.upper() or 'MDI' in col.upper()]
            if adx_columns:
                print(f"   ADX相关列: {adx_columns}")
                for col in adx_columns[:3]:  # 显示前3列的统计信息
                    if len(result[col]) > 0:
                        print(f"   {col}: 最新值={result[col].iloc[-1]:.4f}, 平均值={result[col].mean():.4f}")
                return True, result
            else:
                print("⚠️ 警告: 未找到ADX相关的输出列")
                return False, result
        else:
            print("❌ ADX指标calculate()返回空结果")
            return False, None
            
    except Exception as e:
        print(f"❌ ADX指标calculate()异常: {e}")
        return False, None

def test_adx_get_signal(adx_indicator, test_data):
    """测试ADX指标get_signal()方法"""
    print("\n" + "="*60)
    print("📡 ADX指标get_signal()测试")
    print("="*60)
    
    try:
        signal = adx_indicator.get_signal(test_data)
        
        if signal and isinstance(signal, dict):
            print("✅ ADX指标get_signal()方法成功")
            ...
            print("❌ ADX指标get_signal()返回格式不正确")
            return False, None
            
    except Exception as e:
        print(f"❌ ADX指标get_signal()异常: {e}")
        return False, None

def test_adx_data_validation(adx_indicator):
    """测试ADX指标数据验证"""
    print("\n" + "="*60)
    print("🔒 ADX指标数据验证测试")
    print("="*60)
...
def test_adx_performance(adx_indicator, test_data):
    """测试ADX指标性能"""
    print("\n" + "="*60)
    print("⚡ ADX指标性能测试")
    print("="*60)
    ...
    for i in range(10):
        adx_indicator.calculate(test_data)
    calculate_time = (time.time() - start_time) / 10
    
    # 测试get_signal性能
    start_time = time.time()
    for i in range(100):
        adx_indicator.get_signal(test_data)
...
def run_adx_tests():
    ...
    # 1. 实例化测试
    success, adx_indicator = test_adx_instantiation()
    if success:
        passed_tests += 1
    
    if adx_indicator is None:
        print("❌ 无法继续后续测试，ADX实例化失败")
        return
    
    # 2. calculate测试
    success, result = test_adx_calculate(adx_indicator, test_data)
    if success:
        passed_tests += 1
    
    # 3. get_signal测试
    success, signal = test_adx_get_signal(adx_indicator, test_data)
    if success:
        passed_tests += 1
    
    # 4. 数据验证测试
    success = test_adx_data_validation(adx_indicator)
    if success:
        passed_tests += 1
    
    # 5. 性能测试
    success = test_adx_performance(adx_indicator, test_data)
    if success:
        passed_tests += 1
    ...
    if passed_tests == total_tests:
        print("🎉 ADX指标全面测试通过！符合L4文档设计预期和基类设计预期")
    else:
        print("⚠️ ADX指标存在问题，需要进一步修复")
