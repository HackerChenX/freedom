...
class EnhancedCciValidator:
    """ENHANCED_CCI指标严格标准化5阶段验证器"""
    
    def __init__(self):
        """初始化验证器"""
        self.indicator_name = "ENHANCED_CCI"
        ...
        logger.info(f"🚀 开始ENHANCED_CCI指标严格标准化5阶段验证")
    ...
    def stage1_algorithm_accuracy_validation(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            # 1. 核心算法验证 (40分)
            result = indicator._calculate_enhancedcci(data)
            required_columns = ['cci', 'tp', 'tp_ma', 'mad']
            if all(col in result.columns for col in required_columns):
                algorithm_score += 40
                logger.info("✅ 核心CCI算法计算成功")
            else:
                missing = [col for col in required_columns if col not in result.columns]
                logger.error(f"❌ 核心算法计算失败，缺少列: {missing}")
            
            # 2. 手动算法验证 (35分) - CCI算法验证
            manual_cci = self._calculate_manual_cci(data, period=20, factor=0.015)
            if 'cci' in result.columns:
                cci_corr = np.corrcoef(result['cci'].dropna(), manual_cci.dropna())[0, 1]
                
                if cci_corr > 0.95:
                    algorithm_score += 35
                    logger.info(f"✅ 手动算法验证通过，相关系数: {cci_corr:.6f}")
                else:
                    logger.error(f"❌ 手动算法验证失败，相关系数: {cci_corr:.6f}")
            else:
                logger.error("❌ 缺少CCI计算结果")
            
            # 3. 数据完整性验证 (15分)
            enhanced_columns = ['state', 'cci_slope', 'volatility']
            ...
            # 4. 参数响应验证 (7分)
            try:
                indicator.set_parameters(period=21, factor=0.02)
                result2 = indicator._calculate_enhancedcci(data)
                if not result['cci'].equals(result2['cci']):
                    algorithm_score += 7
            ...
            # 5. 特征验证 (3分)
            if ('cci' in result.columns and
                abs(result['cci'].min()) > 50 and abs(result['cci'].max()) > 50):
                algorithm_score += 3
                logger.info(f"✅ CCI值范围验证通过，范围: {result['cci'].min():.2f} 到 {result['cci'].max():.2f}")
            else:
                logger.error(f"❌ CCI值范围验证失败，范围: {result['cci'].min():.2f} 到 {result['cci'].max():.2f}")
    ...
    def _calculate_manual_cci(self, data: pd.DataFrame, period: int = 20, factor: float = 0.015) -> pd.Series:
        """手动计算CCI指标"""
        # 计算典型价格 (TP)
    ...
    def stage2_basic_function_validation(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(indicator, 'get_patterns_Indicator_Base_Indicator'):
                ...
                logger.error("❌ get_patterns方法不存在")
            ...
            if hasattr(indicator, 'calculate_raw_score_Indicator_Base_Indicator'):
                try:
                    ...
                    logger.error(f"❌ calculate_raw_score方法失败: {e}")
            else:
                logger.error("❌ calculate_raw_score方法不存在")
            ...
            if hasattr(indicator, 'base_period'):
                param_score += 10
                logger.info("✅ base_period属性存在")
            ...
            try:
                ...
                result_empty = indicator._calculate_enhancedcci(empty_data)
                exception_score += 7
                logger.info("✅ 空数据处理正常")
            except:
            ...
            try:
                ...
                result_short = indicator._calculate_enhancedcci(short_data)
                exception_score += 8
            ...
            for _ in range(10):
                indicator._calculate_enhancedcci(data)
    ...
    def run_validation(self) -> Dict[str, Any]:
        """运行完整的5阶段验证"""
        logger.info("🎯 开始ENHANCED_CCI指标完整验证流程")
