...
class WRIndicator5StageValidator:
    """WR指标严格标准化5阶段验证器"""
    ...
    def __init__(self):
        ...
        self.validation_standards = {
            ...
            'pattern_recognition_threshold': 90.0,  # 形态识别阈值（短期振荡指标）
        ...
        logger.info(f"🚀 开始WR指标严格标准化5阶段验证")
    ...
    def stage1_algorithm_accuracy_validation(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'wr' in result.columns:
                actual_wr = result['wr']
            elif 'WR' in result.columns:
                actual_wr = result['WR']
            elif 'WILLIAMS_R' in result.columns:
                actual_wr = result['WILLIAMS_R']
            else:
                raise ValueError("未找到WR指标计算结果列")
            
            # 计算算法准确性
            valid_indices = expected_wr.notna() & actual_wr.notna()
            if valid_indices.sum() == 0:
                raise ValueError("没有有效的WR计算结果")
            
            # 计算相关性和误差
            correlation = np.corrcoef(expected_wr[valid_indices], actual_wr[valid_indices])[0, 1]
            mae = np.mean(np.abs(expected_wr[valid_indices] - actual_wr[valid_indices]))
            rmse = np.sqrt(np.mean((expected_wr[valid_indices] - actual_wr[valid_indices]) ** 2))
            ...
            # 数值范围验证 (25分)
            wr_in_range = ((actual_wr >= -100) & (actual_wr <= 0)).sum() / len(actual_wr.dropna())
            if wr_in_range >= 0.99:
                algorithm_score += 25
            elif wr_in_range >= 0.95:
                algorithm_score += 20
            elif wr_in_range >= 0.90:
                algorithm_score += 15
            else:
                algorithm_score += 10
            ...
            stage1_result = {
                ...
                'details': {
                    ...
                    'mae': mae,
                    'rmse': rmse,
                    'value_range_compliance': wr_in_range,
                    'valid_calculations': valid_indices.sum(),
                    'total_calculations': len(actual_wr),
            ...
            logger.info(f"✅ 阶段1完成: {algorithm_score:.1f}/100分 (相关性: {correlation:.4f}, MAE: {mae:.4f})")
    ...
    def stage2_basic_function_validation(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            # 2. 参数设置功能测试 (25分)
            try:
                indicator.set_parameters(period=20, overbought=-15, oversold=-85)
                basic_score += 25
        ...
        try:
            ...
            try:
                ...
                if 'wr' in result.columns:
                    ...
                    wr_values = result['WILLIAMS_R']
                else:
                    raise ValueError("未找到WR指标值")

                # 验证超买超卖形态识别
                oversold_count = (wr_values <= -80).sum()
                overbought_count = (wr_values >= -20).sum()
                normal_count = ((wr_values > -80) & (wr_values < -20)).sum()

                # 形态分布合理性
                total_valid = len(wr_values.dropna())
                if total_valid > 0:
                    oversold_ratio = oversold_count / total_valid
                    overbought_ratio = overbought_count / total_valid
                    normal_ratio = normal_count / total_valid

                    # 合理的形态分布应该是正常区域占主导
                    if 0.6 <= normal_ratio <= 0.9:
                        pattern_score += 20
                    elif 0.4 <= normal_ratio <= 0.95:
                        pattern_score += 15
                    else:
                        pattern_score += 10

                    # 超买超卖区域应该相对较少但存在
                    if 0.05 <= oversold_ratio <= 0.25 and 0.05 <= overbought_ratio <= 0.25:
                        pattern_score += 20
                    elif 0.02 <= oversold_ratio <= 0.35 and 0.02 <= overbought_ratio <= 0.35:
                        pattern_score += 15
                    else:
                        pattern_score += 10

                logger.info(f"✅ WR形态分布: 超卖{oversold_ratio:.1%}, 正常{normal_ratio:.1%}, 超买{overbought_ratio:.1%}")

            except Exception as e:
                logger.error(f"❌ WR特定形态验证失败: {e}")
            ...
            stage3_result = {
                ...
                'details': {
                    ...
                    'wr_pattern_distribution': {
                        'oversold_ratio': oversold_ratio if 'oversold_ratio' in locals() else 0,
                        'normal_ratio': normal_ratio if 'normal_ratio' in locals() else 0,
                        'overbought_ratio': overbought_ratio if 'overbought_ratio' in locals() else 0
                    }
                },
...
def main():
    """主函数"""
    logger.info("🚀 启动WR指标严格标准化5阶段验证")

    try:
        # 创建验证器
        validator = WRIndicator5StageValidator()
        ...
        report_file = report_dir / f"WR_validation_report.md"
        ...
        logger.error(f"❌ WR指标验证过程中发生错误: {e}")
        raise
