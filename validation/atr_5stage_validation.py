...
class ATRIndicator5StageValidator:
    """ATR指标严格标准化5阶段验证器"""
    ...
    def __init__(self):
        ...
        logger.info(f"🚀 开始ATR指标严格标准化5阶段验证")
    ...
    def stage1_algorithm_accuracy_validation(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            expected_atr = pd.Series(true_range).rolling(window=period).mean()
            
            # 获取实际计算的ATR值
            if 'atr' in result.columns:
                actual_atr = result['atr']
            elif 'ATR' in result.columns:
                actual_atr = result['ATR']
            elif 'ATR_Atr' in result.columns:
                actual_atr = result['ATR_Atr']
            else:
                raise ValueError("未找到ATR指标计算结果列")
            
            # 计算算法准确性
            valid_indices = expected_atr.notna() & actual_atr.notna()
            if valid_indices.sum() == 0:
                raise ValueError("没有有效的ATR计算结果")
            
            # 计算相关性和误差
            expected_series = expected_atr[valid_indices]
            actual_series = actual_atr[valid_indices]
            ...
            stage1_result = {
                ...
                'details': {
                    ...
                    'total_calculations': len(actual_atr),
    ...
    def stage2_basic_function_validation(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            # 2. 参数设置功能测试 (25分)
            try:
                indicator.set_parameters(period=10)
                basic_score += 25
            ...
            try:
                ...
                if result is not None and not result.empty:
                    ...
                    if 'atr' in result.columns:
                        ...
                        atr_values = result['ATR_Atr']
                    else:
                        raise ValueError("未找到ATR指标值")

                    # 验证ATR波动性分布
                    valid_atr = atr_values.dropna()
                    if len(valid_atr) > 0:
                        # 计算波动性分位数
                        q25 = valid_atr.quantile(0.25)
                        q75 = valid_atr.quantile(0.75)
                        median = valid_atr.median()

                        # 低波动期（小于25%分位数）
                        low_vol_count = (valid_atr <= q25).sum()
                        # 高波动期（大于75%分位数）
                        high_vol_count = (valid_atr >= q75).sum()
                        # 正常波动期
                        normal_vol_count = ((valid_atr > q25) & (valid_atr < q75)).sum()

                        total_valid = len(valid_atr)
                        ...
                        if 0.2 <= low_vol_ratio <= 0.3 and 0.2 <= high_vol_ratio <= 0.3:
                            pattern_score += 20
                            logger.info(f"✅ ATR波动性分布合理: 低{low_vol_ratio:.1%}, 正常{normal_vol_ratio:.1%}, 高{high_vol_ratio:.1%}")
                        else:
                            pattern_score += 15
                            logger.info(f"⚠️ ATR波动性分布一般: 低{low_vol_ratio:.1%}, 正常{normal_vol_ratio:.1%}, 高{high_vol_ratio:.1%}")

                        # 验证ATR变化的连续性
                        atr_changes = valid_atr.diff().dropna()
                        if len(atr_changes) > 0:
                            smooth_changes = (np.abs(atr_changes) < valid_atr.std()).sum()
                            smoothness_ratio = smooth_changes / len(atr_changes)

                            if smoothness_ratio >= 0.7:
                                pattern_score += 20
                                logger.info(f"✅ ATR变化平滑: {smoothness_ratio:.1%}")
                            else:
                                pattern_score += 10
                                logger.warning(f"⚠️ ATR变化较剧烈: {smoothness_ratio:.1%}")

            except Exception as e:
                logger.error(f"❌ ATR特定形态验证失败: {e}")
...
def main():
    """主函数"""
    logger.info("🚀 启动ATR指标严格标准化5阶段验证")

    try:
        # 创建验证器
        validator = ATRIndicator5StageValidator()
        ...
        report_file = report_dir / f"ATR_validation_report.md"
        ...
        logger.error(f"❌ ATR指标验证过程中发生错误: {e}")
        raise
