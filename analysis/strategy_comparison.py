...
class StrategyComparison:
    ...
    @safe_run
    def visualize_strategy_comparison(self, comparison_result: Dict[str, Any], 
        ...
        if not matplotlib_available:
            logger.error("未安装matplotlib库，无法生成可视化图表")
            return ""
            
        if not comparison_result:
            logger.error("比较结果为空，无法进行可视化")
            return ""
            
        # 如果未指定输出文件，生成默认路径
        if output_file is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_file = os.path.join(self.result_dir, f"strategy_comparison_{timestamp}.png")
        ...
        try:
            ...
            if not strategies:
                logger.warning("比较结果中没有策略信息，无法进行可视化")
                return ""
                
            strategy_names = [s.get("name", s.get("id", "未知")) for s in strategies]
            ...
            # 创建子图
            ax1 = plt.subplot2grid((2, 2), (0, 0))  # 综合评分
            ax2 = plt.subplot2grid((2, 2), (0, 1))  # 各维度雷达图
            ax3 = plt.subplot2grid((2, 2), (1, 0), colspan=2)  # 时间周期表现
            ...
            if overall_scores:
                ...
                for score_info in overall_scores:
                    s_id = score_info.get("strategy_id", "")
                    for s in strategies:
                        if s.get("id", "") == s_id:
                            names.append(s.get("name", s_id))
                            break
                    else:
                        names.append(s_id)
                    scores.append(score_info.get("overall_score", 0))
                
                # 绘制柱状图
                ax1.bar(names, scores, color='skyblue')
                ax1.set_title('策略综合评分')
                ax1.set_ylabel('评分')
                ax1.set_xlabel('策略')
                ax1.set_ylim(0, 1)
            ...
            if dimension_rankings:
                dimensions = list(dimension_rankings.keys())
                
                # 检查维度是否为空
                if not dimensions:
                    logger.warning("维度排名为空，无法绘制雷达图")
                else:
                    # 计算角度
                    ...
                    for i, s in enumerate(strategies):
                        ...
                        for dim in dimensions:
                            dim_rankings = dimension_rankings.get(dim, [])
                            for rank_info in dim_rankings:
                                if rank_info.get("strategy_id", "") == s_id:
                                    values.append(rank_info.get("score", 0))
                                    break
                            else:
                                values.append(0)
                        ...
                        # 绘制雷达图
                        ax2.plot(angles, values, linewidth=1, label=s.get("name", s_id))
                    ...
                    ax2.set_ylim(0, 1)
                    ax2.set_title('策略各维度评分')
                    ax2.legend(loc='upper right', bbox_to_anchor=(0.1, 0.1))
            
            # 绘制时间周期表现
            period_results = comparison_result.get("period_results", [])
            if period_results:
                periods = [p.get("period_name", "") for p in period_results]
                
                # 准备数据
                strategy_performance = {s.get("id", ""): [] for s in strategies}
                
                for period in period_results:
                    strategy_results = period.get("strategy_results", [])
                    for result in strategy_results:
                        s_id = result.get("strategy_id", "")
                        if s_id in strategy_performance:
                            perf = result.get("performance", {})
                            weighted_return = perf.get("weighted_return", 0) if perf else 0
                            strategy_performance[s_id].append(weighted_return)
                
                # 绘制折线图
                for s in strategies:
                    s_id = s.get("id", "")
                    if s_id in strategy_performance:
                        values = strategy_performance[s_id]
                        # 确保数据长度与周期数量一致
                        if len(values) != len(periods):
                            # 如果数据不足，用0填充
                            values = values + [0] * (len(periods) - len(values))
                        ax3.plot(periods, values, marker='o', label=s.get("name", s_id))
                
                ax3.set_title('各时间周期收益表现')
                ax3.set_ylabel('加权收益')
                ax3.set_xlabel('时间周期')
            ...
            logger.info(f"策略比较可视化已保存到: {output_file}")
            return output_file
            
        except Exception as e:
            logger.error(f"可视化策略比较结果时出错: {e}")
            return ""
