...
def 回测_csv():
    ...
    # lst转换为DataFrame, code,date,type
    df = pd.DataFrame(lst, columns=["code", "name", "涨幅"])
    # 按照涨幅从小到大排序
    df = df.sort_values(by="涨幅", ascending=False)
    # 按照涨幅是否小于0分为两组
    df1 = df[df["涨幅"].astype(float) < 0]
    df2 = df[df["涨幅"].astype(float) >= 0.1]
    # 输出涨幅大于0.1的数量
    print("涨幅大于0.1", len(df[df["涨幅"].astype(float) >= 0.1]))
    # 输出两组数据大小
    print("踩雷", len(df1))
    print("选股", len(df2))
    ...
    lst1 = [(x[0], "20230316", 3) for x in lst1]
    lst2 = [(x[0], "20230316", 1) for x in lst2]
    lst = lst1 + lst2
    回测_汇总(lst)
...
def 回测_汇总(lst):
    ...
    卖出_dict = sorted(卖出_dict.items(), key=lambda x: x[1], reverse=False)
    踩雷_dict = sorted(踩雷_dict.items(), key=lambda x: x[1], reverse=False)
...
def 选股(code, log=False):
    ...
    result = 筛选器(fm, log)
    if result[0]:
        print(fm.get_desc(), "符合条件", 涨幅(fm.get_code(), date, fm.dataDay.close[-1]), result[1].count(True),
              result[1])
        # 输出到txt文件
...
def 买入_基本(fm, log=False):
    ...
    macd = fm.MACD小于("日", 0.9) and fm.DIFF上移("月") and fm.MACD上移("月") and fm.DIFF上移("周") and fm.DEA上移("周")
    月kdj = fm.K和D和J上移("月")
    周kdj = fm.K和D上移("周")
    月吸筹 = (fm.吸筹("月", 15))
    日周吸筹 = fm.吸筹("周", 20) or fm.吸筹("日", 20) or fm.吸筹("60", 20) or fm.吸筹("30", 20) or fm.吸筹("15", 20)
    result = [基本, 均线, macd, 月kdj, 周kdj, 月吸筹, 日周吸筹]
    if log:
        ...
        # 向下取整
    return 基本 and 均线 and macd and (月kdj or 周kdj) and (月吸筹 or 日周吸筹), result
