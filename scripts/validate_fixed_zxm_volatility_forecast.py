...
"""
ZXM_VOLATILITY_FORECAST指标正式验证脚本
"""
...
def run_comprehensive_validation():
    """运行综合验证"""
    logger.info("🚀 开始ZXM_VOLATILITY_FORECAST指标综合验证...")
    ...
    try:
        ...
        for vol_name, vol_value in zip(['5d', '10d', '20d', 'ewma', 'forecast'], volatilities):
            if not (0 <= vol_value <= 2.0):  # 年化波动率通常在0-200%之间
                logger.error(f"❌ 数值验证失败：{vol_name}波动率 {vol_value} 超出合理范围")
                return False
        
        # 检查百分位数
        percentile = result['volatility_percentile']
        if not (0 <= percentile <= 100):
            logger.error(f"❌ 数值验证失败：波动率百分位数 {percentile} 超出0-100范围")
            return False
        ...
        if result['volatility_trend'] not in valid_trends:
            logger.error(f"❌ 数值验证失败：无效的趋势值 {result['volatility_trend']}")
            return False
        
        if result['trend_strength'] not in valid_strengths:
            logger.error(f"❌ 数值验证失败：无效的强度值 {result['trend_strength']}")
            return False
        
        if result['risk_level'] not in valid_risks:
            logger.error(f"❌ 数值验证失败：无效的风险等级 {result['risk_level']}")
            return False
        ...
        required_pattern_keys = [
            ...
            'trends', 'trend_strengths', 'risk_levels', 'thresholds'
        ]
        ...
        logger.info("=" * 60)
        logger.info("🎉 ZXM_VOLATILITY_FORECAST指标验证完成")
        logger.info("=" * 60)
        logger.info(f"📊 验证结果:")
...
def main():
    ...
    if success:
        logger.info("🎉 ZXM_VOLATILITY_FORECAST指标验证成功！")
        return True
    else:
        logger.error("❌ ZXM_VOLATILITY_FORECAST指标验证失败！")
        return False
