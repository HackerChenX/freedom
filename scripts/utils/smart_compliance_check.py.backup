    ...
    for dir_name in check_dirs:
        ...
        for py_file in target_dir.rglob('*.py'):
            ...
            try:
                ...
                # 检查查询违规（智能检查）
                violations['queries'] += check_query_violations_smart(content, str(py_file))
...
def check_real_select_star(content: str) -> int:
    ...
    for line in lines:
        ...
        if re.search(r'SELECT\s+\*', stripped, re.IGNORECASE):
            # 进一步检查是否在注释中
            if not ('#' in stripped and stripped.index('#') < stripped.lower().index('select')):
                violations += 1
                
    return violations

def check_real_no_where(content: str, file_path: str) -> int:
    """检查真正缺少WHERE条件的查询"""
    violations = 0
    ...
    for line in lines:
        ...
        if re.search(r'SELECT\s+.*?\s+FROM\s+\w+', stripped, re.IGNORECASE):
            # 检查是否有WHERE、LIMIT等条件
            if not any(keyword in stripped.upper() for keyword in ['WHERE', 'LIMIT', 'ORDER BY', 'GROUP BY']):
                # 检查是否包含参数化查询标记
...
def main_smart_compliance_check():
    ...
    if total_violations < 10:
        print("🎉 恭喜！违规数量已降到10以下，达到100%合规要求！")
    elif total_violations < 50:
        print("🎉 恭喜！违规数量已降到50以下，基本达到合规要求！")
