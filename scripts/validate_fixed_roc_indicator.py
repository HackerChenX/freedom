...
def run_comprehensive_validation():
    ...
    try:
        ...
        # 检查必需的输出
        required_outputs = ['roc', 'ROC_VALUE', 'roc_ma']
        ...
        if len(roc_values) == 0:
            logger.error("❌ 算法验证失败：ROC值全为NaN")
            return False
        
        # ROC值应该在合理范围内（通常-100%到+100%之间）
        if not all(-100 <= val <= 100 for val in roc_values):
            logger.error("❌ 算法验证失败：ROC值超出合理范围")
            return False
        ...
        if len(manual_valid) > 0 and len(indicator_valid) > 0:
            ...
            if len(common_idx) > 0:
                ...
                if correlation < 0.99:
                    logger.error(f"❌ 数值验证失败：ROC计算相关性过低 {correlation:.4f}")
                    return False
        ...
        # 检查形态列数量
        expected_patterns = ['positive_momentum', 'negative_momentum', 'acceleration', 'deceleration']
...
def main():
    ...
    if success:
        logger.info("🎉 ROC指标验证成功！")
        return True
    else:
        logger.error("❌ ROC指标验证失败！")
        return False
