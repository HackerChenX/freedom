...
def create_comprehensive_test_data(length: int = 200) -> pd.DataFrame:
    ...
    for i in range(data_length):
        ...
        # 在多个位置插入V型反转形态
        if 20 <= i <= 30:  # 第一个V型反转（底部）
            if i <= 25:  # 急速下跌阶段
                decline_factor = (i - 20) * 1.0
                open_price = close + 0.6
                close = close_prices[20] - decline_factor
                high_price = open_price + 0.3
                low_price = close - 0.4
                close_prices[i] = close
            elif i == 26:  # V型底部
                open_price = close_prices[25] + 0.3
                close = close_prices[25] - 0.8  # 最低点
                high_price = open_price + 0.2
                low_price = close - 0.2
                close_prices[i] = close
            else:  # 快速反弹阶段
                rebound_factor = (i - 26) * 1.2
                open_price = close_prices[26] + 0.4
                close = close_prices[26] + rebound_factor
                high_price = close + 0.5
                low_price = open_price - 0.3
                close_prices[i] = close
        elif 80 <= i <= 90:  # 第二个V型反转（倒V型）
            if i <= 85:  # 急速上涨阶段
                rise_factor = (i - 80) * 0.9
                open_price = close - 0.5
                close = close_prices[80] + rise_factor
                high_price = close + 0.4
                low_price = open_price - 0.2
                close_prices[i] = close
            elif i == 86:  # 倒V型顶部
                open_price = close_prices[85] - 0.3
                close = close_prices[85] + 0.6  # 最高点
                high_price = close + 0.2
                low_price = open_price - 0.2
                close_prices[i] = close
            else:  # 快速下跌阶段
                decline_factor = (i - 86) * 1.1
                open_price = close_prices[86] - 0.4
                close = close_prices[86] - decline_factor
                high_price = open_price + 0.3
                low_price = close - 0.5
                close_prices[i] = close
        elif 150 <= i <= 160:  # 第三个V型反转（底部）
            if i <= 155:  # 急速下跌阶段
                decline_factor = (i - 150) * 0.8
                open_price = close + 0.5
                close = close_prices[150] - decline_factor
                high_price = open_price + 0.25
                low_price = close - 0.35
                close_prices[i] = close
            elif i == 156:  # V型底部
                open_price = close_prices[155] + 0.25
                close = close_prices[155] - 0.6
                high_price = open_price + 0.15
                low_price = close - 0.15
                close_prices[i] = close
            else:  # 快速反弹阶段
                rebound_factor = (i - 156) * 1.0
                open_price = close_prices[156] + 0.35
                close = close_prices[156] + rebound_factor
                high_price = close + 0.4
...
def validate_v_shaped_reversal_complete():
    """完整验证V_SHAPED_REVERSAL指标"""
    logger.info("🚀 开始完整验证V_SHAPED_REVERSAL指标...")
    ...
    try:
        ...
        if result is not None and not result.empty:
            if 'v_reversal' in result.columns:
                v_reversal_signals = result['v_reversal']
                detected_count = v_reversal_signals.sum()
                
                logger.info(f"  - 检测到的V型反转形态数量: {detected_count}")
                ...
                if detected_count >= 2:  # 期望检测到至少2个形态
                    ...
                    validation_score += 20
                    logger.info("  ✅ 算法正确性良好 (+20分)")
                else:
                    validation_score += 10
                    logger.warning("  ⚠️ 算法正确性部分通过 (+10分)")
            else:
                logger.error("  ❌ 缺少v_reversal列")
        else:
            logger.error("  ❌ 计算结果为空")
        ...
        if 'v_reversal' in result.columns:
            ...
            # 检查数据类型
            if v_reversal_signals.dtype == bool:
                validation_score += 10
                logger.info("  ✅ 数据类型正确 (布尔型) (+10分)")
            else:
                logger.warning(f"  ⚠️ 数据类型不正确: {v_reversal_signals.dtype}")
            
            # 检查数值范围
            if v_reversal_signals.isin([True, False]).all():
                validation_score += 10
        ...
        try:
            ...
            if patterns is not None and 'v_reversal' in patterns.columns:
                validation_score += 10
                logger.info("  ✅ get_patterns方法正常工作 (+10分)")
            else:
                logger.warning("  ⚠️ get_patterns返回结果不完整")
        ...
        # 生成验证报告
        logger.info("=" * 60)
        logger.info("🎯 V_SHAPED_REVERSAL指标验证报告")
        logger.info("=" * 60)
        ...
        if validation_score >= 95:
            logger.info("🎉 V_SHAPED_REVERSAL指标验证通过！")
            logger.info("✅ 可以标记为PASSED状态")
        else:
            logger.warning("⚠️ V_SHAPED_REVERSAL指标验证未完全通过")
...
def main():
    """主函数"""
    try:
        success, score = validate_v_shaped_reversal_complete()
        
        if success:
            logger.info("🎉 V_SHAPED_REVERSAL指标完整验证成功！")
            logger.info(f"最终得分: {score}/100")
        else:
            logger.error("❌ V_SHAPED_REVERSAL指标验证失败")
