...
class ProductionIndicatorTester:
    ...
    def test_single_indicator(self, indicator_name: str, test_date: str = None, 
        ...
        logger.info(f"🚀 开始测试指标: {indicator_name} (测试日期: {test_date})")
        ...
        logger.info(f"✅ 指标 {indicator_name} 测试完成:")
    ...
    def test_multiple_indicators(self, indicator_names: List[str], test_date: str = None,
        ...
        Returns:
            Dict[str, Any]: 批量测试结果
        """
        logger.info(f"🚀 开始批量测试 {len(indicator_names)} 个指标")
    ...
    def _generate_text_report(self, results: Dict[str, Any], report_path: str):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            if 'batch_summary' in results:
                ...
                f.write(f"成功测试: {batch_summary.get('successful_indicators', 0)}\n")
                f.write(f"测试失败: {batch_summary.get('failed_indicators', 0)}\n")
            ...
            # 如果是单个指标测试结果
                ...
                f.write(f"指标测试结果: {indicator_name}\n")
                f.write("-" * 40 + "\n")
                
                if not results.get('success', False):
                    f.write(f"测试失败: {results.get('error', 'Unknown error')}\n")
                    return
                ...
                for stock_code in selected_stocks:
                    ...
                    if stock_code in indicator_values:
                        ...
                        row.update({
                            ...
                            'signal_value': values.get('signal_value', ''),
                        })
...
def main():
    ...
    parser.add_argument('--indicators', type=str, nargs='+', 
                       help='要测试的指标名称列表，支持多个指标')
    parser.add_argument('--max-stocks', type=int, default=1000,
                       help='最大测试股票数量，默认1000')
    parser.add_argument('--output-dir', type=str, default='results/indicator_test',
                       help='输出目录，默认results/indicator_test')
