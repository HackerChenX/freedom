    ...
    for i in range(data_length):
        ...
        if 20 <= i <= 22:  # 第一个三个白兵
            if i == 20:  # 第一根白兵
                ...
                open_price = close_prices[i-1] - 0.8
                close = close_prices[i-1] + 1.2
                ...
                close_prices[i] = close
        elif 80 <= i <= 82:  # 第二个三个白兵
            if i == 80:
                open_price = close - 1.5
                high_price = close + 0.2
                low_price = open_price - 0.3
            elif i == 81:
                open_price = close_prices[i-1] - 0.6
                close = close_prices[i-1] + 1.0
...
def validate_three_white_soldiers_complete():
    """完整验证THREE_WHITE_SOLDIERS指标"""
    logger.info("🚀 开始完整验证THREE_WHITE_SOLDIERS指标...")
    ...
    try:
        ...
        if result is not None and not result.empty:
            if '三白兵' in result.columns:
                three_white_soldiers = result['三白兵']
                detected_count = three_white_soldiers.sum()
                
                logger.info(f"  - 检测到的三个白兵形态数量: {detected_count}")
        ...
        if '三白兵' in result.columns:
            ...
            # 检查数据类型
            if three_white_soldiers.dtype == bool:
                validation_score += 10
                logger.info("  ✅ 数据类型正确 (布尔型) (+10分)")
            else:
                logger.warning(f"  ⚠️ 数据类型不正确: {three_white_soldiers.dtype}")
            
            # 检查数值范围
            if three_white_soldiers.isin([True, False]).all():
                validation_score += 10
        ...
        # 生成验证报告
        logger.info("=" * 60)
        logger.info("🎯 THREE_WHITE_SOLDIERS指标验证报告")
        logger.info("=" * 60)
        ...
        if validation_score >= 95:
            logger.info("🎉 THREE_WHITE_SOLDIERS指标验证通过！")
            logger.info("✅ 可以标记为PASSED状态")
        else:
            logger.warning("⚠️ THREE_WHITE_SOLDIERS指标验证未完全通过")
...
def main():
    """主函数"""
    try:
        success, score = validate_three_white_soldiers_complete()
        
        if success:
            logger.info("🎉 THREE_WHITE_SOLDIERS指标完整验证成功！")
            logger.info(f"最终得分: {score}/100")
        else:
            logger.error("❌ THREE_WHITE_SOLDIERS指标验证失败")
