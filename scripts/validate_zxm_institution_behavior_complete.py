...
"""
完整验证ZXM_INSTITUTION_BEHAVIOR指标
"""
...
def create_comprehensive_test_data(length: int = 200) -> pd.DataFrame:
    ...
    for i in range(data_length):
        ...
        if 20 <= i <= 35:  # 机构悄悄建仓阶段
            ...
            close = close_prices[i-1] + 0.3  # 缓慢上涨
            high_price = close + 0.15
            low_price = open_price - 0.05
            volume = np.random.lognormal(11.0 + (i-20)*0.05, 0.15)  # 逐步放量
            close_prices[i] = close
        elif 80 <= i <= 95:  # 机构派发阶段
            open_price = close + np.random.uniform(-0.6, 0.6)
            close = close_prices[79] + np.random.uniform(-0.8, 0.8)  # 高位震荡
            high_price = max(open_price, close) + abs(np.random.normal(0, 0.3))
            low_price = min(open_price, close) - abs(np.random.normal(0, 0.3))
            volume = np.random.lognormal(12.3, 0.2)  # 大幅放量
            close_prices[i] = close
        elif 150 <= i <= 160:  # 机构砸盘阶段
            open_price = close + 0.4
            close = close_prices[i-1] - 1.0  # 快速下跌
            high_price = open_price + 0.1
            low_price = close - 0.2
            volume = np.random.lognormal(12.6, 0.12)  # 超大成交量
            close_prices[i] = close
        elif 170 <= i <= 185:  # 机构重新建仓
            open_price = close - 0.3
            close = close_prices[i-1] + 0.5  # 反弹上涨
            high_price = close + 0.2
            low_price = open_price - 0.1
            volume = np.random.lognormal(11.5, 0.18)  # 适度放量
            close_prices[i] = close
        else:
            # 正常交易
...
def validate_zxm_institution_behavior_complete():
    """完整验证ZXM_INSTITUTION_BEHAVIOR指标"""
    logger.info("🚀 开始完整验证ZXM_INSTITUTION_BEHAVIOR指标...")
    ...
    try:
        ...
        indicator = registry.create_indicator('ZXM_INSTITUTION_BEHAVIOR')
        result = indicator.calculate(test_data)
        
        if result is not None and not result.empty:
            # 检查机构行为相关列
            institution_columns = [col for col in result.columns if any(keyword in col.lower() for keyword in ['institution', 'behavior', 'smart', 'money'])]
            logger.info(f"  - 机构行为相关列: {institution_columns}")
            
            if len(institution_columns) >= 1:  # 期望至少1个机构行为相关列
                validation_score += 25
        ...
        # 生成验证报告
        logger.info("=" * 60)
        logger.info("🎯 ZXM_INSTITUTION_BEHAVIOR指标验证报告")
        logger.info("=" * 60)
        ...
        if validation_score >= 95:
            logger.info("🎉 ZXM_INSTITUTION_BEHAVIOR指标验证通过！")
            logger.info("✅ 可以标记为PASSED状态")
        else:
            logger.warning("⚠️ ZXM_INSTITUTION_BEHAVIOR指标验证未完全通过")
...
def main():
    """主函数"""
    try:
        success, score = validate_zxm_institution_behavior_complete()
        
        if success:
            logger.info("🎉 ZXM_INSTITUTION_BEHAVIOR指标完整验证成功！")
            logger.info(f"最终得分: {score}/100")
        else:
            logger.error("❌ ZXM_INSTITUTION_BEHAVIOR指标验证失败")
