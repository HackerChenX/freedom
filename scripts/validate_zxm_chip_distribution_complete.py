...
def create_comprehensive_test_data(length: int = 200) -> pd.DataFrame:
    ...
    for i in range(data_length):
        ...
        if 20 <= i <= 35:  # 筹码高度集中阶段
            ...
            high_price = max(open_price, close) + abs(np.random.normal(0, 0.08))
            low_price = min(open_price, close) - abs(np.random.normal(0, 0.08))
            volume = np.random.lognormal(11.8, 0.15)  # 高成交量，筹码集中
            close_prices[i] = close
        elif 80 <= i <= 95:  # 筹码分散阶段
            open_price = close + np.random.normal(0, 1.2)
            close = close_prices[i-1] + np.random.normal(0, 1.8)
            high_price = max(open_price, close) + abs(np.random.normal(0, 0.6))
            low_price = min(open_price, close) - abs(np.random.normal(0, 0.6))
            volume = np.random.lognormal(12.2, 0.25)  # 超大成交量，筹码分散
            close_prices[i] = close
        elif 150 <= i <= 165:  # 筹码重新分布
            if i <= 157:  # 先分散
                open_price = close + np.random.normal(0, 0.8)
                close = close_prices[i-1] + np.random.normal(0, 1.0)
                high_price = max(open_price, close) + abs(np.random.normal(0, 0.4))
                low_price = min(open_price, close) - abs(np.random.normal(0, 0.4))
                volume = np.random.lognormal(11.9, 0.2)
                close_prices[i] = close
            else:  # 后集中
                price_range = 1.0
                open_price = close + np.random.uniform(-price_range/2, price_range/2)
                close = close_prices[157] + np.random.uniform(-price_range/2, price_range/2)
                high_price = max(open_price, close) + abs(np.random.normal(0, 0.1))
                low_price = min(open_price, close) - abs(np.random.normal(0, 0.1))
                volume = np.random.lognormal(11.6, 0.18)
                close_prices[i] = close
        else:
            # 正常交易
...
def validate_zxm_chip_distribution_complete():
    """完整验证ZXM_CHIP_DISTRIBUTION指标"""
    logger.info("🚀 开始完整验证ZXM_CHIP_DISTRIBUTION指标...")
    ...
    try:
        ...
        indicator = registry.create_indicator('ZXM_CHIP_DISTRIBUTION')
        result = indicator.calculate(test_data)
        
        if result is not None and not result.empty:
            # 检查筹码分布相关列
            chip_columns = [col for col in result.columns if any(keyword in col.lower() for keyword in ['chip', 'distribution', 'concentration'])]
            logger.info(f"  - 筹码分布相关列: {chip_columns}")
            
            if len(chip_columns) >= 3:  # 期望至少3个筹码分布相关列
                validation_score += 25
        ...
        # 生成验证报告
        logger.info("=" * 60)
        logger.info("🎯 ZXM_CHIP_DISTRIBUTION指标验证报告")
        logger.info("=" * 60)
        ...
        if validation_score >= 95:
            logger.info("🎉 ZXM_CHIP_DISTRIBUTION指标验证通过！")
            logger.info("✅ 可以标记为PASSED状态")
        else:
            logger.warning("⚠️ ZXM_CHIP_DISTRIBUTION指标验证未完全通过")
...
def main():
    """主函数"""
    try:
        success, score = validate_zxm_chip_distribution_complete()
        
        if success:
            logger.info("🎉 ZXM_CHIP_DISTRIBUTION指标完整验证成功！")
            logger.info(f"最终得分: {score}/100")
        else:
            logger.error("❌ ZXM_CHIP_DISTRIBUTION指标验证失败")
