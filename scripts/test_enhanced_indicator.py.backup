...
from indicators.synergy import Indicator_synergy
from utils.logger import get_logger

logger = get_logger(__name__)


def get_test_data_Indicator(stock_code: str, start_date: str, end_date: str) -> pd.DataFrame:
    """
    获取测试数据
...
def test_enhanced_obv_Indicator():
    """测试增强型OBV指标"""
    # 获取测试数据
    ...
    data = get_test_data_Indicator(stock_code, start_date, end_date)
    if data.empty:
    ...
    try:
        # 创建增强型OBV指标
        from indicators.enhanced_obv import Enhanced_oBV
...
def test_indicator_synergy():
    ...
    # 创建指标协同框架
    synergy = Indicator_synergy(detector)
    
    # 添加增强型OBV指标
    enhanced_obv = Enhanced_oBV(ma_period=30, sensitivity=1.2)
