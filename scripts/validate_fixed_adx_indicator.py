...
def run_comprehensive_validation():
    ...
    try:
        ...
        # 检查必需的输出
        required_outputs = ['ADX', 'PDI', 'MDI', 'ADXR']
        ...
        if len(adx_values) == 0:
            logger.error("❌ 算法验证失败：ADX值全为NaN")
            return False
        
        # ADX应该在0-100范围内
        if not all(0 <= val <= 100 for val in adx_values):
            logger.error("❌ 算法验证失败：ADX值超出0-100范围")
            return False
        ...
        if len(pdi_values) == 0 or len(mdi_values) == 0:
            logger.error("❌ 数值验证失败：PDI或MDI值全为NaN")
            return False
        
        # PDI和MDI应该都在0-100范围内
        if not all(0 <= val <= 100 for val in pdi_values):
            logger.error("❌ 数值验证失败：PDI值超出0-100范围")
            return False
        
        if not all(0 <= val <= 100 for val in mdi_values):
            logger.error("❌ 数值验证失败：MDI值超出0-100范围")
            return False
        ...
        # 检查形态列数量
        pattern_columns = [col for col in patterns.columns if col.startswith('ADX_')]
        if len(pattern_columns) < 5:  # 至少应该有5种ADX形态
            logger.error(f"❌ 功能验证失败：形态种类过少 {len(pattern_columns)}")
            return False
...
def main():
    ...
    if success:
        logger.info("🎉 ADX指标验证成功！")
        return True
    else:
        logger.error("❌ ADX指标验证失败！")
        return False
