...
def run_comprehensive_validation():
    """运行综合验证"""
    logger.info("🚀 开始ZXM_MARKET_SENTIMENT指标综合验证...")
    ...
    try:
        ...
        # 检查必需的输出
        required_outputs = [
            'FearGreedIndex', 'InvestorSentiment', 'MarketHeat', 'CompositeSentiment',
            'ExtremeFearSignal', 'ExtremeGreedSignal', 'SentimentReversalSignal'
        ]
        ...
        # 检查情绪指标值是否合理
        sentiment_metrics = ['FearGreedIndex', 'InvestorSentiment', 'MarketHeat', 'CompositeSentiment']
        
        for metric in sentiment_metrics:
            values = result[metric].dropna()
            if len(values) == 0:
                logger.error(f"❌ 数值验证失败：{metric} 全部为NaN")
                return False
            
            if not (0 <= values.min() <= values.max() <= 100):
                logger.error(f"❌ 数值验证失败：{metric} 值 [{values.min():.2f}, {values.max():.2f}] 超出0-100范围")
                return False
        
        # 检查信号值
        signal_columns = ['ExtremeFearSignal', 'ExtremeGreedSignal', 'SentimentReversalSignal']
        for signal in signal_columns:
            unique_values = result[signal].dropna().unique()
            if not all(val in [True, False] for val in unique_values):
                logger.error(f"❌ 数值验证失败：{signal} 包含非布尔值")
                return False
        ...
        required_pattern_keys = [
            ...
            'signals', 'sentiment_levels', 'parameters', 'thresholds'
        ]
        ...
        logger.info("=" * 60)
        logger.info("🎉 ZXM_MARKET_SENTIMENT指标验证完成")
        logger.info("=" * 60)
        logger.info(f"📊 验证结果:")
...
def main():
    ...
    if success:
        logger.info("🎉 ZXM_MARKET_SENTIMENT指标验证成功！")
        return True
    else:
        logger.error("❌ ZXM_MARKET_SENTIMENT指标验证失败！")
        return False
