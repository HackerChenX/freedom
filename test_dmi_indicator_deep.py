...
def create_test_data():
    ...
    # 生成具有波动趋势的价格数据，适合DMI测试
    base_price = 100
    trend = np.linspace(0, 15, 100)  # 上升趋势
    volatility = np.random.normal(0, 3, 100)  # 增加波动性以触发DMI信号
    
    prices = base_price + trend + volatility
    
    # 添加一些明显的趋势变化点
    prices[30:40] += 5  # 强势上涨段
    prices[60:70] -= 8  # 明显回调段
...
def test_dmi_instantiation():
    ...
    try:
        ...
        dmi_indicator = manager.create_indicator('DMI')
        
        if dmi_indicator is not None:
            print("✅ DMI指标实例化成功")
            print(f"   类名: {dmi_indicator.__class__.__name__}")
            print(f"   指标名: {getattr(dmi_indicator, 'name', 'N/A')}")
            print(f"   周期: {getattr(dmi_indicator, 'period', 'N/A')}")
            return True, dmi_indicator
        else:
            print("❌ DMI指标实例化失败")
            return False, None
            
    except Exception as e:
        print(f"❌ DMI指标实例化异常: {e}")
        return False, None

def test_dmi_calculate(dmi_indicator, test_data):
    """测试DMI指标calculate()方法"""
    print("\n" + "="*60)
    print("📊 DMI指标calculate()测试")
    print("="*60)
    ...
    try:
        ...
        if result is not None and not result.empty:
            print("✅ DMI指标calculate()方法成功")
            print(f"   结果行数: {len(result)}")
            print(f"   结果列数: {len(result.columns)}")
            print(f"   列名: {list(result.columns)}")
            
            # 检查关键的DMI列
            dmi_columns = [col for col in result.columns if any(x in col.upper() for x in ['PDI', 'MDI', 'ADX', 'ADXR'])]
            if dmi_columns:
                print(f"   DMI相关列: {dmi_columns}")
                for col in dmi_columns[:4]:  # 显示前4列的统计信息
                    if len(result[col]) > 0:
                        print(f"   {col}: 最新值={result[col].iloc[-1]:.4f}, 平均值={result[col].mean():.4f}")
                return True, result
            else:
                print("⚠️ 警告: 未找到DMI相关的输出列")
                return False, result
        else:
            print("❌ DMI指标calculate()返回空结果")
            return False, None
            
    except Exception as e:
        print(f"❌ DMI指标calculate()异常: {e}")
        return False, None

def test_dmi_get_signal(dmi_indicator, test_data):
    """测试DMI指标get_signal()方法"""
    print("\n" + "="*60)
    print("📡 DMI指标get_signal()测试")
    print("="*60)
    
    try:
        signal = dmi_indicator.get_signal(test_data)
        
        if signal and isinstance(signal, dict):
            print("✅ DMI指标get_signal()方法成功")
            ...
            if not missing_fields:
                ...
                if 'metadata' in signal and signal['metadata']:
                    ...
                    if 'pdi_value' in metadata:
                        print(f"   +DI值: {metadata['pdi_value']:.4f}")
                    if 'mdi_value' in metadata:
                        print(f"   -DI值: {metadata['mdi_value']:.4f}")
                    if 'adx_value' in metadata:
                        print(f"   ADX值: {metadata['adx_value']:.4f}")
            ...
            print("❌ DMI指标get_signal()返回格式不正确")
            return False, None
            
    except Exception as e:
        print(f"❌ DMI指标get_signal()异常: {e}")
        return False, None

def test_dmi_data_validation(dmi_indicator):
    """测试DMI指标数据验证"""
    print("\n" + "="*60)
    print("🔒 DMI指标数据验证测试")
    print("="*60)
...
def test_dmi_performance(dmi_indicator, test_data):
    """测试DMI指标性能"""
    print("\n" + "="*60)
    print("⚡ DMI指标性能测试")
    print("="*60)
    ...
    for i in range(10):
        dmi_indicator.calculate(test_data)
    calculate_time = (time.time() - start_time) / 10
    
    # 测试get_signal性能
    start_time = time.time()
    for i in range(100):
        dmi_indicator.get_signal(test_data)
...
def run_dmi_tests():
    ...
    # 1. 实例化测试
    success, dmi_indicator = test_dmi_instantiation()
    if success:
        passed_tests += 1
    
    if dmi_indicator is None:
        print("❌ 无法继续后续测试，DMI实例化失败")
        return
    
    # 2. calculate测试
    success, result = test_dmi_calculate(dmi_indicator, test_data)
    if success:
        passed_tests += 1
    
    # 3. get_signal测试
    success, signal = test_dmi_get_signal(dmi_indicator, test_data)
    if success:
        passed_tests += 1
    
    # 4. 数据验证测试
    success = test_dmi_data_validation(dmi_indicator)
    if success:
        passed_tests += 1
    
    # 5. 性能测试
    success = test_dmi_performance(dmi_indicator, test_data)
    if success:
        passed_tests += 1
    ...
    if passed_tests == total_tests:
        print("🎉 DMI指标全面测试通过！符合L4文档设计预期和基类设计预期")
    else:
        print("⚠️ DMI指标存在问题，需要进一步修复")
