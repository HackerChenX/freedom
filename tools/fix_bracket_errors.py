...
def fix_bracket_errors_in_file(file_path):
    """修复单个文件中的括号错误"""
    try:
...
def find_files_with_bracket_errors():
    """查找所有有括号错误的测试文件"""
    test_files = []
...
def main():
    ...
    # 查找需要修复的文件
    test_files = find_files_with_bracket_errors()
    ...
    for file_path in test_files:
        print(f"\n🔧 修复文件: {file_path}")
        if fix_bracket_errors_in_file(file_path):
            print(f"✅ 修复成功")
            fixed_count += 1
        else:
            print(f"❌ 修复失败")
