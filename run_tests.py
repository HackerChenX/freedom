...
try:
    ...
    HAS_COVERAGE = False
    print("Warning: 'coverage' package not found. Code coverage report will not be available.")
...
def run_tests(test_type, pattern, output_dir, generate_report, measure_coverage):
    ...
    # 设置覆盖率
    cov = None
    if measure_coverage and HAS_COVERAGE:
        cov = coverage.Coverage(
            source=["strategy", "indicators", "db", "utils"],
            omit=["*/test*", "*/migrations/*", "*/venv/*"]
        )
        cov.start()
    elif measure_coverage and not HAS_COVERAGE:
        print("Warning: 覆盖率测量被跳过，因为'coverage'包未安装")
    ...
    # 生成覆盖率报告
    if measure_coverage and HAS_COVERAGE and cov:
        cov.stop()
        cov.save()
