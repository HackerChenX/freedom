...
from concurrent.futures import Thread_pool_executor, Process_pool_executor, as_completed
import asyncio
import time
import gc
...
from db.interfaces.data_access_interface import IData_access, Indicator_result
...
class ProcessingConfig:
    """并行处理配置"""
    mode: processing_mode = Processing_mode.THREAD
...
class ParallelProcessor:
    ...
    def __init___23(self, data_access: IData_access, cache_service: ICache_service,
    ...
    def process_indicators_parallel(self, stock_data: Dict[str, pd.DataFrame],
                                  indicators: List[str],
                                  params: Optional[Dict[str, Any]] = None) -> Dict[str, Dict[str, Indicator_result]]:
        """
        并行计算股票指标
        ...
        Returns:
            Dict[str, Dict[str, Indicator_result]]: {股票代码: {指标名: 结果}}
        """
        ...
        elif self.config.mode == Processing_mode.PROCESS:
    ...
    def _process_with_threads(self, tasks: List[Processing_task],
        ...
        with Thread_pool_executor(max_workers=self.config.max_workers) as executor:
            # 提交任务
            future_to_task = {
    ...
    def _process_with_processes(self, tasks: List[Processing_task],
        ...
        with Process_pool_executor(max_workers=self.config.max_workers) as executor:
            # 提交任务块
            future_to_chunk = {
    ...
    def _calculate_indicators_task(self, task: Processing_task) -> Dict[str, Indicator_result]:
        """计算指标任务"""
        results = {}
    ...
    async def _calculate_indicators_task_async(self, task: Processing_task) -> Dict[str, Indicator_result]:
        """异步计算指标任务"""
        # 在线程池中执行计算密集型任务
        loop = asyncio.get_event_loop()
    ...
    def _calculate_single_indicator(self, data: pd.DataFrame, indicator_name: str,
                                   params: Dict[str, Any]) -> Indicator_result:
        """
        计算单个指标
