...
#!/usr/bin/env python3
"""
2025年5月12日30分钟吸筹信号+MACD上移选股策略（修复版）

修复关键问题：
1. 使用真实的ClickHouse 30分钟K线数据，而非模拟数据
2. 确保数据来源的真实性和可靠性
3. 如果30分钟数据不可用，明确警告并回退到日线数据
...
class FixedAbsorbMACDStrategy:
    """30分钟吸筹信号+MACD上移选股策略（修复版 - 使用真实ClickHouse数据）"""
    
    def __init__(self):
        """初始化策略"""
        self.strategy_name = "30分钟吸筹信号+MACD上移选股策略（修复版）"
    ...
    def execute_strategy(self, stock_pool: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        logger.info(f"分析周期: {self.period} (使用真实ClickHouse数据)")
        ...
        selected_stocks = []
        analysis_details = {}
        data_source_stats = {"real_30min": 0, "simulated": 0, "no_data": 0}
        ...
        for i, stock_code in enumerate(stock_pool, 1):
            ...
            try:
                # 获取30分钟数据（优先使用真实数据）
                stock_data, data_source = self._get_30min_data_with_source(stock_code, self.target_date)
                ...
                if meets_criteria:
                    ...
                    logger.info(f"✅ {stock_code} 符合条件：吸筹信号强度{absorb_result['signal_strength']:.2f}, MACD强度{macd_result['macd_strength']:.2f} (数据源: {data_source})")
        ...
        result = {
            ...
            'strategy_criteria': {
                ...
                'data_priority': '优先使用真实30分钟数据，回退到模拟数据'
            }
        }
        
        logger.info(f"策略执行完成，选出 {len(selected_stocks)} 只股票")
        logger.info(f"数据源统计: 真实30分钟数据{data_source_stats['real_30min']}只, 模拟数据{data_source_stats['simulated']}只, 无数据{data_source_stats['no_data']}只")
        return result
    ...
    def _get_30min_data_with_source(self, stock_code: str, target_date: str) -> tuple:
        """获取30分钟K线数据并返回数据源信息"""
        try:
            # 计算数据范围（获取目标日期前后足够的数据用于技术指标计算）
            ...
            try:
                ...
                if stock_info:
                    ...
                    if not df.empty:
                        ...
                        # 检查是否有目标日期的数据且数据量合理
                        target_day_data = df[df['date'].dt.date == target_date_obj.date()]
                        if len(target_day_data) > 1:  # 真实30分钟数据一天应该有多条记录
                            # 为30分钟数据添加datetime列
                            df = self._add_datetime_to_30min_data(df, target_date_obj)
                            logger.info(f"✅ {stock_code} 使用真实30分钟数据: {len(df)} 条记录")
                            return df, "real_30min"
                
            except Exception as e:
                logger.warning(f"获取{stock_code}真实30分钟数据失败: {e}")
            
            # 回退到日线数据模拟
            logger.warning(f"⚠️ {stock_code} 未找到真实30分钟数据，回退到日线数据模拟")
            simulated_data = self._fallback_to_daily_data(stock_code, target_date)
    ...
    def _fallback_to_daily_data(self, stock_code: str, target_date: str) -> Optional[pd.DataFrame]:
        """回退到日线数据并模拟30分钟数据（仅在没有真实30分钟数据时使用）"""
        try:
            logger.warning(f"🔄 为股票 {stock_code} 使用日线数据模拟30分钟数据")
            ...
            # 转换为DataFrame
            if hasattr(daily_data, 'to_dataframe'):
                df = daily_data.to_dataframe()
            elif hasattr(daily_data, 'data'):
                df = daily_data.data
            ...
            logger.info(f"📊 为股票 {stock_code} 模拟生成30分钟数据: {len(min30_data)} 条记录")
            return min30_data
            
        except Exception as e:
            logger.error(f"回退到日线数据失败: {e}")
            return None
    
    def _simulate_30min_data(self, daily_data: pd.DataFrame, target_date: str) -> pd.DataFrame:
        """基于日线数据模拟30分钟数据（警告：这是模拟数据，不是真实市场数据）"""
        logger.warning("⚠️ 正在使用模拟的30分钟数据，这不是真实的市场数据！")
...
from db.sql_manager import SQLManager, QueryType
    ...
    def save_results(self, results: Dict[str, Any]) -> str:
        ...
        filename = f"fixed_absorb_macd_strategy_results_{timestamp}.json"
    ...
    def print_results(self, results: Dict[str, Any]):
        ...
        print(f"   • 模拟数据: {stats.get('simulated', 0)} 只")
        print(f"   • 无数据: {stats.get('no_data', 0)} 只")
        print()
...
def main_fixed_absorb_macd_strategy_20250512():
    ...
    print("⏰ 分析周期: 30分钟K线（优先使用真实ClickHouse数据）")
    print("🎯 选股条件: 出现吸筹信号 AND MACD上移")
    print("🔧 关键修复: 使用真实30分钟数据，而非模拟数据")
    
    try:
        # 创建策略实例
        strategy = FixedAbsorbMACDStrategy()
