...
**基本原则**: "只有数据是区分模拟还是真实的，计算和处理逻辑都应该是通用的"
...
**已完成验证**:
- ✅ BuypointAnalyzer: 23/23测试通过（100%）
- ✅ SelectionStrategyTester: 架构重构完成
- ✅ ClosedLoopValidator: 统一计算引擎
- ✅ UnifiedIndicatorTester: 去除模式参数

**技术特性**:
- 🔧 优雅降级机制：无法导入真实引擎时自动fallback
- 📊 基础真实计算：使用标准技术分析公式
- 🏗️ 架构统一性：所有组件遵循相同设计模式
- 🧪 测试兼容性：保持100%测试通过率
...
def calculate_indicator(self, data, testing_mode=True):
    if testing_mode:
        return self._mock_calculation(data)  # 禁止
    else:
        return self._real_calculation(data)  # 禁止

# ✅ 正确做法：统一计算，数据层区分
def calculate_indicator(self, data):
    # 统一使用真实计算引擎
    return self.unified_engine.calculate(data)
    # 数据来源通过data.metadata['source']标识
```
...
graph TB
    ...
    C --> F[StockInfo兼容数据生成<br/>🔵 数据层：模拟数据]
    C --> G[历史数据补全器<br/>🔵 数据层：增强数据]
    
    D --> H[统一买点分析器<br/>🟢 计算层：真实引擎]
    E --> I[统一选股脚本<br/>🟢 计算层：真实逻辑]
    
    I --> J[bin/stock_select.py<br/>🟢 处理层：通用逻辑]
    J --> K[策略执行引擎<br/>🟢 处理层：统一执行]
    
    H --> L[形态识别结果<br/>🟡 输出层：标识来源]
    I --> M[选股结果<br/>🟡 输出层：标识来源]
    
    L --> N[闭环验证器<br/>🟢 处理层：统一验证]
    M --> N
    N --> O[统一测试报告<br/>🟡 输出层：完整结果]
...
**架构层次说明**:
- 🔵 **数据层**：区分模拟/真实数据来源，保持格式一致
- 🟢 **计算/处理层**：统一使用真实计算引擎和业务逻辑
- 🟡 **输出层**：标识数据来源，保持结果格式一致
...
class UnifiedIndicatorTester:
    ...
    def __init__(self, config_path: str = "config/unified_test_config.yaml"):
        ...
        注意：不再有testing_mode参数，架构统一
        """
        self.config = self._load_config(config_path)
        
        # 核心组件初始化（全部使用真实实现）
        self.data_generator = StockInfoCompatibleDataGenerator()
        self.buypoint_tester = BuypointRecognitionTester()  # 真实计算引擎
        self.selection_tester = SelectionStrategyTester()   # 真实选股脚本
        self.validator = ClosedLoopValidator()              # 真实验证引擎
    ...
    def test_indicator_comprehensive(self, indicator_name: str) -> Dict[str, Any]:
        ...
        results = {
            ...
            'overall_score': 0.0,
            'data_source': 'simulated',  # 输出层：标识数据来源
            'computation_engine': 'unified_real_engine'  # 输出层：标识计算引擎
        }
        
        for pattern_type in patterns:
            # 1. 数据层：生成符合stockInfo结构的模拟数据
            mock_data_pool = self._generate_comprehensive_data_pool(
                indicator_name, pattern_type
            )
            # 在数据中添加来源标识
            for df in mock_data_pool:
                df.attrs['data_source'] = 'simulated'
                df.attrs['generator'] = 'unified_test_framework'
            ...
            results['pattern_tests'][pattern_type] = {
                ...
                'engine_type': 'unified_real_engine'
            }
...
class StockInfoCompatibleDataGenerator:
    ...
    def generate_stockinfo_compatible_data(self, 
        ...
        Returns:
            pd.DataFrame: 符合stockInfo结构的模拟数据
        """
        # 1. 确定指标所需的最小历史数据量
        ...
        stockinfo_data.attrs['target_pattern'] = pattern_type
        ...
        logger.info(f"✅ 生成模拟数据: {stock_code} ({indicator_name}-{pattern_type})")
    ...
    def _generate_default_column_data(self, column_name: str, length: int) -> pd.Series:
        """
        为缺失列生成默认数据
        ...
        """
        defaults = {
            'pe_ratio': np.random.uniform(10, 30, length),
            'pb_ratio': np.random.uniform(1, 5, length),
            'market_cap': np.random.uniform(50e8, 500e8, length),
            'circulating_cap': np.random.uniform(30e8, 300e8, length),
            'industry': ['科技'] * length,
            'concept': ['人工智能'] * length,
            'amount': np.random.uniform(1e8, 10e8, length),
        }
        
        return pd.Series(defaults.get(column_name, [0] * length))
```
...
```python
# 1. 禁止在计算逻辑中区分模式
def calculate_indicator(self, data, is_mock=True):
    if is_mock:
        return mock_calculation()  # ❌ 禁止
    else:
        return real_calculation()  # ❌ 禁止

# 2. 禁止testing_mode类型的参数
class ComponentClass:
    def __init__(self, testing_mode=True):  # ❌ 禁止
        pass

# 3. 禁止在业务逻辑中创建模拟分支
def process_data(self, data, use_simulation=True):
    if use_simulation:  # ❌ 禁止
        return simulate_processing()
```
...
def calculate_indicator(self, data):
    # 统一使用真实计算引擎
    result = self.unified_engine.calculate(data)
    # 数据来源在data.attrs['data_source']中标识
    return result

# 2. 组件初始化无模式参数
class ComponentClass:
    def __init__(self):  # ✅ 正确
        self.engine = self._initialize_real_engine()

# 3. 优雅降级机制（当无法使用真实引擎时）
def _initialize_real_engine(self):
    try:
        return RealEngine()
    except ImportError:
        logger.warning("使用fallback模式")
        return FallbackEngine()  # 仍使用真实算法，只是简化
```
...
1. **数据层检查**：
   ```python
   assert component.input_data.attrs['data_source'] in ['simulated', 'real']
   assert component.data_format == 'stockinfo_compatible'
   ```

2. **计算层检查**：
   ```python
   assert hasattr(component, 'unified_engine')
   assert not hasattr(component, 'mock_engine')
   assert component.calculation_method == 'real'
   ```

3. **输出层检查**：
   ```python
   assert 'data_source' in component.result
   assert 'computation_engine' in component.result
   assert component.result['computation_engine'] == 'unified_real_engine'
   ```

### 开发团队要求

1. **代码提交前**：运行架构合规性检查脚本
2. **代码审查时**：重点检查是否违反架构原则
3. **测试用例**：必须验证架构分离的正确性
4. **文档更新**：每个组件都要说明如何遵循架构原则
