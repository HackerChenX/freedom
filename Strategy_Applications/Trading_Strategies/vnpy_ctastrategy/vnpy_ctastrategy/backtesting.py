...
class BacktestingEngine:
    ...
    def __init__(self) -> None:
        ...
        # A股T+1交易制度支持
        self.t_plus_one: bool = False  # 是否启用T+1交易
        self.holdings: dict[str, int] = {}  # 可卖持仓 {日期: 数量}
        self.frozen_shares: dict[str, int] = {}  # 冻结股票 {日期: 数量}
    ...
    def clear_data(self) -> None:
        ...
        # 清理T+1相关数据
        self.holdings.clear()
        self.frozen_shares.clear()
        if hasattr(self, 'last_trading_date'):
            delattr(self, 'last_trading_date')
    ...
    def set_parameters(
        ...
        # 自动检测是否应该启用T+1交易制度
        if t_plus_one is None:
            # A股市场（深交所和上交所）默认启用T+1
            self.t_plus_one = self.exchange in [Exchange.SZSE, Exchange.SSE]
        else:
            self.t_plus_one = t_plus_one
            
        if self.t_plus_one:
            self.output(f"已启用A股T+1交易制度：{self.exchange.value}")
        else:
            self.output(f"使用T+0交易制度：{self.exchange.value}")
    ...
    def load_data(self) -> None:
        ...
        while start < self.end:
            ...
            self.output(_("加载进：{} [{:.0%}]").format(progress_bar, progress))
    ...
    def new_bar(self, bar: BarData) -> None:
        ...
        if hasattr(self, 'last_trading_date'):
            ...
            self.last_trading_date = bar.datetime.date()
    ...
    def cross_limit_order(self) -> None:
        ...
        for order in list(self.active_limit_orders.values()):
            ...
            # 更新T+1持仓记录
            self.update_holdings_on_trade(trade)
    ...
    def send_order(
        ...
        # T+1交易制度检查：如果是卖出，检查可卖数量
        if self.t_plus_one and direction == Direction.SHORT:
            available_volume = self.get_available_volume_for_sell()
            if volume > available_volume:
                error_msg = f"T+1限制：尝试卖出{volume}股，但只有{available_volume}股可卖"
    ...
    def get_available_volume_for_sell(self) -> int:
        """
        获取T+1制度下可卖的股票数量
        """
        if not self.t_plus_one:
            return abs(self.strategy.pos)  # T+0情况下，持仓即可卖
        
        # T+1情况：只能卖出T-1及之前买入的股票
        current_date = self.datetime.date().strftime("%Y-%m-%d")
        available_volume = 0
        
        # 计算所有之前交易日的可卖持仓
        for date_str, volume in self.holdings.items():
            if date_str < current_date:
                available_volume += volume
                
        return max(0, available_volume)
    
    def update_holdings_on_trade(self, trade: TradeData) -> None:
        """
        根据成交更新T+1持仓记录
        """
        if not self.t_plus_one:
        ...
        if trade.direction == Direction.LONG:
            # 买入：增加当日冻结持仓
            if current_date not in self.frozen_shares:
                self.frozen_shares[current_date] = 0
            self.frozen_shares[current_date] += trade.volume
            
        else:  # Direction.SHORT 卖出
            # 卖出：从可卖持仓中扣除（优先扣除较早的持仓）
            remaining_volume = trade.volume
            sorted_dates = sorted(self.holdings.keys())
            
            for date_str in sorted_dates:
                if remaining_volume <= 0:
                    break
                if date_str < current_date:  # 只能卖出之前的持仓
                    available = self.holdings[date_str]
                    to_sell = min(remaining_volume, available)
                    self.holdings[date_str] -= to_sell
                    remaining_volume -= to_sell
                    
                    if self.holdings[date_str] <= 0:
                        del self.holdings[date_str]
    
    def process_daily_settlement(self) -> None:
        """
        处理日终清算：冻结股票转为可卖
        """
        if not self.t_plus_one:
        ...
        # 将冻结股票转为可卖持仓
        if current_date in self.frozen_shares:
            frozen_volume = self.frozen_shares[current_date]
            if frozen_volume > 0:
                if current_date not in self.holdings:
                    self.holdings[current_date] = 0
                self.holdings[current_date] += frozen_volume
                
                self.output(f"T+1结算：{frozen_volume}股解冻，可于下一交易日卖出")
            
            # 清除当日冻结记录
            del self.frozen_shares[current_date]
