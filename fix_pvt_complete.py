...
"""
PVT文件100%完整修复工具
基于质量优先策略，确保彻底修复所有赋值语法错误
"""

import re
import subprocess

def complete_pvt_fix():
    """100%完成PVT文件修复"""
    file_path = "indicators/pvt.py"
    
    print("🚀 PVT文件100%完整修复开始...")
    ...
    for i, line in enumerate(lines):
        ...
        # 1. 基本变量赋值: var value -> var = value
        if re.search(r'^(\s+)([a-zA-Z_]\w*)\s+([a-zA-Z_]\w*(?:\.\w+)*(?:\([^)]*\))?|\d+(?:\.\d+)?|".*?"|\'.*?\'|True|False|None)$', line):
            line = re.sub(r'^(\s+)([a-zA-Z_]\w*)\s+([a-zA-Z_]\w*(?:\.\w+)*(?:\([^)]*\))?|\d+(?:\.\d+)?|".*?"|\'.*?\'|True|False|None)$', r'\1\2 = \3', line)
            
        # 2. 复杂表达式赋值: var complex_expression -> var = complex_expression
