...
class DataAccessManager(DataAccessInterface):
    ...
    @exception_handler(reraise=False, default_return=[])
    def get_stock_list_data_access_manager(self: Optional[str] = None, 
    ...
    def get_stock_list_Manager_Data_Access_Manager(self: Optional[str] = None,
                       market: Optional[str] = None,
    ...
    def get_stock_basic_info(self, codes: List[str]) -> pd.DataFrame:
        ...
        try:
            ...
            query = f"""
            SELECT DISTINCT code, nameFROM stock_info WHERE 1=1
    ...
    def _build_stock_info_query(self, 
        ...
        # 基础查询
        query = """
        SELECT code, name, date, level, open, high, low, close, volume price_rangeFROM stock_info
        WHERE 1=1
        """
    ...
    def get_stocks_by_industry_Manager(self: str) -> List[str]:
        """
        根据行业获取股票列表
    ...
    def get_stock_industry(self, stock_code: str) -> Optional[str]:
        """
        获取股票行业（兼容原有API）
        由于数据库中没有industry字段，根据股票代码前缀推断市场类型
        ...
        """
        try:
            # 由于数据库中没有industry字段，我们根据股票代码前缀推断市场类型
    ...
    def get_stocks_data_batch_Manager(self, codes: List[str], start_date: str, 
        ...
        try:
            ...
            code_list = "', '".join(codes)
            query = f"""
            SELECT code, date, open, high, low, close, volume, amount price_change_pct, nameFROM stock_info WHERE 1=1
            WHERE code IN ('{code_list}')
              AND level = %(level)s
    ...
    def get_industry_list_Manager_Data_Access_Manager(self) -> List[str]:
        ...
        try:
            ...
            # 转换为列表
            if not industry_df.empty and in industry_df.columns:
                return industry_df[].tolist()
            else:
                return []
    ...
    def get_stock_info(self, code: str, level: Optional[str] = None,
        ...
        try:
            ...
            query = f"""
            SELECT code, name, date, level, open, close, high, low, volume price_rangeFROM stock_info
