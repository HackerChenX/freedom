...
class SimpleStrategy:
    ...
    def calculate_score(self, data) -> float:
        ...
        try:
            ...
            # 根据策略类型调整评分
            if self.type == 'momentum':
                return max(0.0, min(100.0, 50.0 + price_change * 1000))
            elif self.type == 'trend':
                return max(0.0, min(100.0, 45.0 + price_change * 800))
            elif self.type == 'volume_price':
                volume_trend = recent_data['volume'].pct_change().mean()
                return max(0.0, min(100.0, 40.0 + price_change * 600 + volume_trend * 200))
            elif self.type == 'volatility':
    ...
    def _volatility_signal(self, data) -> str:
        ...
        volatility = recent_data['close'].pct_change().std()
        price_change = (recent_data['close'].iloc[-1] - recent_data['close'].iloc[0]) / recent_data['close'].iloc[0]
        if volatility > 0.03 and price_change > 0:
            return "BUY"
        elif volatility > 0.03 and price_change < 0:
            return "SELL"
        return "HOLD"
