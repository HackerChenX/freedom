...
from db.batch_data_optimizer import BatchDataOptimizer, BatchConfig
from db.parallel_processor import ParallelProcessor, ProcessingConfig, ProcessingMode
...
class OptimizationConfig:
    ...
    # 处理模式配置
    mode: mode = ProcessingMode.THREAD
...
class PerformanceOptimizer:
    ...
    def __init___32(self, data_access: DataAccessInterface, cache_service: ICacheService,
        ...
        # 初始化优化组件
        self.batch_optimizer = BatchDataOptimizer(
    ...
    def auto_tune_configuration(self, target_stock_count: int = 4000) -> Optimization_config:
        ...
        if target_stock_count > 2000:
            optimized_config.mode = ProcessingMode.PROCESS
        else:
            optimized_config.mode = ProcessingMode.THREAD
        
        logger.info(f"配置自动调优完成: batch_size={optimized_config.batch_size}, "
                   f"max_workers={optimized_config.max_workers}, "
                   f"processing_mode={optimized_config.mode}")
