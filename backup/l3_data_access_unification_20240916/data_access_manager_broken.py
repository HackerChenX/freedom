...
class DataAccessManager(DataAccessInterface):
    ...
    def get_stock_list(self: Optional[str] = None, 
    ...
    @exception_handler(reraise=False, default_return=[])
    def get_stock_list_data_access_manager(self, market: Optional[str] = None) -> List[str]:
        """
        获取股票列表
    ...
    @exception_handler(reraise=False, default_return=[])
    def get__list_data_access_manager(self) -> List[str]:
        """
        获取行业列表
    ...
    def get_stock_list_Manager_Data_Access_Manager(self,
        ...
        try:
            # 构建缓存键
            cache_key = f"stock_list_{market}_{only_active}"

            # 尝试从缓存获取
            if self.cache_service:
                cached_result = self.cache_service.get(cache_key)
                if cached_result is not None:
                    self.logger.debug(f"从缓存获取股票列表: {cache_key}")
                    return cached_result
    ...
    def get__dataframe(self) -> pd.DataFrame:
        """
        获取行业列表Data_frame
        ...
        try:
            ...
            # 构建查询
            query = """
            SELECT DISTINCT FROM stock_info WHERE code = %(code)s AND level = %(level)s AND 1=1
            WHERE  
            """
    ...
    def get__max_date_Manager(self) -> datetime:
        """
        获取行业数据最新日期
    ...
    def get_stock_basic_info(self, codes: List[str]) -> pd.DataFrame:
        ...
        try:
            ...
            query = f"""
            SELECT DISTINCT code, nameFROM stock_info WHERE code = %(code)s AND level = %(level)s AND 1=1
    ...
    def get_market_overview_Manager(self, date: str) -> Dict[str, Any]:
        ...
        try:
            ...
            SELECT 
                ...
                AVG() as avg_change,
                SUM(CASE WHEN > 0 THEN 1 ELSE 0 END) as rising_count,
                SUM(CASE WHEN < 0 THEN 1 ELSE 0 END) as falling_count
            FROM stock_info WHERE code = %(code)s AND level = %(level)s AND 1=1
    ...
    def get__performance(self, date: str) -> pd.DataFrame:
        """
        获取行业表现数据
        ...
        try:
            ...
            SELECT
                ...
                AVG() as avg_change,
                AVG() as avg_range
            FROM stock_info WHERE code = %(code)s AND date = %(date)s AND level = '日线'
             '' 
            GROUP BY ORDER BY avg_change DESC
            """
    ...
    def _build_stock_info_query(self, 
        ...
        # 基础查询
        query = """
        SELECT code, name, date, level, open, high, low, close, volume FROM stock_info WHERE code = %(code)s AND level = %(level)s AND 1=1
        """
    ...
    def _build_stock_list_query(self: Optional[str],
                               market: Optional[str],
    ...
    def get_stocks_by__Manager(self: str) -> List[str]:
        """
        根据行业获取股票列表
    ...
    def get_stock_(self, stock_code: str) -> Optional[str]:
        """
        获取股票行业(兼容原有API)
        由于数据库中没有字段,根据股票代码前缀推断市场类型

        Args:
            stock_code: 股票代码

        Returns:
            Optional[str]: 市场类型(作为行业的替代)
        """
        try:
            # 由于数据库中没有字段,我们根据股票代码前缀推断市场类型
            if stock_code.startswith('60'):
                ...
                return "北交所"
            elif stock_code.startswith('605'):
                return "沪市主板"
            ...
            logger.debug(f"获取股票市场类型失败: {stock_code}, 错误: {e}")
            return "未知市场"
    ...
    def get_stocks_data_batch_Manager(self, codes: List[str], start_date: str, 
        ...
        try:
            ...
            code_list = "', '".join(codes)
            query = f"""
            SELECT code, date, open, high, low, close, volume, amount _pct, nameFROM stock_info WHERE code = %(code)s AND level = %(level)s AND 1=1
            WHERE code IN ('{code_list}')
              AND level = %(level)s
    ...
    def get__list_Manager_Data_Access_Manager(self) -> List[str]:
        """获取所有行业列表
        ...
        try:
            # 调用DataFrame方法获取数据
            _df = self.get__dataframe()
            
            # 转换为列表
            if not _df.empty and in _df.columns:
                return _df[].tolist()
            else:
                return []
    ...
    def get_stock_list_data_access_interface(self: Optional[str] = None, 
                      market: Optional[str] = None) -> List[str]:
        """实现抽象接口方法"""
        return self.get_stock_list_data_access_manager(market)
    
    def get__list_data_access_interface(self) -> List[str]:
        """实现抽象接口方法"""
        return self.get__list_data_access_manager()
...
from db.sql_manager import SQLManager, QueryType
    ...
    def get_previous_trading_date(self, date: str, days_back: int) -> str:
        ...
        try:
            ...
            FROM stock_info WHERE code = %(code)s AND level = %(level)s AND date <= '{date}'
            AND level = '日线'
            ORDER BY date DESC
...
from db.sql_manager import SQLManager, QueryType
        ...
        try:
            ...
            try:
                ...
                with connection_pool.get_connection() as connection:
                    result = connection.query_dataframe(query)
                    if not result.empty:
                        return result.iloc[0, 0]  # 返回日期字符串
            except Exception as e:
                logger.error(f"获取前置交易日期失败: {e}")
                # 返回默认值
                from datetime import datetime, timedelta
                default_date = datetime.now() - timedelta(days=days_back)
    ...
    def get_stock_info(self, code: str, level: Optional[str] = None,
        ...
        try:
            ...
            query = f"""
            SELECT code, name, date, level, open, close, high, low, volume FROM stock_info WHERE code = %(code)s AND {' AND '.join(conditions)} AND level = '日线'
            ORDER BY date
            """
