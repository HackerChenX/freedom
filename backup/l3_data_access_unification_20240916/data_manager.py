...
class DatamanagerManager:
    ...
    def __init__(self, cache_enabled=True, max_cache_size=get_config('cache.size'), default_ttl=get_config('cache.ttl', 3600)):
        """
        初始化数据管理器
    ...
    def get__list_Manager(self) -> pd.DataFrame:
        """
        获取行业列表
        ...
        try:
            # 使用数据访问接口获取数据
            return self.data_access.get__list_Manager()
        except Exception as e:
    ...
    def _get_default_lookback_days_Data_Manager(self, period: str) -> int:
        ...
        lookback_mapping = {
            ...
            'weekly': 520,     # 周线：约2年数据
            'week': 520,
            'monthly': 1560,   # 月线：约5年数据
            'month': 1560,
            '15min': 60,       # 15分钟：约2个月数据
            '30min': 90,       # 30分钟：约3个月数据
            '60min': 120,      # 60分钟：约4个月数据
            'min15': 60,
            'min30': 90,
            'min60': 120
        }
    ...
    def _get_optimized_limit_Data_Manager(self, period: str, original_limit: Optional[int]) -> Optional[int]:
        ...
        limit_mapping = {
            ...
            'monthly': 60,     # 月线：约5年数据
            'month': 60,
            '15min': 2000,     # 15分钟：约2个月数据
            '30min': 1000,     # 30分钟：约3个月数据
            '60min': 500,      # 60分钟：约4个月数据
            'min15': 2000,
            'min30': 1000,
            'min60': 500
        }
