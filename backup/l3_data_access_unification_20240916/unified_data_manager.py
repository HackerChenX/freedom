...
from db.sql_manager import SQLManager, QueryType
    ...
    def get_stock_list(self, 
        ...
        Args:
            ...
            FROM stock_info WHERE code = %(code)s AND level = %(level)s AND {' AND '.join(conditions)}
            ORDER BY code
            """
    ...
    def _build_optimized_query(self,
        ...
        {query_hints}
        SELECT {field_str}
        FROM stock_info WHERE code = %(code)s AND level = %(level)s AND {where_clause}
        ORDER BY {order_by}
        """
    ...
    def get_stock_info_info(self, stock_codes: List[str]) -> pd.DataFrame:
        ...
        try:
            ...
            if not result.empty:
                ...
                # 只保留需要的列并去重
                result = result[['stock_code', 'stock_name', ]].drop_duplicates()
    ...
    def get__list(self) -> pd.DataFrame:
        """
        获取行业列表（兼容原有API）
        ...
        try:
            with self.connection_pool.get_connection() as conn:
                query = "SELECT DISTINCT FROM stock_info WHERE code = %(code)s AND level = %(level)s AND IS NOT NULL ORDER BY result = conn.query_dataframe(query)
                return result
        except Exception as e:
