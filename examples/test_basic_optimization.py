...
用于测试MA、RSI和MACD指标优化后的评分效果，不使用绘图功能
"""
...
def test_ma_optimization(data):
    ...
    for idx in sample_indices:
        ...
        print(f"日期: {date}, 优化前: {old_score.iloc[idx]:.2f}, 优化后: {new_score.iloc[idx]:.2f}, 变化: {new_score.iloc[idx] - old_score.iloc[idx]:.2f}")
...
def test_rsi_optimization(data):
    ...
    for idx in sample_indices:
        date = data.index[idx].strftime('%Y-%m-%d')
        rsi_value = data['RSI14'].iloc[idx]
        print(f"日期: {date}, RSI: {rsi_value:.2f}, 优化前: {old_score.iloc[idx]:.2f}, 优化后: {new_score.iloc[idx]:.2f}, 变化: {new_score.iloc[idx] - old_score.iloc[idx]:.2f}")
...
def test_macd_optimization(data):
    ...
    for idx in sample_indices:
        date = data.index[idx].strftime('%Y-%m-%d')
        dif = data['DIF'].iloc[idx]
        dea = data['DEA'].iloc[idx]
        macd = data['MACD'].iloc[idx]
        print(f"日期: {date}, DIF: {dif:.4f}, DEA: {dea:.4f}, MACD: {macd:.4f}")
        print(f"优化前: {old_score.iloc[idx]:.2f}, 优化后: {new_score.iloc[idx]:.2f}, 变化: {new_score.iloc[idx] - old_score.iloc[idx]:.2f}")
